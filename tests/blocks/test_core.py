...
from syntask.testing.utilities import AsyncMock, assert_blocks_equal
...
class TestAPICompatibility:
    ...
    async def test_create_block_schema_uses_syntask_version_for_built_in_blocks(self):
        try:
    ...
    async def test_block_load(
        self, test_block, block_document, in_memory_syntask_client
    ):
        my_block = await test_block.load(
            block_document.name, client=in_memory_syntask_client
        )
    ...
    @patch("syntask.blocks.core.load_syntask_collections")
    async def test_block_load_loads_collections(
        self,
        mock_load_syntask_collections,
        test_block,
        block_document: BlockDocument,
        in_memory_syntask_client,
    ):
        await Block.load(
            block_document.block_type.slug + "/" + block_document.name,
            client=in_memory_syntask_client,
        )
    ...
    async def test_load_nested_block(self, session, in_memory_syntask_client):
        class B(Block):
        ...
        block_instance = await E.load(
            "outer-block-document", client=in_memory_syntask_client
        )
    ...
    async def test_save_protected_block_with_new_block_schema_version(
        self, session, syntask_client: SyntaskClient
    ):
        """
...
class TestRegisterBlockTypeAndSchema:
    ...
    async def test_register_nested_block_list(self, syntask_client: SyntaskClient):
        class A(Block):
            a: str
        ...
        list_collection_block_type = await syntask_client.read_block_type_by_slug(
            slug="listcollection"
        )
        ...
        list_collection_block_type = await syntask_client.read_block_schema_by_checksum(
            checksum=ListCollection._calculate_schema_checksum()
        )
        assert list_collection_block_type is not None

    async def test_register_nested_block_tuple(self, syntask_client: SyntaskClient):
        class A(Block):
            a: str
        ...
        tuple_collection_block_type = await syntask_client.read_block_type_by_slug(
        ...
        tuple_collection_block_type = (
            await syntask_client.read_block_schema_by_checksum(
                checksum=TupleCollection._calculate_schema_checksum()
            )
        )
        assert tuple_collection_block_type is not None

    async def test_register_nested_block_dict(self, syntask_client: SyntaskClient):
        class A(Block):
            a: str
        ...
        dict_collection_block_type = await syntask_client.read_block_type_by_slug(
            slug="dictcollection"
        )
        ...
        dict_collection_block_type = await syntask_client.read_block_schema_by_checksum(
    ...
    async def test_register_nested_block_type_nested(
        ...
        depth_block_type = await syntask_client.read_block_type_by_slug(slug="depth")
        ...
        depth_block_type = await syntask_client.read_block_schema_by_checksum(
...
class TestSaveBlock:
    ...
    async def test_save_and_load_block_with_secrets_includes_secret_data(
        ...
        api_block = await syntask_client.read_block_document(
