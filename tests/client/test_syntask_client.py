...
import pytest
import respx
from syntask._vendor.fastapi import Depends, FastAPI, status
from syntask._vendor.fastapi.security import HTTPBearer
...
from syntask.client.orchestration import SyntaskClient, ServerType, get_client
...
from syntask.client.schemas.schedules import CronSchedule, IntervalSchedule, NoSchedule
...
class TestInjectClient:
    ...
    async def test_use_existing_client_from_flow_run_ctx(self, syntask_client):
        with syntask.context.FlowRunContext.construct(client=syntask_client):
            client = await TestInjectClient.injected_func()
        assert client is syntask_client, "Client should be the same object"
        assert not client._closed, "Client should not be closed after function returns"
...
async def test_create_then_read_deployment(
    syntask_client, infrastructure_document_id, storage_document_id
):
    @flow
    def foo():
        pass
...
async def test_updating_deployment(
    ...
    initial_lookup = await syntask_client.read_deployment(deployment_id)
    ...
    second_lookup = await syntask_client.read_deployment(deployment_id)
...
async def test_create_flow_run_from_deployment_with_options_emits_warning(
    syntask_client, deployment, enable_infra_overrides
):
    with pytest.warns(
        ExperimentalFeature,
...
async def test_delete_task_run(syntask_client):
    ...
    with pytest.raises(syntask.exceptions.SyntaskHTTPStatusError, match="Not Found"):
...
async def test_syntask_api_tls_insecure_skip_verify_setting_set_to_true(monkeypatch):
    with temporary_settings(updates={SYNTASK_API_TLS_INSECURE_SKIP_VERIFY: True}):
        mock = Mock()
        monkeypatch.setattr("syntask.client.orchestration.SyntaskHttpxClient", mock)
        get_client()
...
class TestResolveDataDoc:
    ...
    async def test_does_not_allow_other_types(self, syntask_client):
        with pytest.raises(TypeError, match="invalid type str"):
            await syntask_client.resolve_datadoc("foo")

    async def test_resolves_data_document(self, syntask_client):
        innermost = await syntask_client.resolve_datadoc(
            DataDocument.encode("cloudpickle", "hello")
        )
        assert innermost == "hello"

    async def test_resolves_nested_data_documents(self, syntask_client):
        innermost = await syntask_client.resolve_datadoc(
...
class TestClientWorkQueues:
    @pytest.fixture
    async def deployment(self, syntask_client, infrastructure_document_id):
    ...
    async def test_create_work_queue_with_tags_deprecated(self, syntask_client):
        with pytest.deprecated_call():
            await syntask_client.create_work_queue(name="test-queue", tags=["a"])

    async def test_get_runs_from_queue_includes(
        self, session, syntask_client, deployment
    ):
...
def test_server_type_ephemeral(syntask_client):
...
async def test_update_deployment_schedule_active_does_not_overwrite_when_not_provided(
    ...
    schedule = IntervalSchedule(interval=timedelta(days=1))
    await syntask_client.update_deployment(deployment, schedule=schedule)
    deployment = await syntask_client.read_deployment(deployment_id)
    assert deployment.is_schedule_active == expected_value
...
async def test_update_deployment_schedule_active_overwrites_when_provided(
    ...
    await syntask_client.update_deployment(deployment, is_schedule_active=on_update)
    deployment = await syntask_client.read_deployment(deployment_id)
...
class TestAutomations:
    ...
    async def test_create_not_cloud_runtime_error(
        self, syntask_client, automation: AutomationCore
    ):
        with pytest.raises(
            RuntimeError,
            match="The current server and client configuration does not support",
        ):
            await syntask_client.create_automation(automation)
    ...
    async def test_delete_owned_automations_not_cloud_runtime_error(
        ...
        with pytest.raises(
            ...
            await syntask_client.delete_resource_owned_automations(resource_id)
...
class TestSyntaskClientDeploymentSchedules:
    ...
    async def test_update_deployment_schedule_success(self, deployment, syntask_client):
...
class TestSyntaskClientCsrfSupport:
    def test_enabled_ephemeral(self, syntask_client: SyntaskClient):
        assert syntask_client.server_type == ServerType.EPHEMERAL
