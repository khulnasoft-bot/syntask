...
def events_page_one() -> List[ReceivedEvent]:
    return [
        ReceivedEvent(
            ...
            resource=Resource(__root__={"syntask.resource.id": "my.resource"}),
...
def query_events(
    ...
    with mock.patch("syntask.server.api.events.query_events") as query_events:
...
def query_next_page(
    ...
    with mock.patch(
        "syntask.server.api.events.query_next_page", new_callable=mock.AsyncMock
    ) as query_next_page:
...
def count_events() -> Generator[mock.AsyncMock, None, None]:
    with mock.patch("syntask.server.api.events.count_events") as count_events:
