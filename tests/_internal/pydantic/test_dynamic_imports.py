...
def test_v2_installed_and_compatibility_layer_disabled():
    ...
    from syntask import pydantic as syntask_pydantic
    from syntask.pydantic import _dynamic_imports  # type: ignore

    # Ensure that all attributes in syntask.pydantic are imported from pydantic v2
    for attr_name in syntask_pydantic.__all__:
        ...
        if attr_name in _dynamic_imports:
            ...
            # Import the attribute from syntask.pydantic, and ensure that it is from pydantic.v1
            attr = getattr(syntask_pydantic, attr_name)
