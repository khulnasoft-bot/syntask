...
def interactive_console(monkeypatch):
    monkeypatch.setattr("syntask.cli.config.is_interactive", lambda: True)
...
def test_view_excludes_unset_settings_without_show_defaults_flag(monkeypatch):
    ...
    for key in syntask.settings.Settings.valid_setting_names():
    ...
    assert "SYNTASK_PROFILE='foo'" in lines

    assert len(expected) < len(
        syntask.settings.Settings.valid_setting_names()
    ), "All settings were not expected; we should only have a subset."


@pytest.mark.skip("TODO")
def test_view_includes_unset_settings_with_show_defaults():
    expected_settings = syntask.settings.get_current_settings().model_dump()
    ...
    assert (
        printed_settings.keys() == syntask.settings.Settings.valid_setting_names()
    ...
    for key, value in printed_settings.items():
        ...
        assert (
            ...
            == (
                expected_value
                := f"'{expected_settings[syntask.settings.env_var_to_attr_name(key)]}'"
            )
...
def test_view_shows_setting_sources(monkeypatch, command):
    ...
    i = next(i for i, line in enumerate(lines) if "SYNTASK_PROFILE" in line)
    assert lines[i] == "SYNTASK_PROFILE='foo'"
