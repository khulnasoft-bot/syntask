...
async def read_queue(syntask_client, name, pool=None):
    return await syntask_client.read_work_queue_by_name(name=name, work_pool_name=pool)
...
class TestSetConcurrencyLimit:
    async def test_set_concurrency_limit(self, syntask_client, work_queue):
        ...
        q = await read_queue(syntask_client, work_queue.name)
        assert q.concurrency_limit == 5

    async def test_set_concurrency_limit_by_id(self, syntask_client, work_queue):
        assert work_queue.concurrency_limit is None
        await run_sync_in_worker_thread(
            invoke_and_assert,
    ...
    async def test_set_concurrency_limit_with_pool_with_name(
        ...
        q = await read_queue(
            syntask_client, work_queue_1.name, pool=work_queue_1.work_pool.name
        )
        assert q.concurrency_limit == 5
...
class TestClearConcurrencyLimit:
    async def test_clear_concurrency_limit(self, syntask_client, work_queue):
        await run_sync_in_worker_thread(
            invoke_and_assert,
            command=f"work-queue set-concurrency-limit {work_queue.name} 5",
        )
    ...
    async def test_clear_concurrency_limit_by_id(self, syntask_client, work_queue):
        await run_sync_in_worker_thread(
            invoke_and_assert,
            command=f"work-queue set-concurrency-limit {work_queue.name} 5",
        )
    ...
    async def test_clear_concurrency_limit_with_pool(
        ...
        await syntask_client.update_work_queue(id=work_queue_1.id, concurrency_limit=5)
        ...
        q = await read_queue(syntask_client, work_queue_1.name, pool=pool_name)
        assert q.concurrency_limit is None
...
class TestPauseWorkQueue:
    async def test_pause(self, syntask_client, work_queue):
    ...
    async def test_pause_by_id(self, syntask_client, work_queue):
        assert not work_queue.is_paused
        await run_sync_in_worker_thread(
            invoke_and_assert,
...
class TestResumeWorkQueue:
    async def test_resume(self, syntask_client, work_queue):
        await run_sync_in_worker_thread(
            invoke_and_assert,
            command=f"work-queue pause {work_queue.name} --pool default-agent-pool",
        )
    ...
    async def test_resume_by_id(self, syntask_client, work_queue):
        await run_sync_in_worker_thread(
            invoke_and_assert,
            command=f"work-queue pause {work_queue.name} --pool default-agent-pool",
        )
...
class TestDelete:
    async def test_delete(self, syntask_client, work_queue):
        await run_sync_in_worker_thread(
            invoke_and_assert,
        ...
        with pytest.raises(syntask.exceptions.ObjectNotFound):
            await read_queue(syntask_client, work_queue.name)

    async def test_delete_by_id(self, syntask_client, work_queue):
        await run_sync_in_worker_thread(
            invoke_and_assert,
    ...
    async def test_delete_with_pool(
        ...
        with pytest.raises(syntask.exceptions.ObjectNotFound):
            await read_queue(syntask_client, work_queue_1.name, pool=pool_name)
    ...
    async def test_delete_with_bad_pool(
        ...
        assert await read_queue(syntask_client, work_queue_1.name, pool=pool_name)

    async def test_delete_with_bad_queue_name(self, syntask_client, work_queue):
        await run_sync_in_worker_thread(
            invoke_and_assert,
            command=f"work-queue delete {work_queue.name}bad",
            expected_code=1,
        )
        assert await read_queue(syntask_client, work_queue.name)
...
class TestReadRuns:
    async def create_runs_in_queue(self, syntask_client, queue, count: int):
        ...
        for _ in range(count):
            await syntask_client.create_flow_run_from_deployment(deployment_id)

    async def test_read_wq(self, syntask_client, work_queue):
        n_runs = 3
        await self.create_runs_in_queue(syntask_client, work_queue, n_runs)
    ...
    async def test_read_wq_with_pool(self, syntask_client, work_queue):
        n_runs = 3
        await self.create_runs_in_queue(syntask_client, work_queue, n_runs)
        cmd = (
