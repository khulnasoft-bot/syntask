...
def main():
    # Create deployment
    if Version(syntask.__version__) < Version("2.1.0"):
    ...
    if Version(syntask.__version__) < Version("2.1"):
        # work queue is positional instead of a flag and requires creation
        subprocess.run(["syntask", "work-queue", "create", "test"])
        try:
            subprocess.run(["syntask", "agent", "start", "test"], timeout=30)
        except subprocess.TimeoutExpired:
            pass
    elif (
        Version(syntask.__version__) > Version("2.6")
        ...
        return
    elif Version(syntask.__version__) < Version("2.6"):
        # --run-once is not available so just run for a bit
        try:
            subprocess.run(
                ["syntask", "agent", "start", "--work-queue", "default"], timeout=30
            )
        except subprocess.TimeoutExpired:
            pass
    else:
        subprocess.check_call(
            ["syntask", "agent", "start", "--run-once", "--work-queue", "default"],
        )
