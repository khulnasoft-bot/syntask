...
from syntask.settings import SYNTASK_API_KEY, SYNTASK_API_URL
...
class SyntaskCloudEventSubscriber:
    """
    Subscribes to a Syntask Cloud event stream, yielding events as they occur.
    ...
    Example:
        ...
        async with SyntaskCloudEventSubscriber(api_url, api_key, filter) as subscriber:
    ...
    async def __aenter__(self) -> "SyntaskCloudEventSubscriber":
        # Don't handle any errors in the initial connection, because these are most
        # likely a permission or configuration issue that should propagate
        await self._reconnect()
        return self
    ...
    def __aiter__(self) -> "SyntaskCloudEventSubscriber":
        return self
