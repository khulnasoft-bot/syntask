...
- **The flow is called by the user or an existing flow run is executed in a new process.**
    ...
    See `Flow.__call__` and `syntask.engine.__main__` (`python -m syntask.engine`)
...
- **The task run is orchestrated through states, calling the user's function as necessary.**
    ...
    user thread as we do for flows. See [#9855](https://github.com/Synopkg/syntask/pull/9855).
"""
...
from syntask._internal.compatibility.experimental import experimental_parameter
from syntask._internal.concurrency.api import create_call, from_async, from_sync
from syntask._internal.concurrency.calls import get_current_call
...
from syntask.context import (
    FlowRunContext,
    SyntaskObjectRegistry,
    TagsContext,
    TaskRunContext,
)
from syntask.deployments import load_flow_from_flow_run
...
from syntask.futures import SyntaskFuture, call_repr, resolve_futures_to_states
...
from syntask.logging.handlers import APILogHandler
...
def enter_flow_run_engine_from_flow_call(
    ...
    wait_for: Optional[Iterable[SyntaskFuture]],
    ...
    registry = SyntaskObjectRegistry.get()
...
async def begin_flow_run(
    ...
    display_state = repr(terminal_state) if SYNTASK_DEBUG_MODE else str(terminal_state)
...
async def orchestrate_flow_run(
    ...
    while state.is_running():
        ...
        # block storage and send the resulting data document to the Syntask API instead.
        # This prevents the pickled return value of flow runs
        # from being sent to the Syntask API and stored in the Syntask database.
        ...
        if state.type != terminal_state.type and SYNTASK_DEBUG_MODE:
            logger.debug(
                (
...
def enter_task_run_engine(
    ...
    mapped: bool,
) -> Union[SyntaskFuture, Awaitable[SyntaskFuture], TaskRun]:
    ...
    if not flow_run_context:
        if return_type == "future" or mapped:
            raise RuntimeError(
                ...
                " `syntask config set SYNTASK_EXPERIMENTAL_ENABLE_TASK_SCHEDULING=true`"
                " and use `your_task.submit()` instead of `your_task()`."
            )
        from syntask.task_engine import submit_autonomous_task_run_to_engine
...
) -> List[Union[SyntaskFuture, Awaitable[SyntaskFuture], TaskRun]]:
...
async def begin_task_run(
    ...
    log_prints: bool,
    settings: syntask.context.SettingsContext,
):
    """
    ...
    remote worker:
    --> Remote worker imports Syntask (may not occur)
    ...
    """
    maybe_flow_run_context = syntask.context.FlowRunContext.get()
    ...
    async with AsyncExitStack() as stack:
        ...
        if syntask.context.SettingsContext.get() != settings:
...
async def orchestrate_task_run(
    ...
    threshold = SYNTASK_TASK_INTROSPECTION_WARN_THRESHOLD.value()
    if threshold and introspection_time > threshold:
        logger.warning(
            f"Task parameter introspection took {introspection_time} seconds "
            f", exceeding `SYNTASK_TASK_INTROSPECTION_WARN_THRESHOLD` of {threshold}. "
            "Try wrapping large task parameters with "
            "`syntask.utilities.annotations.quote` for increased performance, "
            "e.g. `my_task(quote(param))`. To disable this message set "
            "`SYNTASK_TASK_INTROSPECTION_WARN_THRESHOLD=0`."
        )
    ...
    # Setting on task level overrules the Syntask setting (env var)
    refresh_cache = (
        task.refresh_cache
    ...
    # If debugging, use the more complete `repr` than the usual `str` description
    display_state = repr(state) if SYNTASK_DEBUG_MODE else str(state)
...
@asynccontextmanager
async def report_flow_run_crashes(flow_run: FlowRun, client: SyntaskClient, flow: Flow):
    """
...
@asynccontextmanager
async def report_task_run_crashes(task_run: TaskRun, client: SyntaskClient):
    """
...
async def _run_flow_hooks(flow: Flow, flow_run: FlowRun, state: State) -> None:
    ...
    enable_cancellation_and_crashed_hooks = (
        os.environ.get("SYNTASK__ENABLE_CANCELLATION_AND_CRASHED_HOOKS", "true").lower()
        == "true"
    )
