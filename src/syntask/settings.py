...
The `SettingsContext` is set when the `syntask` module is imported. This context is
...
management should be left to Syntask application internals.
...
from syntask._internal.schemas.validators import validate_settings
...
from syntask.utilities.names import OBFUSCATED_PREFIX, obfuscate
from syntask.utilities.pydantic import add_cloudpickle_reduction
...
REMOVED_EXPERIMENTAL_FLAGS = {
    "SYNTASK_EXPERIMENTAL_ENABLE_ENHANCED_SCHEDULING_UI",
    "SYNTASK_EXPERIMENTAL_ENABLE_ENHANCED_DEPLOYMENT_PARAMETERS",
    "SYNTASK_EXPERIMENTAL_ENABLE_EVENTS_CLIENT",
    "SYNTASK_EXPERIMENTAL_WARN_EVENTS_CLIENT",
}
...
class Setting(Generic[T]):
    ...
    def value_from(self, settings: "Settings", bypass_callback: bool = False) -> T:
        ...
        Example:
        ```python
        from syntask.settings import get_default_settings
        SYNTASK_API_URL.value_from(get_default_settings())
        ```
        """
    ...
    def name(self):
        ...
        raise ValueError("Setting not found in `syntask.settings` module.")
...
def get_extra_loggers(_: "Settings", value: str) -> List[str]:
    """
    `value_callback` for `SYNTASK_LOGGING_EXTRA_LOGGERS`that parses the CSV string into a
...
def debug_mode_log_level(settings, value):
    """
    `value_callback` for `SYNTASK_LOGGING_LEVEL` that overrides the log level to DEBUG
    when debug mode is enabled.
    """
    if SYNTASK_DEBUG_MODE.value_from(settings):
        return "DEBUG"
    else:
        return value
...
def only_return_value_in_test_mode(settings, value):
    """
    `value_callback` for `SYNTASK_TEST_SETTING` that only allows access during test mode
    """
    if SYNTASK_TEST_MODE.value_from(settings):
        return value
    else:
        return None
...
def default_ui_api_url(settings, value):
    """
    `value_callback` for `SYNTASK_UI_API_URL` that sets the default value to
    ...
    return template_with_settings(
        SYNTASK_SERVER_API_HOST, SYNTASK_SERVER_API_PORT, SYNTASK_API_URL
    )(settings, value)


def status_codes_as_integers_in_range(_, value):
    """
    `value_callback` for `SYNTASK_CLIENT_RETRY_EXTRA_CODES` that ensures status codes
    ...
    if any(not v.isdigit() or int(v) < 100 or int(v) > 599 for v in values):
        raise ValueError(
            "SYNTASK_CLIENT_RETRY_EXTRA_CODES must be a comma separated list of "
...
def max_log_size_smaller_than_batch_size(values):
    ...
    """
    if (
        values["SYNTASK_LOGGING_TO_API_BATCH_SIZE"]
        < values["SYNTASK_LOGGING_TO_API_MAX_LOG_SIZE"]
    ):
        raise ValueError(
...
def warn_on_database_password_value_without_usage(values):
    ...
    """
    value = values["SYNTASK_API_DATABASE_PASSWORD"]
    if (
        value
        and not value.startswith(OBFUSCATED_PREFIX)
        and (
            "SYNTASK_API_DATABASE_PASSWORD"
            not in values["SYNTASK_API_DATABASE_CONNECTION_URL"]
        )
    ):
        warnings.warn(
...
def check_for_deprecated_cloud_url(settings, value):
    deprecated_value = SYNTASK_CLOUD_URL.value_from(settings, bypass_callback=True)
    if deprecated_value is not None:
        warnings.warn(
            (
                "`SYNTASK_CLOUD_URL` is set and will be used instead of"
                " `SYNTASK_CLOUD_API_URL` for backwards compatibility."
                " `SYNTASK_CLOUD_URL` is deprecated, set `SYNTASK_CLOUD_API_URL`"
                " instead."
            ),
            DeprecationWarning,
        )
...
def warn_on_misconfigured_api_url(values):
    ...
    """
    api_url = values["SYNTASK_API_URL"]
...
def default_database_connection_url(settings, value):
    templater = template_with_settings(SYNTASK_HOME, SYNTASK_API_DATABASE_PASSWORD)
    ...
    home = SYNTASK_HOME.value_from(settings)

    old_default = home / "orion.db"
    new_default = home / "syntask.db"
...
def default_ui_url(settings, value):
    ...
    ui_url = api_url = SYNTASK_API_URL.value_from(settings)

    if not api_url:
        return None

    cloud_url = SYNTASK_CLOUD_API_URL.value_from(settings)
    cloud_ui_url = SYNTASK_CLOUD_UI_URL.value_from(settings)
...
def default_cloud_ui_url(settings, value):
    ...
    # Otherwise, infer a value from the API URL
    ui_url = api_url = SYNTASK_CLOUD_API_URL.value_from(settings)

    if api_url.startswith("https://api.syntask.cloud"):
        ui_url = ui_url.replace(
            "https://api.syntask.cloud", "https://app.syntask.cloud", 1
        )
...
SYNTASK_HOME = Setting(
    Path,
    default=Path("~") / ".syntask",
    value_callback=expanduser_in_path,
)
"""Syntask's home directory. Defaults to `~/.syntask`. This
directory may be created automatically when required.
"""

SYNTASK_EXTRA_ENTRYPOINTS = Setting(
    str,
    default="",
)
"""
Modules for Syntask to import when Syntask is imported.
...
SYNTASK_CLI_WRAP_LINES = Setting(
    bool,
    default=True,
)
...
SYNTASK_UNIT_TEST_MODE = Setting(
    bool,
    default=False,
)
"""
...
"""
SYNTASK_UNIT_TEST_LOOP_DEBUG = Setting(
    bool,
    default=True,
)
"""
...
If accessed when `SYNTASK_TEST_MODE` is not set, `None` is returned.
"""

SYNTASK_API_TLS_INSECURE_SKIP_VERIFY = Setting(
    bool,
    default=False,
)
...
SYNTASK_API_SSL_CERT_FILE = Setting(
    str,
...
SYNTASK_API_URL = Setting(
    str,
    default=None,
)
"""
If provided, the URL of a hosted Syntask API. Defaults to `None`.

When using Syntask Cloud, this will include an account and workspace.
"""

SYNTASK_SILENCE_API_URL_MISCONFIGURATION = Setting(
    bool,
    default=False,
)
...
SYNTASK_API_KEY = Setting(
    str,
    default=None,
    is_secret=True,
)
"""API key used to authenticate with a the Syntask API. Defaults to `None`."""

SYNTASK_API_ENABLE_HTTP2 = Setting(bool, default=True)
"""
...
SYNTASK_CLIENT_MAX_RETRIES = Setting(int, default=5)
"""
...
SYNTASK_CLIENT_RETRY_JITTER_FACTOR = Setting(float, default=0.2)
"""
...
SYNTASK_CLIENT_RETRY_EXTRA_CODES = Setting(
...
SYNTASK_CLIENT_CSRF_SUPPORT_ENABLED = Setting(bool, default=True)
"""
...
SYNTASK_CLOUD_API_URL = Setting(
    str,
    default="https://api.syntask.cloud/api",
    value_callback=check_for_deprecated_cloud_url,
)
"""API URL for Syntask Cloud. Used for authentication."""


SYNTASK_CLOUD_URL = Setting(
    str,
    default=None,
    deprecated=True,
    deprecated_start_date="Dec 2022",
    deprecated_help="Use `SYNTASK_CLOUD_API_URL` instead.",
)
"""
DEPRECATED: Use `SYNTASK_CLOUD_API_URL` instead.
"""

SYNTASK_UI_URL = Setting(
    Optional[str],
    default=None,
    value_callback=default_ui_url,
)
"""
The URL for the UI. By default, this is inferred from the SYNTASK_API_URL.

When using Syntask Cloud, this will include the account and workspace.
When using an ephemeral server, this will be `None`.
"""


SYNTASK_CLOUD_UI_URL = Setting(
    str,
    default=None,
    value_callback=default_cloud_ui_url,
)
"""
The URL for the Cloud UI. By default, this is inferred from the SYNTASK_CLOUD_API_URL.

Note: SYNTASK_UI_URL will be workspace specific and will be usable in the open source too.
      In contrast, this value is only valid for Cloud and will not include the workspace.
"""

SYNTASK_API_REQUEST_TIMEOUT = Setting(
    float,
    default=60.0,
)
"""The default timeout for requests to the API"""

SYNTASK_EXPERIMENTAL_WARN = Setting(bool, default=True)
"""
If enabled, warn on usage of experimental features.
"""

SYNTASK_PROFILES_PATH = Setting(
    Path,
    default=Path("${SYNTASK_HOME}") / "profiles.toml",
    value_callback=template_with_settings(SYNTASK_HOME),
)
"""The path to a profiles configuration files."""

SYNTASK_RESULTS_DEFAULT_SERIALIZER = Setting(
    str,
    default="pickle",
)
...
SYNTASK_RESULTS_PERSIST_BY_DEFAULT = Setting(
    bool,
    default=False,
)
"""
...
SYNTASK_TASKS_REFRESH_CACHE = Setting(
    bool,
    default=False,
)
"""
...
SYNTASK_TASK_DEFAULT_RETRIES = Setting(int, default=0)
"""
...
SYNTASK_FLOW_DEFAULT_RETRIES = Setting(int, default=0)
"""
...
SYNTASK_FLOW_DEFAULT_RETRY_DELAY_SECONDS = Setting(Union[int, float], default=0)
"""
This value sets the retry delay seconds for all flows.
This value does not overwrite individually set retry delay seconds
"""

SYNTASK_TASK_DEFAULT_RETRY_DELAY_SECONDS = Setting(
...
SYNTASK_TASK_RUN_TAG_CONCURRENCY_SLOT_WAIT_SECONDS = Setting(int, default=30)
"""
The number of seconds to wait before retrying when a task run
cannot secure a concurrency slot from the server.
"""

SYNTASK_LOCAL_STORAGE_PATH = Setting(
    Path,
    default=Path("${SYNTASK_HOME}") / "storage",
    value_callback=template_with_settings(SYNTASK_HOME),
)
"""The path to a block storage directory to store things in."""

SYNTASK_MEMO_STORE_PATH = Setting(
    Path,
    default=Path("${SYNTASK_HOME}") / "memo_store.toml",
    value_callback=template_with_settings(SYNTASK_HOME),
)
"""The path to the memo store file."""

SYNTASK_MEMOIZE_BLOCK_AUTO_REGISTRATION = Setting(
    bool,
    default=True,
)
"""
...
SYNTASK_LOGGING_LEVEL = Setting(
    str,
    default="INFO",
    value_callback=debug_mode_log_level,
)
"""
The default logging level for Syntask loggers. Defaults to
"INFO" during normal operation. Is forced to "DEBUG" during debug mode.
"""


SYNTASK_LOGGING_INTERNAL_LEVEL = Setting(
    str,
    default="ERROR",
    value_callback=debug_mode_log_level,
)
"""
The default logging level for Syntask's internal machinery loggers. Defaults to
"ERROR" during normal operation. Is forced to "DEBUG" during debug mode.
"""

SYNTASK_LOGGING_SERVER_LEVEL = Setting(
    str,
    default="WARNING",
)
"""The default logging level for the Syntask API server."""

SYNTASK_LOGGING_SETTINGS_PATH = Setting(
    Path,
    default=Path("${SYNTASK_HOME}") / "logging.yml",
    value_callback=template_with_settings(SYNTASK_HOME),
)
"""
The path to a custom YAML logging configuration file. If
no file is found, the default `logging.yml` is used.
Defaults to a logging.yml in the Syntask home directory.
"""

SYNTASK_LOGGING_EXTRA_LOGGERS = Setting(
    str,
    default="",
    value_callback=get_extra_loggers,
)
"""
Additional loggers to attach to Syntask logging at runtime.
Values should be comma separated. The handlers attached to the 'syntask' logger
will be added to these loggers. Additionally, if the level is not set, it will
be set to the same level as the 'syntask' logger.
"""

SYNTASK_LOGGING_LOG_PRINTS = Setting(
    bool,
    default=False,
)
"""
If set, `print` statements in flows and tasks will be redirected to the Syntask logger
for the given run. This setting can be overridden by individual tasks and flows.
"""

SYNTASK_LOGGING_TO_API_ENABLED = Setting(
    bool,
    default=True,
)
"""
...
SYNTASK_LOGGING_TO_API_BATCH_INTERVAL = Setting(float, default=2.0)
"""The number of seconds between batched writes of logs to the API."""

SYNTASK_LOGGING_TO_API_BATCH_SIZE = Setting(
    int,
    default=4_000_000,
)
"""The maximum size in bytes for a batch of logs."""

SYNTASK_LOGGING_TO_API_MAX_LOG_SIZE = Setting(
    int,
    default=1_000_000,
)
"""The maximum size in bytes for a single log."""

SYNTASK_LOGGING_TO_API_WHEN_MISSING_FLOW = Setting(
...
SYNTASK_SQLALCHEMY_POOL_SIZE = Setting(
    int,
    default=None,
)
"""
Controls connection pool size when using a PostgreSQL database with the Syntask API. If not set, the default SQLAlchemy pool size will be used.
"""

SYNTASK_SQLALCHEMY_MAX_OVERFLOW = Setting(
    int,
    default=None,
)
"""
Controls maximum overflow of the connection pool when using a PostgreSQL database with the Syntask API. If not set, the default SQLAlchemy maximum overflow value will be used.
"""

SYNTASK_LOGGING_COLORS = Setting(
    bool,
    default=True,
)
"""Whether to style console logs with color."""

SYNTASK_LOGGING_MARKUP = Setting(
    bool,
    default=False,
)
"""
...
SYNTASK_TASK_INTROSPECTION_WARN_THRESHOLD = Setting(
    float,
    default=10.0,
)
"""
...
and each element needs to be inspected. See `syntask.utilities.annotations.quote`
for more details.
Defaults to `10.0`.
Set to `0` to disable logging the warning.
"""

SYNTASK_AGENT_QUERY_INTERVAL = Setting(
    float,
    default=15,
)
"""
...
SYNTASK_ASYNC_FETCH_STATE_RESULT = Setting(bool, default=False)
"""
...
SYNTASK_API_BLOCKS_REGISTER_ON_START = Setting(
    bool,
    default=True,
)
"""
...
SYNTASK_API_DATABASE_PASSWORD = Setting(
    str,
    default=None,
    is_secret=True,
)
"""
Password to template into the `SYNTASK_API_DATABASE_CONNECTION_URL`.
...
SYNTASK_API_DATABASE_CONNECTION_URL = Setting(
    str,
    default=None,
...
Defaults to a sqlite database stored in the Syntask home directory.

If you need to provide password via a different environment variable, you use
the `SYNTASK_API_DATABASE_PASSWORD` setting. For example:

```
SYNTASK_API_DATABASE_PASSWORD='mypassword'
SYNTASK_API_DATABASE_CONNECTION_URL='postgresql+asyncpg://postgres:${SYNTASK_API_DATABASE_PASSWORD}@localhost/syntask'
```
"""

SYNTASK_API_DATABASE_ECHO = Setting(
    bool,
    default=False,
)
"""If `True`, SQLAlchemy will log all SQL issued to the database. Defaults to `False`."""

SYNTASK_API_DATABASE_MIGRATE_ON_START = Setting(
    bool,
    default=True,
)
"""If `True`, the database will be upgraded on application creation. If `False`, the database will need to be upgraded manually."""

SYNTASK_API_DATABASE_TIMEOUT = Setting(
    Optional[float],
    default=10.0,
)
"""
...
SYNTASK_API_DATABASE_CONNECTION_TIMEOUT = Setting(
    Optional[float],
    default=5,
)
...
SYNTASK_API_SERVICES_SCHEDULER_LOOP_SECONDS = Setting(
    float,
    default=60,
)
...
SYNTASK_API_SERVICES_SCHEDULER_DEPLOYMENT_BATCH_SIZE = Setting(
    int,
    default=100,
)
...
SYNTASK_API_SERVICES_SCHEDULER_MAX_RUNS = Setting(
    int,
    default=100,
)
...
SYNTASK_API_SERVICES_SCHEDULER_MIN_RUNS = Setting(
    int,
    default=3,
)
...
SYNTASK_API_SERVICES_SCHEDULER_MAX_SCHEDULED_TIME = Setting(
    timedelta,
...
SYNTASK_API_SERVICES_SCHEDULER_MIN_SCHEDULED_TIME = Setting(
    timedelta,
...
SYNTASK_API_SERVICES_SCHEDULER_INSERT_BATCH_SIZE = Setting(
    int,
    default=500,
)
...
SYNTASK_API_SERVICES_LATE_RUNS_LOOP_SECONDS = Setting(
    float,
    default=5,
)
...
SYNTASK_API_SERVICES_LATE_RUNS_AFTER_SECONDS = Setting(
    timedelta,
...
SYNTASK_API_SERVICES_PAUSE_EXPIRATIONS_LOOP_SECONDS = Setting(
    float,
    default=5,
)
...
SYNTASK_API_SERVICES_CANCELLATION_CLEANUP_LOOP_SECONDS = Setting(
    float,
    default=20,
)
...
SYNTASK_API_DEFAULT_LIMIT = Setting(
    int,
    default=200,
)
...
SYNTASK_SERVER_API_HOST = Setting(
    str,
    default="127.0.0.1",
)
"""The API's host address (defaults to `127.0.0.1`)."""

SYNTASK_SERVER_API_PORT = Setting(
    int,
    default=4200,
)
"""The API's port address (defaults to `4200`)."""

SYNTASK_SERVER_API_KEEPALIVE_TIMEOUT = Setting(
    int,
    default=5,
)
"""
...
SYNTASK_SERVER_CSRF_PROTECTION_ENABLED = Setting(bool, default=False)
"""
...
SYNTASK_SERVER_CSRF_TOKEN_EXPIRATION = Setting(timedelta, default=timedelta(hours=1))
"""
...
SYNTASK_UI_ENABLED = Setting(
    bool,
    default=True,
)
"""Whether or not to serve the Syntask UI."""

SYNTASK_UI_API_URL = Setting(
    str,
    default=None,
    value_callback=default_ui_api_url,
)
"""The connection url for communication from the UI to the API.
Defaults to `SYNTASK_API_URL` if set. Otherwise, the default URL is generated from
`SYNTASK_SERVER_API_HOST` and `SYNTASK_SERVER_API_PORT`. If providing a custom value,
the aforementioned settings may be templated into the given string.
"""

SYNTASK_SERVER_ANALYTICS_ENABLED = Setting(
    bool,
    default=True,
)
"""
...
SYNTASK_API_SERVICES_SCHEDULER_ENABLED = Setting(
    bool,
    default=True,
)
...
SYNTASK_API_SERVICES_LATE_RUNS_ENABLED = Setting(
    bool,
    default=True,
)
...
SYNTASK_API_SERVICES_FLOW_RUN_NOTIFICATIONS_ENABLED = Setting(
    bool,
    default=True,
)
...
SYNTASK_API_SERVICES_PAUSE_EXPIRATIONS_ENABLED = Setting(
    bool,
    default=True,
)
...
SYNTASK_API_TASK_CACHE_KEY_MAX_LENGTH = Setting(int, default=2000)
"""
...
SYNTASK_API_SERVICES_CANCELLATION_CLEANUP_ENABLED = Setting(
    bool,
    default=True,
)
...
SYNTASK_API_MAX_FLOW_RUN_GRAPH_NODES = Setting(int, default=10000)
"""
The maximum size of a flow run graph on the v2 API
"""

SYNTASK_API_MAX_FLOW_RUN_GRAPH_ARTIFACTS = Setting(int, default=10000)
"""
The maximum number of artifacts to show on a flow run graph on the v2 API
"""

SYNTASK_EXPERIMENTAL_ENABLE_ARTIFACTS_ON_FLOW_RUN_GRAPH = Setting(bool, default=True)
"""
Whether or not to enable artifacts on the flow run graph.
"""

SYNTASK_EXPERIMENTAL_ENABLE_STATES_ON_FLOW_RUN_GRAPH = Setting(bool, default=True)
"""
Whether or not to enable flow run states on the flow run graph.
"""

SYNTASK_EXPERIMENTAL_ENABLE_WORK_POOLS = Setting(bool, default=True)
"""
Whether or not to enable experimental Syntask work pools.
"""

SYNTASK_EXPERIMENTAL_WARN_WORK_POOLS = Setting(bool, default=False)
"""
Whether or not to warn when experimental Syntask work pools are used.
"""

SYNTASK_EXPERIMENTAL_ENABLE_WORKERS = Setting(bool, default=True)
"""
Whether or not to enable experimental Syntask workers.
"""

SYNTASK_EXPERIMENTAL_WARN_WORKERS = Setting(bool, default=False)
"""
Whether or not to warn when experimental Syntask workers are used.
"""

SYNTASK_EXPERIMENTAL_WARN_VISUALIZE = Setting(bool, default=False)
"""
Whether or not to warn when experimental Syntask visualize is used.
"""

SYNTASK_EXPERIMENTAL_ENABLE_ENHANCED_CANCELLATION = Setting(bool, default=True)
"""
Whether or not to enable experimental enhanced flow run cancellation.
"""

SYNTASK_EXPERIMENTAL_WARN_ENHANCED_CANCELLATION = Setting(bool, default=False)
"""
Whether or not to warn when experimental enhanced flow run cancellation is used.
"""

SYNTASK_EXPERIMENTAL_ENABLE_DEPLOYMENT_STATUS = Setting(bool, default=True)
"""
Whether or not to enable deployment status in the UI
"""

SYNTASK_EXPERIMENTAL_WARN_DEPLOYMENT_STATUS = Setting(bool, default=False)
"""
Whether or not to warn when deployment status is used.
"""

SYNTASK_EXPERIMENTAL_FLOW_RUN_INPUT = Setting(bool, default=False)
"""
Whether or not to enable flow run input.
"""

SYNTASK_EXPERIMENTAL_WARN_FLOW_RUN_INPUT = Setting(bool, default=True)
"""
Whether or not to enable flow run input.
"""


# Syntask Events feature flags

SYNTASK_EXPERIMENTAL_EVENTS = Setting(bool, default=False)
"""
Whether to enable Syntask's server-side event features. Note that Syntask Cloud clients
will always emit events during flow and task runs regardless of this setting.
"""


SYNTASK_RUNNER_PROCESS_LIMIT = Setting(int, default=5)
"""
Maximum number of processes a runner will execute in parallel.
"""

SYNTASK_RUNNER_POLL_FREQUENCY = Setting(int, default=10)
"""
Number of seconds a runner should wait between queries for scheduled work.
"""

SYNTASK_RUNNER_SERVER_MISSED_POLLS_TOLERANCE = Setting(int, default=2)
"""
Number of missed polls before a runner is considered unhealthy by its webserver.
"""

SYNTASK_RUNNER_SERVER_HOST = Setting(str, default="localhost")
"""
The host address the runner's webserver should bind to.
"""

SYNTASK_RUNNER_SERVER_PORT = Setting(int, default=8080)
"""
The port the runner's webserver should bind to.
"""

SYNTASK_RUNNER_SERVER_LOG_LEVEL = Setting(str, default="error")
"""
The log level of the runner's webserver.
"""

SYNTASK_RUNNER_SERVER_ENABLE = Setting(bool, default=False)
"""
Whether or not to enable the runner's webserver.
"""

SYNTASK_WORKER_HEARTBEAT_SECONDS = Setting(float, default=30)
"""
Number of seconds a worker should wait between sending a heartbeat.
"""

SYNTASK_WORKER_QUERY_SECONDS = Setting(float, default=10)
"""
Number of seconds a worker should wait between queries for scheduled flow runs.
"""

SYNTASK_WORKER_PREFETCH_SECONDS = Setting(float, default=10)
"""
...
SYNTASK_WORKER_WEBSERVER_HOST = Setting(
    str,
    default="0.0.0.0",
)
"""
...
SYNTASK_WORKER_WEBSERVER_PORT = Setting(
    int,
    default=8080,
)
"""
...
SYNTASK_TASK_SCHEDULING_DEFAULT_STORAGE_BLOCK = Setting(
    str,
    default="local-file-system/syntask-task-scheduling",
)
"""The `block-type/block-document` slug of a block to use as the default storage
for autonomous tasks."""

SYNTASK_TASK_SCHEDULING_DELETE_FAILED_SUBMISSIONS = Setting(
    bool,
    default=True,
)
"""
...
SYNTASK_TASK_SCHEDULING_MAX_SCHEDULED_QUEUE_SIZE = Setting(
    int,
    default=1000,
)
"""
...
SYNTASK_TASK_SCHEDULING_MAX_RETRY_QUEUE_SIZE = Setting(
    int,
    default=100,
)
"""
...
SYNTASK_TASK_SCHEDULING_PENDING_TASK_TIMEOUT = Setting(
    timedelta,
...
SYNTASK_EXPERIMENTAL_ENABLE_FLOW_RUN_INFRA_OVERRIDES = Setting(bool, default=False)
"""
Whether or not to enable infrastructure overrides made on flow runs.
"""

SYNTASK_EXPERIMENTAL_WARN_FLOW_RUN_INFRA_OVERRIDES = Setting(bool, default=True)
"""
Whether or not to warn infrastructure when experimental flow runs overrides are used.
"""

SYNTASK_EXPERIMENTAL_ENABLE_EXTRA_RUNNER_ENDPOINTS = Setting(bool, default=False)
"""
Whether or not to enable experimental worker webserver endpoints.
"""

SYNTASK_EXPERIMENTAL_ENABLE_ARTIFACTS = Setting(bool, default=True)
"""
Whether or not to enable experimental Syntask artifacts.
"""

SYNTASK_EXPERIMENTAL_WARN_ARTIFACTS = Setting(bool, default=False)
"""
Whether or not to warn when experimental Syntask artifacts are used.
"""

SYNTASK_EXPERIMENTAL_ENABLE_WORKSPACE_DASHBOARD = Setting(bool, default=True)
"""
Whether or not to enable the experimental workspace dashboard.
"""

SYNTASK_EXPERIMENTAL_WARN_WORKSPACE_DASHBOARD = Setting(bool, default=False)
"""
Whether or not to warn when the experimental workspace dashboard is enabled.
"""

SYNTASK_EXPERIMENTAL_ENABLE_TASK_SCHEDULING = Setting(bool, default=False)
"""
Whether or not to enable experimental task scheduling.
"""

SYNTASK_EXPERIMENTAL_ENABLE_WORK_QUEUE_STATUS = Setting(bool, default=True)
"""
...
SYNTASK_DEFAULT_RESULT_STORAGE_BLOCK = Setting(
    str,
    default=None,
)
"""The `block-type/block-document` slug of a block to use as the default result storage."""

SYNTASK_DEFAULT_WORK_POOL_NAME = Setting(str, default=None)
"""
The default work pool to deploy to.
"""

SYNTASK_DEFAULT_DOCKER_BUILD_NAMESPACE = Setting(
    str,
    default=None,
)
"""
...
SYNTASK_UI_SERVE_BASE = Setting(
    str,
    default="/",
)
"""
The base URL path to serve the Syntask UI from.

Defaults to the root path.
"""

SYNTASK_UI_STATIC_DIRECTORY = Setting(
    str,
    default=None,
)
"""
...
SYNTASK_MESSAGING_BROKER = Setting(
    str, default="syntask.server.utilities.messaging.memory"
)
"""
...
SYNTASK_MESSAGING_CACHE = Setting(
    str, default="syntask.server.utilities.messaging.memory"
)
"""
...
SYNTASK_EVENTS_MAXIMUM_LABELS_PER_RESOURCE = Setting(int, default=500)
"""
The maximum number of labels a resource may have.
"""

SYNTASK_EVENTS_MAXIMUM_RELATED_RESOURCES = Setting(int, default=500)
"""
The maximum number of related resources an Event may have.
"""

SYNTASK_EVENTS_MAXIMUM_SIZE_BYTES = Setting(int, default=1_500_000)
"""
The maximum size of an Event when serialized to JSON
"""

SYNTASK_API_SERVICES_EVENT_LOGGER_ENABLED = Setting(bool, default=True)
"""
Whether or not to start the event debug logger service in the server application.
"""

SYNTASK_API_SERVICES_TRIGGERS_ENABLED = Setting(bool, default=True)
"""
Whether or not to start the triggers service in the server application.
"""

SYNTASK_EVENTS_EXPIRED_BUCKET_BUFFER = Setting(timedelta, default=timedelta(seconds=60))
"""
The amount of time to retain expired automation buckets
"""

SYNTASK_EVENTS_PROACTIVE_GRANULARITY = Setting(timedelta, default=timedelta(seconds=5))
"""
How frequently proactive automations are evaluated
"""

SYNTASK_API_SERVICES_EVENT_PERSISTER_ENABLED = Setting(bool, default=True)
"""
Whether or not to start the event persister service in the server application.
"""

SYNTASK_API_SERVICES_EVENT_PERSISTER_BATCH_SIZE = Setting(int, default=20, gt=0)
"""
The number of events the event persister will attempt to insert in one batch.
"""

SYNTASK_API_SERVICES_EVENT_PERSISTER_FLUSH_INTERVAL = Setting(float, default=5, gt=0.0)
"""
The maximum number of seconds between flushes of the event persister.
"""

SYNTASK_API_EVENTS_STREAM_OUT_ENABLED = Setting(bool, default=True)
"""
...
class Settings(SettingsFieldsMixin):
    """
    Contains validated Syntask settings.
    ...
    ```python
    from syntask.settings import Settings
    Settings().SYNTASK_PROFILES_PATH  # PosixPath('${SYNTASK_HOME}/profiles.toml')
    ```
    """
    ...
    @validator(SYNTASK_LOGGING_LEVEL.name, SYNTASK_LOGGING_SERVER_LEVEL.name)
    ...
    def post_root_validators(cls, values):
        ...
        if not values["SYNTASK_SILENCE_API_URL_MISCONFIGURATION"]:
...
def load_profiles() -> ProfilesCollection:
    ...
    user_profiles_path = SYNTASK_PROFILES_PATH.value()
...
def save_profiles(profiles: ProfilesCollection) -> None:
    ...
    """
    profiles_path = SYNTASK_PROFILES_PATH.value()
