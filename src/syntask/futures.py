...
from syntask._internal.concurrency.event_loop import run_coroutine_in_loop_from_async
...
from syntask.utilities.asyncutils import A, Async, Sync, sync
...
class SyntaskFuture(Generic[R, A]):
    """
    ...
    Examples:
        ...
        >>> @flow
        >>> def my_flow():
        >>>     future = my_task.submit()  # SyntaskFuture[str, Sync] includes result type
    ...
    @overload
    def wait(
        self: "SyntaskFuture[R, Async]", timeout: None = None
    ) -> Awaitable[State[R]]:
        ...

    @overload
    def wait(self: "SyntaskFuture[R, Sync]", timeout: None = None) -> State[R]:
        ...

    @overload
    def wait(
        self: "SyntaskFuture[R, Async]", timeout: float
    ) -> Awaitable[Optional[State[R]]]:
        ...

    @overload
    def wait(self: "SyntaskFuture[R, Sync]", timeout: float) -> Optional[State[R]]:
        ...
    ...
    @overload
    def get_state(
        self: "SyntaskFuture[R, Async]", client: SyntaskClient = None
    ) -> Awaitable[State[R]]:
        ...

    @overload
    def get_state(
        self: "SyntaskFuture[R, Sync]", client: SyntaskClient = None
    ) -> State[R]:
        ...

    def get_state(self, client: SyntaskClient = None):
        """
    ...
    @inject_client
    async def _get_state(self, client: SyntaskClient = None) -> State[R]:
    ...
    def __repr__(self) -> str:
        return f"SyntaskFuture({self.name!r})"

    def __bool__(self) -> bool:
        warnings.warn(
            (
                "A 'SyntaskFuture' from a task call was cast to a boolean; "
...
async def resolve_futures_to_data(
    expr: Union[SyntaskFuture[R, Any], Any],
    raise_on_failure: bool = True,
) -> Union[R, Any]:
    """
