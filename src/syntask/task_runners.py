...
[Task Runners](/concepts/task-runners/) in Syntask are responsible for managing the execution of Syntask task runs. Generally speaking, users are not expected to interact with task runners outside of configuring and initializing them for a flow.
...
from syntask.states import exception_to_crashed_state
...
class BaseTaskRunner(metaclass=abc.ABCMeta):
    ...
    async def submit(
        ...
        """
        Submit a call for execution and return a `SyntaskFuture` that can be used to
        get the call result.
    ...
    async def wait(self, key: UUID, timeout: float = None) -> Optional[State]:
        """
        Given a `SyntaskFuture`, wait for its return state up to `timeout` seconds.
