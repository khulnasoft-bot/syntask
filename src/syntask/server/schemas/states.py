...
class State(StateBaseModel, Generic[R]):
    ...
    def from_orm_without_result(
        cls,
        orm_state: Union[
            "syntask.server.database.orm_models.ORMFlowRunState",
            "syntask.server.database.orm_models.ORMTaskRunState",
        ],
    ...
    def to_state_create(self):
        ...
        warnings.warn(
            (
                "Use of `syntask.server.schemas.states.State` from the client is"
                " deprecated and support will be removed in a future release. Use"
                " `syntask.states.State` instead."
            ),
            DeprecationWarning,
            stacklevel=2,
        )
