...
from syntask.server.database.orm_models import ORMWorker, ORMWorkPool, ORMWorkQueue
...
async def read_work_pool(
    db: SyntaskDBInterface, session: AsyncSession, work_pool_id: UUID
...
async def read_work_pool_by_name(
    db: SyntaskDBInterface, session: AsyncSession, work_pool_name: str
) -> Optional[ORMWorkPool]:
    """
