...
from syntask._vendor.fastapi import APIRouter, Depends, FastAPI, Request, status
from syntask._vendor.fastapi.encoders import jsonable_encoder
from syntask._vendor.fastapi.exceptions import RequestValidationError
from syntask._vendor.fastapi.middleware.cors import CORSMiddleware
from syntask._vendor.fastapi.middleware.gzip import GZipMiddleware
from syntask._vendor.fastapi.openapi.utils import get_openapi
from syntask._vendor.fastapi.responses import JSONResponse
from syntask._vendor.fastapi.staticfiles import StaticFiles
...
from syntask._internal.compatibility.experimental import enabled_experiments
...
from syntask.server.events.services.event_logger import EventLogger
...
def create_app(
    settings: syntask.settings.Settings = None,
    ...
    async def start_services():
        ...
        if syntask.settings.SYNTASK_EXPERIMENTAL_ENABLE_TASK_SCHEDULING.value():
            service_instances.append(services.task_scheduling.TaskSchedulingTimeouts())

        if (
            syntask.settings.SYNTASK_EXPERIMENTAL_EVENTS.value()
            and syntask.settings.SYNTASK_API_SERVICES_EVENT_LOGGER_ENABLED.value()
        ):
            service_instances.append(EventLogger())

        if (
            syntask.settings.SYNTASK_EXPERIMENTAL_EVENTS.value()
            and syntask.settings.SYNTASK_API_SERVICES_TRIGGERS_ENABLED.value()
        ):
        ...
        if (
            syntask.settings.SYNTASK_EXPERIMENTAL_EVENTS
            and syntask.settings.SYNTASK_API_SERVICES_EVENT_PERSISTER_ENABLED
        ):
            service_instances.append(EventPersister())

        if (
            syntask.settings.SYNTASK_EXPERIMENTAL_EVENTS
            and syntask.settings.SYNTASK_API_EVENTS_STREAM_OUT_ENABLED
        ):
