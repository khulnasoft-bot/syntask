...
from syntask.server.utilities.schemas import DateTimeTZ, SyntaskBaseModel
...
class Resource(Labelled):
    ...
    def enforce_maximum_labels(cls, values: Dict[str, Any]):
        ...
        if len(labels) > SYNTASK_EVENTS_MAXIMUM_LABELS_PER_RESOURCE.value():
            raise ValueError(
    ...
    def requires_resource_id(cls, values: Dict[str, Any]):
        ...
        if "syntask.resource.id" not in labels:
            raise ValueError("Resources must include the syntask.resource.id label")
        if not labels["syntask.resource.id"]:
...
class RelatedResource(Resource):
    ...
    def requires_resource_role(cls, values: Dict[str, Any]):
        ...
        if "syntask.resource.role" not in labels:
            raise ValueError(
                "Related Resources must include the syntask.resource.role label"
            )
        if not labels["syntask.resource.role"]:
...
class ResourceSpecification(SyntaskBaseModel):
    ...
    def matches_every_resource_of_kind(self, prefix: str) -> bool:
        ...
        if len(self.__root__) == 1:
            if resource_id := self.__root__.get("syntask.resource.id"):
