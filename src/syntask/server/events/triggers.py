...
from syntask.server.events.actions import ActionTypes
...
async def with_preceding_event_confirmed(event: ReceivedEvent, depth: int = 0):
    ...
    if event.event == "syntask.log.write":
...
@db_injector
async def record_follower(db: SyntaskDBInterface, event: ReceivedEvent):
...
@db_injector
async def forget_follower(db: SyntaskDBInterface, follower: ReceivedEvent):
...
async def get_followers(
    db: SyntaskDBInterface, leader: ReceivedEvent
) -> List[ReceivedEvent]:
...
@db_injector
async def get_lost_followers(db: SyntaskDBInterface) -> List[ReceivedEvent]:
