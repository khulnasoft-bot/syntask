...
from syntask.server.services.loop_service import LoopService, run_multiple_services
from syntask.settings import (
    SYNTASK_API_SERVICES_SCHEDULER_DEPLOYMENT_BATCH_SIZE,
    SYNTASK_API_SERVICES_SCHEDULER_INSERT_BATCH_SIZE,
    SYNTASK_API_SERVICES_SCHEDULER_LOOP_SECONDS,
...
class Scheduler(LoopService):
    ...
    def __init__(self, loop_seconds: Optional[float] = None, **kwargs):
        super().__init__(
            loop_seconds=(
                ...
                or SYNTASK_API_SERVICES_SCHEDULER_LOOP_SECONDS.value()
            ),
            **kwargs,
        )
        self.deployment_batch_size: int = (
            SYNTASK_API_SERVICES_SCHEDULER_DEPLOYMENT_BATCH_SIZE.value()
        )
        self.max_runs: int = SYNTASK_API_SERVICES_SCHEDULER_MAX_RUNS.value()
        self.min_runs: int = SYNTASK_API_SERVICES_SCHEDULER_MIN_RUNS.value()
        ...
        self.min_scheduled_time: datetime.timedelta = (
            SYNTASK_API_SERVICES_SCHEDULER_MIN_SCHEDULED_TIME.value()
        )
    ...
    @inject_db
    def _get_select_deployments_to_schedule_query(self, db: SyntaskDBInterface):
        """
