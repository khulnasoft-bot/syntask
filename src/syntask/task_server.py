...
from syntask import Task, get_client
...
from syntask.engine import emit_task_run_state_change_event, propose_state
from syntask.exceptions import Abort, SyntaskHTTPStatusError
...
async def serve(*tasks: Task, task_runner: Optional[Type[BaseTaskRunner]] = None):
    ...
    Example:
        ...
        ```
    """
    if not SYNTASK_EXPERIMENTAL_ENABLE_TASK_SCHEDULING.value():
        raise RuntimeError(
            "To enable task scheduling, set SYNTASK_EXPERIMENTAL_ENABLE_TASK_SCHEDULING"
            " to True."
        )
