...
from syntask.settings import SYNTASK_HOME, Profile, Settings
...
class SyntaskObjectRegistry(ContextModel):
    """
    A context that acts as a registry for all Syntask objects that are
    ...
    def register_instances(cls, type_: Type[T]) -> Type[T]:
        """
        Decorator for a class that adds registration to the `SyntaskObjectRegistry`
...
class EngineContext(RunContext):
    ...
    task_run_futures: List[SyntaskFuture] = Field(default_factory=list)
    ...
    # Events worker to emit events to Syntask Cloud
...
class SettingsContext(ContextModel):
    ...
    def __enter__(self):
        ...
        try:
            syntask_home = Path(self.settings.value_of(SYNTASK_HOME))
            syntask_home.mkdir(mode=0o0700, exist_ok=True)
        except OSError:
            warnings.warn(
                (
                    "Failed to create the Syntask home directory at "
                    f"{self.settings.value_of(SYNTASK_HOME)}"
                ),
                stacklevel=2,
            )
...
def use_profile(
    ...
    if existing_context and include_current_context:
        ...
        settings = syntask.settings.get_settings_from_env()
...
GLOBAL_OBJECT_REGISTRY: ContextManager[SyntaskObjectRegistry] = None
...
def initialize_object_registry():
    ...
    GLOBAL_OBJECT_REGISTRY = SyntaskObjectRegistry()
