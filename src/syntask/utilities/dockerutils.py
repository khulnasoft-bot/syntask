...
def get_syntask_image_name(
    syntask_version: str = None, python_version: str = None, flavor: str = None
) -> str:
    """
    ...
    """
    parsed_version = (syntask_version or syntask.__version__).split("+")
    is_prod_build = len(parsed_version) == 1
    syntask_version = (
        parsed_version[0]
        if is_prod_build
        else "sha-" + syntask.__version_tuple__[-1][:7]
    )
...
def parse_image_tag(name: str) -> Tuple[str, Optional[str]]:
    ...
    - If a tag exists, this function parses and returns the image registry and tag,
      ...
      - Example 1: 'syntaskhq/syntask:latest' -> ('syntaskhq/syntask', 'latest')
    ...
    if len(name_parts) == 1:
        ...
        # 1. Separates index (hostname.io or syntaskhq) from path:tag (folder/subfolder:latest or syntask:latest)
...
def generate_default_dockerfile(context: Optional[Path] = None):
    ...
    if (context / "requirements.txt").exists():
        lines.append(f"COPY requirements.txt /opt/syntask/{dir_name}/requirements.txt")
        lines.append(
