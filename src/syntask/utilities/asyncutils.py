...
def sync_compatible(async_fn: T) -> T:
    ...
    def coroutine_wrapper(*args, **kwargs):
        ...
        from syntask._internal.concurrency.calls import get_current_call, logger
        ...
        if current_thread.ident == global_thread_portal.thread.ident:
            ...
            # In the syntask async context; return the coro for us to await
            ...
            # call in Syntask's event loop thread
