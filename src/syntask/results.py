...
async def get_default_result_storage() -> ResultStorage:
    ...
    """
    return (
        await Block.load(SYNTASK_DEFAULT_RESULT_STORAGE_BLOCK.value())
        if SYNTASK_DEFAULT_RESULT_STORAGE_BLOCK.value() is not None
        else LocalFileSystem(basepath=SYNTASK_LOCAL_STORAGE_PATH.value())
    )
...
async def get_or_create_default_task_scheduling_storage() -> ResultStorage:
    ...
    default_storage_name, storage_path = cache_key = (
        SYNTASK_TASK_SCHEDULING_DEFAULT_STORAGE_BLOCK.value(),
        SYNTASK_LOCAL_STORAGE_PATH.value(),
    )
...
def get_default_result_serializer() -> ResultSerializer:
    ...
    Generate a default file system for result storage.
    """
    return SYNTASK_RESULTS_DEFAULT_SERIALIZER.value()
...
class ResultFactory(pydantic.BaseModel):
    ...
    @classmethod
    @inject_client
    async def default_factory(cls, client: "SyntaskClient" = None, **kwargs):
        """
    ...
    @inject_client
    async def from_flow(
        cls: Type[Self], flow: "Flow", client: "SyntaskClient" = None
    ) -> Self:
        """
    ...
    @inject_client
    async def from_task(
        cls: Type[Self], task: "Task", client: "SyntaskClient" = None
    ) -> Self:
        """
...
class LiteralResult(BaseResult):
    ...
    These values are stored inline and JSON serialized when sent to the Syntask API.
    ...
    async def create(
        ...
        description = f"Result with value `{obj}` persisted to Syntask."
...
class PersistedResult(BaseResult):
    ...
    @sync_compatible
    @inject_client
    async def get(self, client: "SyntaskClient") -> R:
        """
    ...
    @inject_client
    async def _read_blob(self, client: "SyntaskClient") -> "PersistedResultBlob":
        assert (
            self.storage_block_id is not None
        ), "Unexpected storage block ID. Was it persisted?"
...
class PersistedResultBlob(pydantic.BaseModel):
    ...
    serializer: Serializer
    data: bytes
    syntask_version: str = pydantic.Field(default=syntask.__version__)
...
class UnknownResult(BaseResult):
    ...
    async def create(
        ...
        description = "Unknown result persisted to Syntask."
        return cls(value=obj, artifact_type="result", artifact_description=description)
