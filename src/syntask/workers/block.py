...
from syntask._internal.schemas.validators import validate_block_is_infrastructure
from syntask.blocks.core import Block
from syntask.client.schemas.objects import BlockDocument
from syntask.utilities.collections import get_from_dict
from syntask.workers.base import BaseWorker, BaseWorkerResult
...
class BlockWorkerJobConfiguration(BaseModel):
    ...
    def is_using_a_runner(self):
        return (
            ...
            and "syntask flow-run execute" in shlex.join(self.block.command)
        )
    ...
    async def from_template_and_values(
        cls, base_job_template: dict, values: dict, client: "SyntaskClient" = None
    ):
...
class BlockWorker(BaseWorker):
    ...
    async def run(
        ...
        # logic for applying infra overrides taken from src/syntask/agent.py
