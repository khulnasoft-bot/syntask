...
from syntask.client.schemas.objects import DeploymentSchedule
...
from syntask.context import SyntaskObjectRegistry, registry_from_script
from syntask.deployments import Deployment, load_deployments_from_yaml
...
from syntask.utilities.filesystem import create_default_ignore_file
...
async def _print_deployment_work_pool_instructions(
    work_pool_name: str, client: SyntaskClient = None
):
    ...
    command = f"$ syntask agent start -p {work_pool_name!r}"
    if work_pool.type != "syntask-agent":
        if experiment_enabled("workers"):
            ...
            command = f"$ syntask worker start -p {work_pool_name!r}"
        else:
            blurb = (
            ...
            command = (
                "$ syntask config set"
                " SYNTASK_EXPERIMENTAL_ENABLE_WORKERS=True\n$ syntask"
...
@deployment_app.command(
    ...
    deprecated_help="Use 'syntask deployment schedule create' instead.",
...
async def _set_schedule(
    ...
    This command is deprecated. Use 'syntask deployment schedule create' instead.
    """
    assert_deployment_name_format(name)
        ...
        async with get_client() as client:
            ...
            if len(deployment.schedules) > 1:
                return exit_with_error(
                    ...
                    "Use `syntask deployment schedules create` instead."
                )
...
@deployment_app.command(
    ...
    deprecated=True,
    deprecated_start_date="Mar 2024",
    deprecated_help="Use 'syntask deployment schedule pause' instead.",
...
async def _pause_schedule(
    ...
    This command is deprecated. Use `syntask deployment schedule pause` instead.
    """
    assert_deployment_name_format(name)
    ...
    async with get_client() as client:
        ...
        if len(deployment.schedules) > 1:
            return exit_with_error(
                f"Deployment {name!r} has multiple schedules. Use `syntask deployment"
...
@deployment_app.command(
    ...
    deprecated=True,
    deprecated_start_date="Mar 2024",
    deprecated_help="Use 'syntask deployment schedule resume' instead.",
...
async def _resume_schedule(
    ...
    This command is deprecated. Use `syntask deployment schedule resume` instead.
    """
    ...
    async with get_client() as client:
        ...
        if len(deployment.schedules) > 1:
            return exit_with_error(
                f"Deployment {name!r} has multiple schedules. "
                "Use `syntask deployment schedule pause <deployment_name> <schedule_id>"
            )
...
async def run(
    ...
    if SYNTASK_UI_URL:
        run_url = f"{SYNTASK_UI_URL.value()}/flow-runs/flow-run/{flow_run.id}"
    else:
...
def _load_deployments(path: Path, quietly=False) -> SyntaskObjectRegistry:
    """
...
@deployment_app.command(
    ...
    deprecated_help="Use 'syntask deploy' to deploy flows via YAML instead.",
)
async def apply(
    ...
    async with get_client() as client:
        for path in paths:
            ...
            if client.server_type != ServerType.CLOUD and deployment.triggers:
                app.console.print(
                    (
                        ...
                        f"Syntask Cloud. Triggers defined in {path!r} will be "
                        "ignored."
                    ),
                    style="red",
                )
            ...
            if deployment.work_pool_name is not None:
                ...
                app.console.print(
                    f"$ syntask agent start -q {deployment.work_queue_name!r}",
                    style="blue",
                )
            else:
                app.console.print(
...
async def build(
    ...
    if create_default_ignore_file(path="."):
        app.console.print(
            (
                "Default '.syntaskignore' file written to"
                f" {(Path('.') / '.syntaskignore').absolute()}"
            ),
            style="green",
        )
