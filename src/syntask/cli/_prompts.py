...
from syntask.client.schemas.actions import BlockDocumentCreate, WorkPoolCreate
...
async def prompt_push_custom_docker_image(
    ...
    if confirm("Is this a private registry?", console=console):
        ...
        if confirm(
            "Would you like use syntask-docker to manage Docker registry credentials?",
            console=console,
            default=False,
        ):
            try:
                ...
                console.print("Installing syntask-docker...")
                await run_process(
...
async def prompt_select_remote_flow_storage(
    console: Console, client: SyntaskClient = None
) -> Optional[str]:
...
async def prompt_select_blob_storage_credentials(
    console: Console, storage_provider: str, client: SyntaskClient = None
) -> str:
    """
    ...
    if SYNTASK_UI_URL:
        console.print(
            ...
            f"\n[blue]{SYNTASK_UI_URL.value()}/blocks/block/{new_block_document.id}[/]\n"
        )
