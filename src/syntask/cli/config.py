...
help_message = """
    Commands for interacting with Syntask settings.
"""
...
def set_(settings: List[str]):
    ...
    for item in settings:
        ...
        if setting not in syntask.settings.SETTING_VARIABLES:
    ...
    for setting, value in parsed_settings.items():
        ...
        if syntask.settings.SETTING_VARIABLES[setting].deprecated:
            app.console.print(
                f"[yellow]{syntask.settings.SETTING_VARIABLES[setting].deprecated_message}."
            )
...
def unset(settings: List[str]):
    ...
    for setting in settings:
        ...
        parsed.add(syntask.settings.SETTING_VARIABLES[setting])
...
def view(
    ...
    default_settings = syntask.settings.get_default_settings()
    env_settings = syntask.settings.get_settings_from_env()
    ...
    app.console.print(f"SYNTASK_PROFILE={context.profile.name!r}")
