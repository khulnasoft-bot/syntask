...
cloud_app = SyntaskTyper(
    name="cloud", help="Commands for interacting with Syntask Cloud"
)
workspace_app = SyntaskTyper(
    name="workspace", help="Commands for interacting with Syntask Cloud Workspaces"
)
...
async def login(
    ...
    env_var_api_key = SYNTASK_API_KEY.value()
        ...
        try:
            ...
            if key.startswith("pcu"):
                help_message = (
                    ...
                    " (https://cloud.syntask.io). Make sure that you generate API key"
...
async def open():
    ...
    current_workspace = get_current_workspace(
        await syntask.get_cloud_client().read_workspaces()
    )
