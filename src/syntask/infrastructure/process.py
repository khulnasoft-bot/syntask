...
It has been replaced by the process worker from the `syntask.workers` module, which offers enhanced functionality and better performance.
...
def _use_threaded_child_watcher():
    if (
        ...
        from syntask.utilities.compat import ThreadedChildWatcher
...
class Process(Infrastructure):
    ...
    Current environment variables and Syntask settings will be included in the created
    ...
    _documentation_url = "https://docs.syntask.io/concepts/infrastructure/#process"
    ...
    def _get_environment_variables(self, include_os_environ: bool = True):
        ...
        # the Syntask settings context may not be respected
    ...
    def _base_flow_run_command(self):
        return [get_sys_executable(), "-m", "syntask.engine"]
