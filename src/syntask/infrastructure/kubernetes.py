...
It has been replaced by the Kubernetes worker from the syntask-kubernetes package, which offers enhanced functionality and better performance.

For upgrade instructions, see https://docs.syntask.io/latest/guides/upgrade-guide-agents-to-workers/.
"""
import copy
import enum
import math
...
@deprecated_class(
    start_date="Mar 2024",
    help="Use the Kubernetes worker from syntask-kubernetes instead."
    " Refer to the upgrade guide for more information:"
    " https://docs.syntask.io/latest/guides/upgrade-guide-agents-to-workers/.",
)
class KubernetesJob(Infrastructure):
    """
    Runs a command as a Kubernetes Job.

    For a guided tutorial, see [How to use Kubernetes with Syntask](https://medium.com/the-syntask-blog/how-to-use-kubernetes-with-syntask-419b2e8b8cb2/).
    For more information, including examples for customizing the resulting manifest, see [`KubernetesJob` infrastructure concepts](https://docs.syntask.io/concepts/infrastructure/#kubernetesjob).
    ...
    Attributes:
        ...
        image: An optional string specifying the image reference of a container image
            to use for the job, for example, docker.io/syntaskhq/syntask:2-latest. The
            behavior is as described in https://kubernetes.io/docs/concepts/containers/images/#image-names.
            Defaults to the Syntask image.
    ...
    _documentation_url = "https://docs.syntask.io/api-ref/syntask/infrastructure/#syntask.infrastructure.KubernetesJob"
    ...
    image: Optional[str] = Field(
        ...
        description=(
            ...
            " `docker.io/syntaskhq/syntask:2-latest`.The behavior is as described in"
            " the Kubernetes documentation and uses the latest version of Syntask by"
    ...
    def _get_environment_variables(self):
        ...
        if (
            "SYNTASK_API_URL" in env
            and "SYNTASK_API_URL" not in self.env
