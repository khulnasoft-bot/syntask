...
MIN_COMPAT_SYNTASK_VERSION = "2.0b12"


if TYPE_CHECKING:
    from syntask.client.schemas.objects import Deployment, Flow, FlowRun
...
class Infrastructure(Block, abc.ABC):
    ...
    async def publish_as_work_pool(self, work_pool_name: Optional[str] = None):
        ...
        if SYNTASK_UI_URL:
            console.print(
                ...
                f" {SYNTASK_UI_URL.value()}/work-pools/work-pool/{work_pool.name}"
            )
        ...
        console.print(f"\t[blue]syntask worker start --pool {work_pool.name}[/]\n")
    ...
    @property
    def logger(self):
        return get_logger(f"syntask.infrastructure.{self.type}")

    @property
    def is_using_a_runner(self):
        return self.command is not None and "syntask flow-run execute" in shlex.join(
            self.command
        )
    ...
    def _base_flow_run_command() -> List[str]:
        ...
        if experiment_enabled("enhanced_cancellation"):
            if (
                SYNTASK_EXPERIMENTAL_WARN
                and SYNTASK_EXPERIMENTAL_WARN_ENHANCED_CANCELLATION
            ):
                warnings.warn(
    ...
    def _base_flow_run_environment(flow_run: "FlowRun") -> Dict[str, str]:
        ...
        """
        environment = {}
        environment["SYNTASK__FLOW_RUN_ID"] = str(flow_run.id)
        return environment

    @staticmethod
    def _base_deployment_labels(deployment: "Deployment") -> Dict[str, str]:
        labels = {
            "syntask.io/deployment-name": deployment.name,
        }
        if deployment.updated is not None:
            labels["syntask.io/deployment-updated"] = deployment.updated.in_timezone(
                "utc"
            ).to_iso8601_string()
        return labels
