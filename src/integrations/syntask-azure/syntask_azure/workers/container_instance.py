...
```bash
syntask worker start --pool 'my-work-pool' --type azure-container-instance
```
...
from syntask_azure.container_instance import ACRManagedIdentity
from syntask_azure.credentials import AzureContainerInstanceCredentials
...
class AzureContainerJobConfiguration(BaseJobConfiguration):
    ...
    image: str = Field(default_factory=get_syntask_image_name)
    ...
    def prepare_for_flow_run(
        ...
        # command overrides the container's built-in entrypoint. Syntask base images
...
class AzureContainerVariables(BaseVariables):
    ...
    image: str = Field(
        ...
        description=(
            "The image to use for the Syntask container in the task. This value "
    ...
    resource_group_name: str = Field(
        ...
        description=(
            "The name of the Azure Resource Group in which to run Syntask ACI tasks."
        ),
    )
    ...
    entrypoint: Optional[str] = Field(
        ...
        description=(
            ...
            "defaults to the entrypoint used by Syntask images and should only be "
            "changed when using a custom image that is not based on an official "
            "Syntask image. Any commands set on deployments will be passed "
    ...
    stream_output: bool = Field(
        default=False,
        description=(
            "If `True`, logs will be streamed from the Syntask container to the local "
            "console."
        ),
    )
    # Execution settings
...
class AzureContainerWorker(BaseWorker):
    """
    A Syntask worker that runs flows in an Azure Container Instance.
    """
    ...
    _documentation_url = (
        "https://synopkg.github.io/syntask-azure/container_instance_worker/"
    )
    ...
    async def run(
        ...
        # for the max deployment name length (64) including "syntask-"
    ...
    async def _provision_container_group(
        ...
        creation_status_poller = await run_sync_in_worker_thread(
            ...
            resource_group_name=configuration.resource_group_name,
            deployment_name=f"syntask-{container_group_name}",
