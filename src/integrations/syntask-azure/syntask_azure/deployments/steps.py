"""
Syntask deployment steps for code storage and retrieval in Azure Blob Storage.

These steps can be used in a `syntask.yaml` file to define the default
...
!!! example
    Sample `syntask.yaml` file that is configured to push and pull to and
    ...
    push:
        - syntask_azure.deployments.steps.push_to_azure_blob_storage:
            requires: syntask-azure[blob_storage]
            container: my-container
            folder: my-folder
            credentials: "{{ syntask.blocks.azure-blob-storage-credentials.dev-credentials }}"
...
For more information about using deployment steps, check out out the Syntask [docs](https://docs.syntask.khulnasoft.com/latest/deploy/).
"""  # noqa
...
from syntask.utilities.filesystem import filter_files, relative_path_to_current_platform
...
def push_to_azure_blob_storage(
    ...
    ignore_file: Optional[str] = ".syntaskignore",
):
    """
    ...
    Args:
        ...
        ignore_file: The path to a file containing patterns of files to ignore when
            pushing to Azure Blob Storage. If not provided, the default `.syntaskignore`
            file will be used.
