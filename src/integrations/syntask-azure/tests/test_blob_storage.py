...
from syntask_azure.blob_storage import (
...
async def test_blob_storage_download_flow(blob_storage_credentials):
    ...
    assert data.decode() == "syntask_works"
...
@pytest.mark.parametrize("blob_expected", [None, "syntask.txt", "syntask_new.txt"])
...
class TestAzureBlobStorageContainer:
    async def test_download_folder_to_path(
        ...
        assert (tmp_path / "folder" / "syntask.txt").exists()
        with open(tmp_path / "folder" / "syntask.txt", "rb") as f:
            assert f.read() == b"syntask_works"
    ...
    async def test_download_object_to_file_object(
        ...
        with open(file_path, "wb") as f:
            await container.download_object_to_file_object(
                from_path="syntask.txt", to_file_object=f
            )
    ...
    async def test_upload_from_file_object(
        ...
        file_content = b"syntask_works_again"
        ...
        with open(tmp_path / "file.txt", "rb") as f:
            assert f.read() == b"syntask_works_again"
    ...
    async def test_upload_from_path(self, mock_blob_storage_credentials, tmp_path):
        ...
        with open(from_path, "wb") as f:
            f.write(b"syntask_works_yet_again")
        ...
        with open(tmp_path / "file.txt", "rb") as f:
            assert f.read() == b"syntask_works_yet_again"
    ...
    async def test_get_directory(slef, mock_blob_storage_credentials, tmp_path):
        ...
        assert (tmp_path / "local_directory" / "syntask.txt").exists()
        with open(tmp_path / "local_directory" / "syntask.txt", "rb") as f:
            assert f.read() == b"syntask_works"
    ...
    async def test_blob_storage_write_path(self, mock_blob_storage_credentials):
        container = AzureBlobStorageContainer(
            container_name="syntask",
            credentials=mock_blob_storage_credentials,
        )
        await container.write_path("syntask-write-path.txt", b"write_path_works")

        result = await container.read_path("syntask-write-path.txt")
    ...
    async def test_list_blobs(self, mock_blob_storage_credentials):
        ...
        assert sorted(blob_result) == ["folder/syntask.txt"]
