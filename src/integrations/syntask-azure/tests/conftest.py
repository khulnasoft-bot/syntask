...
mock_container = {
    "syntask.txt": b"syntask_works",
    "folder/syntask.txt": b"syntask_works",
}


class BlobStorageClientMethodsMock:
    def __init__(self, blob="syntask.txt"):
        self.blob = blob
...
def mock_blob_storage_credentials():
    ...
    block_mock.name = "folder/syntask.txt"
...
def datastore(monkeypatch):
    ...
    monkeypatch.setattr("syntask_azure.ml_datastore.Datastore", DatastoreMock)
...
def cosmos_connection_string(monkeypatch):
    monkeypatch.setattr("syntask_azure.credentials.CosmosClient", CosmosClientMock)
