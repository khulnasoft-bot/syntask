...
import pytest
import syntask_azure.container_instance
...
from syntask_azure.workers.container_instance import (
...
@pytest.fixture
async def aci_worker(mock_syntask_client, monkeypatch):
...
@pytest.fixture()
def mock_syntask_client(monkeypatch, worker_flow):
    """
    A fixture that provides a mock Syntask client
    """
    mock_client = Mock()
    ...
    monkeypatch.setattr(
        syntask_azure.workers.container_instance,
        "get_client",
...
async def test_worker_credentials_are_used(
    ...
    monkeypatch.setattr(
        syntask_azure.credentials, "ClientSecretCredential", mock_credential
    )
...
async def test_task_status_started_on_provisioning_success(
    ...
    flow = await mock_syntask_client.read_flow(worker_flow_run.flow_id)
...
async def test_task_status_not_started_on_provisioning_failure(
    worker_flow_run, job_configuration, monkeypatch, mock_syntask_client
):
    async with AzureContainerWorker(work_pool_name="test_pool") as aci_worker:
        monkeypatch.setattr(
...
async def test_output_streaming(
    ...
    monkeypatch.setattr(
        syntask_azure.workers.container_instance, "datetime", mock_datetime
    )
...
def test_block_accessible_in_module_toplevel():
    ...
    # is not accessible directly from `syntask_azure`
    from syntask_azure import AzureContainerWorker  # noqa
...
def test_secure_environment_variables(
    ...
    base_env["SYNTASK_API_KEY"] = "my-api-key"
    ...
    api_key_aci_env_variable: List[Dict] = list(
        filter(lambda v: v["name"] == "SYNTASK_API_KEY", container_env)
    )
...
def test_job_configuration_creation():
    config = AzureContainerJobConfiguration(
        ...
        arm_template=(
            syntask_azure.workers.container_instance._get_default_arm_template()
        ),
        image="my-image",
        cpu=1,
        memory=1,
...
async def test_image_populated_in_template_when_not_provided(worker_flow_run):
    ...
    assert config.image == get_syntask_image_name()
    assert (
...
async def test_consistent_container_group_naming(
    ...
    mock_syntask_client.read_flow.return_value = flow
    ...
    assert container_group_name.startswith("syntask-")
