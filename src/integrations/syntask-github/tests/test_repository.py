...
import pytest
import syntask_github
from syntask_github import GitHubCredentials
from syntask_github.repository import GitHubRepository
...
class TestGitHubRepository:
    ...
    async def test_repository_default(self, monkeypatch):
        ...
        monkeypatch.setattr(syntask_github.repository, "run_process", mock)
        g = GitHubRepository(repository_url="syntask")
        await g.get_directory()

        assert mock.await_count == 1
        assert ["git", "clone", "syntask"] == mock.await_args[0][0][:3]
    ...
    async def test_reference_default(self, monkeypatch):
        ...
        mock = AsyncMock(return_value=p())
        monkeypatch.setattr(syntask_github.repository, "run_process", mock)
        g = GitHubRepository(repository_url="syntask", reference="2.0.0")
        await g.get_directory()

        assert mock.await_count == 1
        assert "git clone syntask -b 2.0.0 --depth 1" in " ".join(mock.await_args[0][0])
    ...
    async def test_token_added_correctly_from_credential(self, monkeypatch):
        ...
        g = GitHubRepository(
            repository_url="https://github.com/synopkg/syntask.git",
            credentials=credential,
        )
        await g.get_directory()
        assert mock.await_count == 1
        assert (
            "git clone https://XYZ@github.com/synopkg/syntask.git --depth 1"
