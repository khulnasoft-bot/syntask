...
- cpu: 1024
  image: synopkg/syntask:2.1.0-python3.9
  memory: 2048
  name: syntask
family: syntask
"""
...
async def test_default(aws_credentials: AwsCredentials, flow_run: FlowRun):
    ...
    assert task == {
        ...
        #       reasonable values
        "containers": [{"exitCode": 0, "name": "syntask"}],
        "desiredStatus": "STOPPED",
        "lastStatus": "STOPPED",
        "launchType": "FARGATE",
        "overrides": {
            "containerOverrides": [
                {"name": "syntask", "environment": [], "command": ["echo", "test"]}
            ]
        },
        "startedBy": ANY,
        "tags": [],
...
async def test_image(aws_credentials: AwsCredentials, flow_run: FlowRun):
    configuration = await construct_configuration(
        aws_credentials=aws_credentials, image="synopkg/syntask-dev:main-python3.9"
    )
    ...
    assert task_definition["containerDefinitions"] == [
        {
            "name": ECS_DEFAULT_CONTAINER_NAME,
            "image": "synopkg/syntask-dev:main-python3.9",
            "cpu": 0,
            "memory": 0,
            "portMappings": [],
            "essential": True,
...
async def test_cpu_and_memory(
    ...
    # And as overrides for the Syntask container
...
async def test_container_command(
    ...
    assert container_overrides["command"] == ["syntask", "version"]
...
async def test_environment_variables(
    ...
    task = describe_task(ecs_client, task_arn)
    task_definition = describe_task_definition(ecs_client, task)
    syntask_container_definition = _get_container(
        task_definition["containerDefinitions"], ECS_DEFAULT_CONTAINER_NAME
    )
    assert not syntask_container_definition[
        "environment"
    ], "Variables should not be passed until runtime"

    syntask_container_overrides = _get_container(
        task["overrides"]["containerOverrides"], ECS_DEFAULT_CONTAINER_NAME
    )
    expected = [{"name": "FOO", "value": "BAR"}]
    assert syntask_container_overrides.get("environment") == expected
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            },  # regression test for https://github.com/synopkg/syntask/issues/13174
            {
                "flow-name": "Hello-World",
            },
        ),
...
async def test_stream_output(
    ...
    async def write_fake_log(task_arn):
        ...
        stream_name = f"test-prefix/syntask/{task_arn.rsplit('/')[-1]}"
        logs_client.put_log_events(
            logGroupName="syntask",
    ...
    logs_client = session.client("logs")
    streams = logs_client.describe_log_streams(logGroupName="syntask")["logStreams"]
...
async def test_cloudwatch_log_options(
    ...
    for container in task_definition["containerDefinitions"]:
        prefix = f"syntask-logs_{work_pool_name}_{flow_run.deployment_id}"
...
async def test_user_defined_container_image_in_task_definition_template(
    ...
    syntask_container = _get_container(
        task_definition["containerDefinitions"], ECS_DEFAULT_CONTAINER_NAME
    )
    assert (
        syntask_container["image"] == "use-this-image"
...
async def test_user_defined_environment_variables_in_task_definition_template(
    ...
    assert syntask_container_definition["environment"] == [
        {"name": "BAR", "value": "FOO"},
        {"name": "OVERRIDE", "value": "OLD"},
    ]
    ...
    assert syntask_container_overrides.get("environment") == [
        {"name": "FOO", "value": "BAR"},
...
async def test_user_defined_environment_variables_in_task_run_request_template(
    ...
    assert (
        syntask_container_definition["environment"] == []
...
async def test_mask_sensitive_env_values():
    task_run_request = {
        "overrides": {
            "containerOverrides": [
                {
                    "environment": [
                        {"name": "SYNTASK_API_KEY", "value": "SeNsItiVe VaLuE"},
                        {"name": "SYNTASK_API_URL", "value": "NORMAL_VALUE"},
                    ]
                }
            ]
        }
    ...
    res = mask_sensitive_env_values(task_run_request, ["SYNTASK_API_KEY"], 3, "***")
    assert (
