...
def tmp_files(tmp_path: Path):
    ...
    (tmp_path / ".syntaskignore").write_text(
        """
    testdir1/*
    .syntaskignore
    """
...
def test_custom_credentials_and_client_parameters(s3_setup, tmp_files):
    ...
    for file in tmp_files.iterdir():
        if file.is_file() and file.name != ".syntaskignore":
...
def test_without_syntaskignore_file(s3_setup, tmp_files: Path, mock_aws_credentials):
    s3, bucket_name = s3_setup
    folder = "my-project"

    # Remove the .syntaskignore file
    (tmp_files / ".syntaskignore").unlink()

    os.chdir(tmp_files)

    # Test push_to_s3 without .syntaskignore file
    push_to_s3(bucket_name, folder)

    # Test pull_from_s3 without .syntaskignore file
    tmp_path = tmp_files / "test_pull"
    tmp_path.mkdir(parents=True, exist_ok=True)
    os.chdir(tmp_path)
...
def test_syntaskignore_with_comments_and_empty_lines(
    s3_setup, tmp_files: Path, mock_aws_credentials
):
    s3, bucket_name = s3_setup
    folder = "my-project"

    # Update the .syntaskignore file with comments and empty lines
    (tmp_files / ".syntaskignore").write_text(
        """
        # This is a comment
        testdir1/*
