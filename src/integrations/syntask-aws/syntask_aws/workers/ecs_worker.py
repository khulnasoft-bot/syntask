"""
Syntask worker for executing flow runs as ECS tasks.

Get started by creating a work pool:

```
$ syntask work-pool create --type ecs my-ecs-pool
```

Then, you can start a worker for the pool:

```
$ syntask worker start --pool my-ecs-pool
```
...
ECS_DEFAULT_CONTAINER_NAME = "syntask"
ECS_DEFAULT_CPU = 1024
ECS_DEFAULT_COMMAND = "python -m syntask.engine"
ECS_DEFAULT_MEMORY = 2048
ECS_DEFAULT_LAUNCH_TYPE = "FARGATE"
ECS_DEFAULT_FAMILY = "syntask"
...
def mask_api_key(task_run_request):
    return mask_sensitive_env_values(
        deepcopy(task_run_request), ["SYNTASK_API_KEY"], keep_length=6
    )
...
class ECSVariables(BaseVariables):
    ...
    env: Dict[str, Optional[str]] = Field(
        ...
        description=(
            ...
            "on the Syntask container at task runtime. These will not be set on the "
            "task definition."
        ),
    )
    ...
    image: Optional[str] = Field(
        default=None,
        description=(
            "The image to use for the Syntask container in the task. If this value is "
            "not null, it will override the value in the task definition. This value "
            "defaults to a Syntask base image matching your local versions."
        ),
    )
    ...
    configure_cloudwatch_logs: Optional[bool] = Field(
        default=None,
        description=(
            "If enabled, the Syntask container will be configured to send its output "
    ...
    cloudwatch_logs_prefix: Optional[str] = Field(
        ...
        description=(
            ...
            " be `syntask-logs_<work_pool_name>_<deployment_id>`. If"
    ...
    stream_output: Optional[bool] = Field(
        default=None,
        description=(
            "If enabled, logs will be streamed from the Syntask container to the local "
...
class ECSWorker(BaseWorker):
    """
    A Syntask worker to run flow runs as ECS tasks.
    """
    ...
    _documentation_url = "https://synopkg.github.io/prefect-aws/ecs_worker/"
    ...
    async def run(
        ...
        # Note "::" is used despite the Syntask standard being ":" because ARNs contain
        # single colons.
        identifier = (
    ...
    def _watch_task_and_get_exit_code(
        ...
        """
        Wait for the task run to complete and retrieve the exit code of the Syntask
        container.
        """
        ...
        # Check the status code of the Syntask container
    ...
    def _wait_for_task_finish(
        ...
        If configured, logs from the Syntask container are streamed to stderr.
        ...
        if configuration.stream_output:
            ...
            if not container_def:
                logger.warning(
                    "Syntask container definition not found in "
    ...
    def _stream_available_logs(
        ...
        while last_log_stream_token != next_log_stream_token:
            ...
            for log_event in log_events:
                ...
                #       when the content is output from a Syntask logger on the
    ...
    def _prepare_task_definition(
        ...
        # Configure the Syntask runtime container
        ...
        container.setdefault("image", get_syntask_image_name())
        ...
        if configuration.configure_cloudwatch_logs:
            prefix = f"syntask-logs_{self._work_pool_name}_{flow_run.deployment_id}"
            container["logConfiguration"] = {
                "logDriver": "awslogs",
                "options": {
                    "awslogs-create-group": "true",
                    "awslogs-group": "syntask",
