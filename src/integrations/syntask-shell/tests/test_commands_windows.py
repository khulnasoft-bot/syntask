...
import pytest
from syntask_shell.commands import ShellOperation, shell_run_command
...
async def test_shell_run_command_error_windows(syntask_task_runs_caplog):
    @flow
    async def test_flow():
    ...
    assert len(syntask_task_runs_caplog.records) == 7


async def test_shell_run_command_windows(syntask_task_runs_caplog):
    syntask_task_runs_caplog.set_level(logging.INFO)
    echo_msg = "WORKING"
    ...
    for record in syntask_task_runs_caplog.records:
...
async def test_shell_run_command_stream_level_windows(syntask_task_runs_caplog):
    syntask_task_runs_caplog.set_level(logging.WARNING)
    echo_msg = "WORKING"
    ...
    print(syntask_task_runs_caplog.text)
...
async def test_shell_run_command_ensure_tmp_file_removed():
    ...
    assert len(glob.glob(f"{temp_dir}\\syntask-*.ps1")) == 0
...
async def test_shell_run_command_override_shell(shell, monkeypatch):
    ...
    monkeypatch.setattr("syntask_shell.commands.TextReceiveStream", AsyncIter)
...
class TestShellOperation:
    ...
    @pytest.mark.parametrize("method", ["run", "trigger"])
    async def test_output(self, syntask_task_runs_caplog, method):
        op = ShellOperation(commands=["echo 'testing'"])
        assert await self.execute(op, method) == ["testing"]
        records = syntask_task_runs_caplog.records
