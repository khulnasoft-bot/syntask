...
async def test_shell_run_command_error(syntask_task_runs_caplog):
    @flow
    async def test_flow():
...
async def test_shell_run_command(syntask_task_runs_caplog):
    ...
    assert echo_msg in syntask_task_runs_caplog.text


async def test_shell_run_command_stream_level(syntask_task_runs_caplog):
...
class TestShellOperation:
    ...
    @pytest.mark.parametrize("method", ["run", "trigger"])
    async def test_stream_output(self, syntask_task_runs_caplog, method):
