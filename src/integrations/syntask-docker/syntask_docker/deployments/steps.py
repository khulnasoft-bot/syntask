"""
Syntask deployment steps for building and pushing Docker images.
...
logger = get_logger("syntask_docker.deployments.steps")
...
def build_docker_image(
    ...
    **build_kwargs,
    ...
    """
    Builds a Docker image for a Syntask deployment.

    Can be used within a `syntask.yaml` file to build a Docker
    ...
    Examples:
        ...
        push:
            - syntask_docker.deployments.steps.push_docker_image:
                ...
                image_name: "{{ build-image.image_name }}"
                tag: "{{ build-image.tag }}"
                credentials: "{{ syntask.blocks.docker-registry-credentials.dev-registry }}"
        ```
