...
```bash
syntask worker start --pool 'my-work-pool' --type docker
```
...
checkout out the [Syntask docs](https://docs.syntask.khulnasoft.com/latest/deploy/infrastructure-concepts).
"""
...
from syntask_docker.credentials import DockerRegistryCredentials
...
class DockerWorkerJobConfiguration(BaseJobConfiguration):
    ...
    image: str = Field(
        default_factory=get_syntask_image_name,
    ...
    def _base_environment(self):
        ...
        if (
            "SYNTASK_API_URL" in base_env
            and base_env["SYNTASK_API_URL"] is not None
            and network_mode != "host"
        ):
            base_env["SYNTASK_API_URL"] = (
                base_env["SYNTASK_API_URL"]
    ...
    def prepare_for_flow_run(
        ...
        self.image = self.image or get_syntask_image_name()
    ...
    def get_network_mode(self) -> Optional[str]:
        ...
        api_url = self.env.get("SYNTASK_API_URL", SYNTASK_API_URL.value())
    ...
    def get_extra_hosts(self, docker_client) -> Optional[Dict[str, str]]:
        ...
        if sys.platform == "linux" and (
            ...
            # a local address
            "SYNTASK_API_URL" not in self.env
            or re.search(
...
class DockerWorker(BaseWorker):
    """Syntask worker that executes flow runs within Docker containers."""
    ...
    _documentation_url = "https://synopkg.github.io/prefect-docker/worker/"
    ...
    def __init__(
        ...
        if test_mode is None:
            self.test_mode = bool(os.getenv("SYNTASK_DOCKER_TEST_MODE", False))
        else:
        ...
        if not self.test_mode:
            ...
            if self._client.server_type == ServerType.EPHEMERAL:
                raise RuntimeError(
                    ...
                    " SYNTASK_API_URL to the URL for your Syntask API instance. You"
                    " can use a local Syntask API instance by running `syntask server"
                    " start`."
                )
    ...
    def _container_as_resource(self, container: "Container") -> Dict[str, str]:
        ...
        return {
            "syntask.resource.id": f"syntask.docker.container.{container.id}",
            "syntask.resource.name": container.name,
        }

    def _emit_container_creation_failed_event(
        self, configuration: DockerWorkerJobConfiguration
    ) -> Event:
        """Emit a Syntask event when a docker container fails to be created."""
        return emit_event(
            event="syntask.docker.container.creation-failed",
    ...
    def _emit_container_status_change_event(
        ...
        """Emit a Syntask event for a Docker container event."""
        related = self._event_related_resources(configuration=configuration)
        ...
        return emit_event(
            event=f"syntask.docker.container.{container.status.lower()}",
