...
import pendulum
import pytest
from syntask_docker.deployments.steps import (
    build_docker_image,
    push_docker_image,
)

import syntask
import syntask.utilities.dockerutils
...
def reset_cachable_steps(monkeypatch):
    monkeypatch.setattr("syntask_docker.deployments.steps.STEP_OUTPUT_CACHE", {})
...
def mock_docker_client(monkeypatch):
    ...
    mock_docker_client_func = MagicMock(
        name="docker_client", spec=syntask.utilities.dockerutils.docker_client
    )
    mock_docker_client_func.return_value.__enter__.return_value = mock_client
    monkeypatch.setattr(
        "syntask_docker.deployments.steps.docker_client", mock_docker_client_func
    )
    return mock_client
...
def mock_pendulum(monkeypatch):
    ...
    monkeypatch.setattr("syntask_docker.deployments.steps.pendulum", mock_pendulum)
...
def test_build_docker_image(
    ...
    mock_docker_client.api.build.assert_called_once_with(
        ...
        decode=True,
        pull=True,
        labels=syntask.utilities.dockerutils.IMAGE_LABELS,
    )
...
def test_real_auto_dockerfile_build(docker_client_with_cleanup):
    ...
    try:
        ...
        expected_syntask_version = syntask.__version__
        expected_syntask_version = expected_syntask_version.replace(".dirty", "")
        expected_syntask_version = expected_syntask_version.split("+")[0]

        cases = [
            {"command": "syntask version", "expected": expected_syntask_version},
...
class TestCachedSteps:
    ...
    def test_avoids_aggressive_caching(self, mock_docker_client):
        """this is a regression test for https://github.com/synopkg/syntask/issues/15258
