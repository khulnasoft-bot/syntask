...
from syntask_docker.worker import (
    CONTAINER_LABELS,
    DockerWorker,
...
def bypass_api_check(monkeypatch):
    monkeypatch.setenv("SYNTASK_DOCKER_TEST_MODE", True)
...
async def test_uses_label_setting(
    ...
    assert call_labels == {
        **CONTAINER_LABELS,
        "io.syntask.flow-run-id": str(flow_run.id),
        "io.syntask.flow-run-name": flow_run.name,
        "foo": "FOO",
        "bar": "BAR",
    }
...
async def test_allows_unsetting_environment_variables(
    ...
    default_docker_worker_job_configuration.env = {"SYNTASK_TEST_MODE": None}
    default_docker_worker_job_configuration.prepare_for_flow_run(flow_run=flow_run)
    async with DockerWorker(work_pool_name="test") as worker:
        await worker.run(
            flow_run=flow_run, configuration=default_docker_worker_job_configuration
...
async def test_network_mode_defaults_to_host_if_using_localhost_api_on_linux(
    ...
    default_docker_worker_job_configuration.env = dict(
        SYNTASK_API_URL=f"http://{localhost}/test"
    )
    default_docker_worker_job_configuration.prepare_for_flow_run(flow_run=flow_run)
    async with DockerWorker(work_pool_name="test") as worker:
        await worker.run(
...
async def test_network_mode_defaults_to_none_if_using_networks(
    ...
    default_docker_worker_job_configuration.env = dict(
        SYNTASK_API_URL="http://localhost/test"
    )
...
async def test_does_not_override_user_provided_api_host(
    ...
    default_docker_worker_job_configuration.env = dict(
        SYNTASK_API_URL="http://localhost/api"
    )
    default_docker_worker_job_configuration.prepare_for_flow_run(flow_run=flow_run)
    async with DockerWorker(work_pool_name="test") as worker:
        await worker.run(
...
async def test_default_image_pull_policy_pulls_image_with_latest_tag(
    ...
    default_docker_worker_job_configuration.image = "syntask:latest"
    default_docker_worker_job_configuration.prepare_for_flow_run(flow_run=flow_run)
    async with DockerWorker(work_pool_name="test") as worker:
        await worker.run(
            flow_run=flow_run, configuration=default_docker_worker_job_configuration
...
async def test_default_image_pull_policy_pulls_image_with_no_tag(
    ...
    default_docker_worker_job_configuration.image = "syntask"
    default_docker_worker_job_configuration.prepare_for_flow_run(flow_run=flow_run)
    async with DockerWorker(work_pool_name="test") as worker:
        await worker.run(
            flow_run=flow_run, configuration=default_docker_worker_job_configuration
...
async def test_default_image_pull_policy_pulls_image_with_tag_other_than_latest_if_not_present(  # noqa
    ...
    default_docker_worker_job_configuration.image = "syntask:omega"
    default_docker_worker_job_configuration.prepare_for_flow_run(flow_run=flow_run)
    async with DockerWorker(work_pool_name="test") as worker:
        await worker.run(
            flow_run=flow_run, configuration=default_docker_worker_job_configuration
...
async def test_warns_if_docker_version_does_not_support_host_gateway_on_linux(
    ...
    default_docker_worker_job_configuration.env = (
        {"SYNTASK_API_URL": explicit_api_url} if explicit_api_url else {}
    )
    default_docker_worker_job_configuration.prepare_for_flow_run(flow_run=flow_run)
    with pytest.warns(
        UserWarning,
...
async def test_does_not_warn_about_gateway_if_user_has_provided_nonlocal_api_url(
    ...
    default_docker_worker_job_configuration.env = {
        "SYNTASK_API_URL": "http://my-domain.test/api"
    }
    default_docker_worker_job_configuration.prepare_for_flow_run(flow_run=flow_run)
...
async def test_worker_errors_out_on_ephemeral_apis():
    with temporary_settings(
        {SYNTASK_API_URL: None, SYNTASK_SERVER_ALLOW_EPHEMERAL_MODE: True}
    ):
...
async def test_emits_events(
    ...
    with patch("syntask_docker.worker.emit_event", side_effect=event) as mock_emit:
        async with DockerWorker(work_pool_name="test") as worker:
            await worker.run(
                flow_run=flow_run, configuration=default_docker_worker_job_configuration
            )
    ...
    mock_emit.assert_has_calls(
        [
            call(
                event="syntask.docker.container.created",
                resource={
                    "syntask.resource.id": "syntask.docker.container.fake-id",
                    "syntask.resource.name": "fake-name",
                },
                related=[worker_related_resource],
                follows=None,
            ),
            call(
                event="syntask.docker.container.running",
                resource={
                    "syntask.resource.id": "syntask.docker.container.fake-id",
                    "syntask.resource.name": "fake-name",
                },
            ...
            call(
                event="syntask.docker.container.exited",
                resource={
                    "syntask.resource.id": "syntask.docker.container.fake-id",
                    "syntask.resource.name": "fake-name",
                },
...
async def test_emits_event_container_creation_failure(
    ...
    with patch("syntask_docker.worker.emit_event") as mock_emit:
        with pytest.raises(ExceptionGroup) as exc:
            async with DockerWorker(work_pool_name="test") as worker:
                worker_resource = worker._event_resource()
                await worker.run(
