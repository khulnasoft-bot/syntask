...
import pytest
from syntask_docker.images import pull_docker_image
...
class TestPullDockerImage:
    async def test_tag_and_all_tags(self, mock_docker_client_from_env: MagicMock):
        pull_kwargs = dict(repository="synopkg/syntask", tag="latest", all_tags=True)
        with pytest.raises(
    ...
    async def test_defaults(self, mock_docker_client_from_env: MagicMock):
        with disable_run_logger():
            image = await pull_docker_image.fn(repository="synopkg/syntask")
        assert image.id == "id_1"

    async def test_host(self, mock_docker_host: MagicMock):
        pull_kwargs = dict(
            repository="synopkg/syntask",
        )
        with disable_run_logger():
    ...
    async def test_all_tags(self, mock_docker_host: MagicMock):
        pull_kwargs = dict(repository="synopkg/syntask", all_tags=True)
        with disable_run_logger():
