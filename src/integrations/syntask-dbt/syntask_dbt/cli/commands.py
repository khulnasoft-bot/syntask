...
from syntask_dbt.cli.credentials import DbtCliProfile
...
async def trigger_dbt_cli_command(
    ...
    Args:
        ...
        create_summary_artifact: If True, creates a Syntask artifact on the task run
            with the dbt results using the specified artifact key.
            Defaults to False.
        summary_artifact_key: The key under which to store the dbt results artifact in Syntask.
        ...
        stream_output: If True, the output from the dbt command will be logged in Syntask
            as it happens.
            Defaults to True.
    ...
    Examples:
        ...
        from syntask_dbt.cli.commands import trigger_dbt_cli_command
        ...
        from syntask_dbt.cli.credentials import DbtCliProfile
        from syntask_dbt.cli.commands import trigger_dbt_cli_command
        from syntask_dbt.cli.configs import SnowflakeTargetConfigs
        from syntask_snowflake.credentials import SnowflakeCredentials
...
class DbtCoreOperation(ShellOperation):
    ...
    Examples:
        ...
        ```python
        from syntask_dbt import DbtCoreOperation
        ...
        ```python
        from syntask_dbt import DbtCoreOperation, DbtCliProfile
        from syntask_dbt.cli.configs import SnowflakeTargetConfigs
        from syntask_snowflake import SnowflakeConnector
    ...
    _documentation_url = "https://synopkg.github.io/prefect-dbt/cli/commands/#syntask_dbt.cli.commands.DbtCoreOperation"  # noqa
...
async def run_dbt_build(
    ...
    """
    Executes the 'dbt build' command within a Syntask task,
    and optionally creates a Syntask artifact summarizing the dbt build results.
    ...
    Args:
        ...
        summary_artifact_key: The key under which to store
            the dbt build results artifact in Syntask.
    ...
    ```python
        from syntask import flow
        from syntask_dbt.cli.tasks import dbt_build_task
...
async def run_dbt_model(
    ...
    """
    Executes the 'dbt run' command within a Syntask task,
    and optionally creates a Syntask artifact summarizing the dbt model results.
    ...
    Args:
        ...
        create_summary_artifact: If True, creates a Syntask artifact on the task run
            ...
            Defaults to False.
        summary_artifact_key: The key under which to store
            the dbt model run results artifact in Syntask.
    ...
    ```python
        from syntask import flow
        from syntask_dbt.cli.tasks import dbt_run_task
...
async def run_dbt_test(
    ...
    """
    Executes the 'dbt test' command within a Syntask task,
    and optionally creates a Syntask artifact summarizing the dbt test results.
    ...
    Args:
        ...
        create_summary_artifact: If True, creates a Syntask artifact on the task run
            ...
            Defaults to False.
        summary_artifact_key: The key under which to store
            the dbt test results artifact in Syntask.
    ...
    ```python
        from syntask import flow
        from syntask_dbt.cli.tasks import dbt_test_task
...
async def run_dbt_snapshot(
    ...
    """
    Executes the 'dbt snapshot' command within a Syntask task,
    and optionally creates a Syntask artifact summarizing the dbt snapshot results.
    ...
    Args:
        ...
        create_summary_artifact: If True, creates a Syntask artifact on the task run
            ...
            Defaults to False.
        summary_artifact_key: The key under which to store
            the dbt snapshot results artifact in Syntask.
    ...
    ```python
        from syntask import flow
        from syntask_dbt.cli.tasks import dbt_snapshot_task
...
async def run_dbt_seed(
    ...
    """
    Executes the 'dbt seed' command within a Syntask task,
    and optionally creates a Syntask artifact summarizing the dbt seed results.
    ...
    Args:
        ...
        create_summary_artifact: If True, creates a Syntask artifact on the task run
            ...
            Defaults to False.
        summary_artifact_key: The key under which to store
            the dbt seed results artifact in Syntask.
    ...
    ```python
        from syntask import flow
        from syntask_dbt.cli.tasks import dbt_seed_task
...
async def run_dbt_source_freshness(
    ...
    """
    Executes the 'dbt source freshness' command within a Syntask task,
    and optionally creates a Syntask artifact summarizing the dbt source freshness results.
    ...
    Args:
        ...
        create_summary_artifact: If True, creates a Syntask artifact on the task run
            ...
            Defaults to False.
        summary_artifact_key: The key under which to store
            the dbt source freshness results artifact in Syntask.
    ...
    ```python
        from syntask import flow
        from syntask_dbt.cli.commands import run_dbt_source_freshness
...
def create_summary_markdown(run_results: dict, command: str) -> str:
    """
    Creates a Syntask task artifact summarizing the results
