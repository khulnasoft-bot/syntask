...
from syntask_dbt.cloud.clients import DbtCloudMetadataClient
...
def test_metadata_client_query(monkeypatch):
    ...
    assert mock_headers["X-dbt-partner-source"] == "syntask"
    assert mock_headers["Authorization"] == "Bearer my_api_key"
    assert mock_headers["User-agent"] == f"syntask-{syntask.__version__}"
