...
from syntask_dask.task_runners import SyntaskDaskFuture

from syntask import flow, task
from syntask.futures import as_completed
from syntask.server.schemas.states import StateType
from syntask.states import State
from syntask.testing.fixtures import (  # noqa: F401
    hosted_api_server,
...
class TestDaskTaskRunner:
    ...
    def test_dask_task_key_has_syntask_task_name(self):
    ...
    class TestInputArguments:
        async def test_dataclasses_can_be_passed_to_task_runners(self, task_runner):
            """
            this is a regression test for https://github.com/synopkg/syntask/issues/6905
            """
    ...
    def test_nested_task_submission(self, task_runner):
        ...
        def parent_task():
            ...
            assert isinstance(nested_task_future, SyntaskDaskFuture)
            return nested_task_future.result()

        @flow(task_runner=task_runner)
        def umbrella_flow():
            future = parent_task.submit()
            assert isinstance(future, SyntaskDaskFuture)
