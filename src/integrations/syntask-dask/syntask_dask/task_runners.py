...
[Task Runners](https://docs.syntask.khulnasoft.com/api-ref/syntask/task-runners/)
...
Example:
    ...
    from syntask import flow, task
    from syntask_dask import DaskTaskRunner
...
from syntask_dask.client import SyntaskDaskClient
...
class SyntaskDaskFuture(SyntaskWrappedFuture[R, distributed.Future]):
    """
    A Syntask future that wraps a distributed.Future. This future is used
...
class DaskTaskRunner(TaskRunner):
    ...
    def __init__(
        ...
        if "asynchronous" in client_kwargs:
            raise ValueError(
                ...
                "This option is managed by Syntask."
            )
        ...
        # Runtime attributes
        self._client: SyntaskDaskClient = None
    ...
    def submit(
        ...
        # unpack the upstream call in order to cast Syntask futures to Dask futures
        ...
        return SyntaskDaskFuture(wrapped_future=future, task_run_id=future.task_run_id)
    ...
    def map(
        ...
        parameters: Dict[str, Any],
        wait_for: Optional[Iterable[SyntaskFuture]] = None,
    ) -> SyntaskFutureList[SyntaskDaskFuture[R]]: ...
    ...
    def _optimize_futures(self, expr):
        def visit_fn(expr):
            if isinstance(expr, SyntaskDaskFuture):
    ...
    def __enter__(self):
        ...
        self._client = exit_stack.enter_context(
            SyntaskDaskClient(self._connect_to, **self.client_kwargs)
        )
