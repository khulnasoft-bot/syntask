...
[Task Runners](https://docs.syntask.khulnasoft.com/latest/develop/task-runners/)
in Syntask are responsible for managing the execution of Syntask task runs.
...
from syntask.context import serialize_context
from syntask.futures import SyntaskFuture, SyntaskFutureList, SyntaskWrappedFuture
from syntask.logging.loggers import get_logger, get_run_logger
from syntask.states import State, exception_to_crashed_state
...
from syntask_ray.context import RemoteOptionsContext
...
P = ParamSpec("P")
T = TypeVar("T")
F = TypeVar("F", bound=SyntaskFuture)
R = TypeVar("R")


class SyntaskRayFuture(SyntaskWrappedFuture[R, "ray.ObjectRef"]):
...
class RayTaskRunner(TaskRunner[SyntaskRayFuture]):
    """
    ...
    def __init__(
        ...
        self.init_kwargs.setdefault("namespace", "syntask")
    ...
    def submit(
        ...
        parameters, upstream_ray_obj_refs = self._exchange_syntask_for_ray_futures(
            parameters
        )
        task_run_id = uuid4()
        ...
        object_ref = (
            ray_decorator(self._run_syntask_task)
        ...
        return SyntaskRayFuture(task_run_id=task_run_id, wrapped_future=object_ref)
    ...
    def map(
        ...
        parameters: Dict[str, Any],
        wait_for: Optional[Iterable[SyntaskFuture]] = None,
    ) -> SyntaskFutureList[SyntaskRayFuture[R]]: ...
    ...
    def _exchange_syntask_for_ray_futures(self, kwargs_syntask_futures):
        """Exchanges Syntask futures for Ray futures."""

        upstream_ray_obj_refs = []

        def exchange_syntask_for_ray_future(expr):
            """Exchanges Syntask future for Ray future."""
            if isinstance(expr, SyntaskRayFuture):
        ...
        kwargs_ray_futures = visit_collection(
            kwargs_syntask_futures,
            visit_fn=exchange_syntask_for_ray_future,
            return_data=True,
        )
    ...
    def _run_syntask_task(
        *upstream_ray_obj_refs,
        ...
        """Resolves Ray futures before calling the actual Syntask task function.
