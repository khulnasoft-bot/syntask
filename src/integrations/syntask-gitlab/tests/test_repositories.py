...
import pytest
import syntask_gitlab
from pydantic import SecretStr
from syntask_gitlab.credentials import GitLabCredentials
from syntask_gitlab.repositories import GitLabRepository  # noqa: E402
...
class TestGitLab:
    ...
    async def test_repository_default(self, monkeypatch):
        ...
        monkeypatch.setattr(syntask_gitlab.repositories, "run_process", mock)
        g = GitLabRepository(repository="syntask")
        await g.get_directory()
    ...
    async def test_reference_default(self, monkeypatch):
        ...
        mock = AsyncMock(return_value=p())
        monkeypatch.setattr(syntask_gitlab.repositories, "run_process", mock)
        g = GitLabRepository(repository="syntask", reference="2.0.0")
        await g.get_directory()
    ...
    async def test_https_connection_with_token_added_correctly(self, monkeypatch):
        ...
        monkeypatch.setattr(syntask_gitlab.repositories, "run_process", mock)
        credential = "XYZ"
        repo = "https://gitlab.com/SynoPKG/syntask.git"
        g = GitLabRepository(
            repository=repo,
        ...
        expected_cmd = [
            "git",
            "clone",
            f"https://oauth2:{credential}@gitlab.com/SynoPKG/syntask.git",
            "--depth",
            "1",
        ]
        assert mock.await_args[0][0][: len(expected_cmd)] == expected_cmd
    ...
    async def test_http_connection_with_token_added_correctly(self, monkeypatch):
        ...
        monkeypatch.setattr(syntask_gitlab.repositories, "run_process", mock)
        credential = "XYZ"
        repo = "http://gitlab.xxx.com/SynoPKG/syntask.git"
        g = GitLabRepository(
            repository=repo,
            credentials=GitLabCredentials(token=SecretStr(credential)),
        )
        ...
        expected_cmd = [
            "git",
            "clone",
            f"http://oauth2:{credential}@gitlab.xxx.com/SynoPKG/syntask.git",
            "--depth",
            "1",
        ]
        assert mock.await_args[0][0][: len(expected_cmd)] == expected_cmd
    ...
    async def test_cloning_with_custom_depth(self, monkeypatch):
        ...
        mock = AsyncMock(return_value=p())
        monkeypatch.setattr(syntask_gitlab.repositories, "run_process", mock)
        repo = "git@gitlab.com:SynoPKG/syntask.git"
        depth = None
        g = GitLabRepository(
            repository=repo,
            git_depth=depth,
    ...
    async def test_dir_contents_copied_correctly_with_get_directory(self, monkeypatch):  # noqa
        ...
        with TemporaryDirectory() as tmp_src:
            ...
            with TemporaryDirectory() as tmp_dst:
                monkeypatch.setattr(
                    syntask_gitlab.repositories,
                    "TemporaryDirectory",
                    self.MockTmpDir,
                )

                g = GitLabRepository(
                    repository="https://gitlab.com/SynoPKG/syntask.git",
                )
    ...
    async def test_get_directory_retries(self, monkeypatch):
        ...
        g = GitLabRepository(repository="https://gitlab.com/synoPKG/syntask.git")
