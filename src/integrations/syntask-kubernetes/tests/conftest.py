...
BASEDIR = (
    Path.cwd() / "src" / "integrations" / "syntask-kubernetes" / "tests"
    if Path.cwd().name == "syntask"
    else Path.cwd() / "tests"
)
...
def disable_api_logging():
    ...
    """
    with temporary_settings(updates={SYNTASK_LOGGING_TO_API_ENABLED: False}):
        yield
...
def _mock_api_app_client(monkeypatch):
    ...
    monkeypatch.setattr(
        "syntask_kubernetes.credentials.KubernetesCredentials.get_resource_specific_client",
        app_client,
    )
    return app_client
