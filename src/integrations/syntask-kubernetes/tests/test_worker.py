...
from syntask_kubernetes import KubernetesWorker
from syntask_kubernetes.utilities import (
    KeepAliveClientRequest,
    _slugify_label_value,
    _slugify_name,
)
from syntask_kubernetes.worker import KubernetesWorkerJobConfiguration
...
def mock_cluster_config(monkeypatch):
    ...
    monkeypatch.setattr("syntask_kubernetes.worker.config", mock)
    monkeypatch.setattr(
        "syntask_kubernetes.worker.config.ConfigException", ConfigException
    )
    return mock
...
def mock_core_client(monkeypatch, mock_cluster_config):
    ...
    monkeypatch.setattr(
        "syntask_kubernetes.worker.KubernetesWorker._get_configured_kubernetes_client",
        MagicMock(spec=ApiClient),
    )
    monkeypatch.setattr("syntask_kubernetes.worker.CoreV1Api", mock)
...
def mock_batch_client(monkeypatch, mock_job):
    ...
    monkeypatch.setattr(
        "syntask_kubernetes.worker.KubernetesWorker._get_batch_client",
        get_batch_client,
    )

    mock.return_value.create_namespaced_job.return_value = mock_job
    monkeypatch.setattr("syntask_kubernetes.worker.BatchV1Api", mock)
    return mock
...
def enable_store_api_key_in_secret(monkeypatch):
    monkeypatch.setenv("SYNTASK_KUBERNETES_WORKER_STORE_SYNTASK_API_IN_SECRET", "true")
                            ...
                            "containers": [
                                {
                                    ...
                                    "env": [
                                        *[
                                        ...
                                        {
                                            "name": "SYNTASK__FLOW_RUN_ID",
                                            "value": str(flow_run.id),
                                        },
                                    ],
                                    "image": get_syntask_image_name(),
                                    "args": [
                                        "syntask",
                                        "flow-run",
                                        "execute",
        ...
        # default base template with custom env
        {
            ...
            "variables": {
                ...
                "properties": {
                    ...
                    "image": {
                        "title": "Image",
                        "description": "The image reference of a container image to use for created jobs. If not set, the latest Syntask image will be used.",
    ...
    (
        ...
        lambda flow_run, deployment, flow: KubernetesWorkerJobConfiguration(
            ...
            labels={
                "syntask.khulnasoft.com/flow-run-id": str(flow_run.id),
                "syntask.khulnasoft.com/flow-run-name": flow_run.name,
                "syntask.khulnasoft.com/version": syntask.__version__.split("+")[0],
                "syntask.khulnasoft.com/deployment-id": str(deployment.id),
                "syntask.khulnasoft.com/deployment-name": deployment.name,
                "syntask.khulnasoft.com/flow-id": str(flow.id),
                "syntask.khulnasoft.com/flow-name": flow.name,
...
class TestKubernetesWorkerJobConfiguration:
    ...
    async def test_user_can_supply_a_sidecar_container_and_volume(self, flow_run):
        ...
        assert pod["containers"][0]["args"] == ["syntask", "flow-run", "execute"]
...
class TestKubernetesWorker:
    ...
    async def test_can_store_api_key_in_secret(
        ...
        with temporary_settings(updates={SYNTASK_API_KEY: "fake"}):
            async with KubernetesWorker(work_pool_name="test") as k8s_worker:
                ...
                assert {
                    ...
                    "valueFrom": {
                        "secretKeyRef": {
                            "name": f"syntask-{_slugify_name(k8s_worker.name)}-api-key",
                            "key": "value",
                        }
                    },
                } in env
        ...
        response.data = json.dumps(
            {
                ...
                "metadata": {},
                "status": "Failure",
                "message": 'jobs.batch is forbidden: User "system:serviceaccount:helm-test:syntask-worker-dev" cannot create resource "jobs" in API group "batch" in the namespace "syntask"',
                "reason": "Forbidden",
        ...
        async with KubernetesWorker(work_pool_name="test") as k8s_worker:
            with pytest.raises(
                ...
                match=re.escape(
                    ...
                    '"system:serviceaccount:helm-test:syntask-worker-dev" cannot '
    ...
    async def test_uses_custom_env_list_from_base_template(
        ...
        assert any(env for env in created_env if env["name"] == "SYNTASK__FLOW_RUN_ID")
