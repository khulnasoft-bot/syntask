...
from syntask_kubernetes.events import EVICTED_REASONS, KubernetesEventsReplicator
...
@pytest.fixture
def worker_resource():
    return {"syntask.resource.id": "syntask.worker.my-k8s-worker"}
...
def related_resources():
    return [
        RelatedResource(
            {
                "syntask.resource.id": "syntask.flow-run.1234",
...
async def test_replicate_successful_pod_events(
    ...
    with patch("syntask_kubernetes.events.emit_event", side_effect=event) as mock_emit:
        async with replicator:
    ...
    mock_emit.assert_has_calls(
        [
            call(
                event="syntask.kubernetes.pod.pending",
                resource={
                    "syntask.resource.id": "syntask.kubernetes.pod.1234",
                    "syntask.resource.name": "test-pod",
                ...
                related=[
                    ...
                    RelatedResource(
                        {
                            "syntask.resource.id": "syntask.worker.my-k8s-worker",
            ...
            call(
                event="syntask.kubernetes.pod.running",
                resource={
                    "syntask.resource.id": "syntask.kubernetes.pod.1234",
                    "syntask.resource.name": "test-pod",
                    "kubernetes.namespace": "test-namespace",
            ...
            call(
                event="syntask.kubernetes.pod.succeeded",
                resource={
                    "syntask.resource.id": "syntask.kubernetes.pod.1234",
                    "syntask.resource.name": "test-pod",
                    "kubernetes.namespace": "test-namespace",
...
async def test_replicate_failed_pod_events(replicator, mock_watch, failed_pod_stream):
    ...
    mock_emit.assert_has_calls(
        [
            ...
            call(
                event="syntask.kubernetes.pod.failed",
                resource={
                    "syntask.resource.id": "syntask.kubernetes.pod.1234",
                    "syntask.resource.name": "test-pod",
                    "kubernetes.namespace": "test-namespace",
...
async def test_replicate_evicted_pod_events(replicator, mock_watch, evicted_pod_stream):
    ...
    mock_emit.assert_has_calls(
        [
            ...
            call(
                event="syntask.kubernetes.pod.evicted",
                resource={
                    "syntask.resource.id": "syntask.kubernetes.pod.1234",
                    "syntask.resource.name": "test-pod",
                    "kubernetes.namespace": "test-namespace",
