...
from syntask_kubernetes.credentials import (
...
def mock_cluster_config(monkeypatch):
    ...
    monkeypatch.setattr("syntask_kubernetes.credentials.config", mock)
    monkeypatch.setattr(
        "syntask_kubernetes.credentials.config.ConfigException", ConfigException
    )
    return mock
