...
class KubernetesEventsReplicator:
    """Replicates Kubernetes pod events to Syntask events."""
    ...
    def _pod_as_resource(self, pod: "V1Pod") -> Dict[str, str]:
        ...
        return {
            "syntask.resource.id": f"syntask.kubernetes.pod.{pod.metadata.uid}",
            "syntask.resource.name": pod.metadata.name,
            "kubernetes.namespace": pod.metadata.namespace,
        }

    async def _replicate_pod_events(self):
        """Replicate Kubernetes pod events as Syntask Events."""
        seen_phases = set()
        last_event = None
    ...
    async def _emit_pod_event(
        ...
        """Emit a Syntask event for a Kubernetes pod event."""
        ...
        return emit_event(
            event=f"syntask.kubernetes.pod.{pod_phase.lower()}",
            resource=resource,
