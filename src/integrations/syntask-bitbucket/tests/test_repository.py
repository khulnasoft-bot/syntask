...
import syntask_bitbucket
from syntask_bitbucket.credentials import BitBucketCredentials
from syntask_bitbucket.repository import BitBucketRepository
...
class TestBitBucketRepository:
    ...
    async def test_repository_default(self, monkeypatch):
        ...
        monkeypatch.setattr(syntask_bitbucket.repository, "run_process", mock)
        b = BitBucketRepository(repository="syntask")
        await b.get_directory()
    ...
    async def test_repository_default_with_credentials_none(self, monkeypatch):
        ...
        mock = AsyncMock(return_value=p())
        monkeypatch.setattr(syntask_bitbucket.repository, "run_process", mock)
        b = BitBucketRepository(repository="syntask", bitbucket_credentials=None)
        await b.get_directory()
    ...
    async def test_reference_default(self, monkeypatch):
        ...
        mock = AsyncMock(return_value=p())
        monkeypatch.setattr(syntask_bitbucket.repository, "run_process", mock)
        b = BitBucketRepository(repository="syntask", reference="2.0.0")
        await b.get_directory()
    ...
    async def test_token_added_correctly_from_credential(self, monkeypatch):
        ...
        monkeypatch.setattr(syntask_bitbucket.repository, "run_process", mock)
        credential = "XYZ"
        repo = "https://bitbucket.org/SynoPKG/syntask.git"
        ...
        expected_cmd = [
            "git",
            "clone",
            f"https://x-token-auth:{credential}@bitbucket.org/SynoPKG/syntask.git",
            "--depth",
            "1",
        ]
        assert mock.await_args[0][0][: len(expected_cmd)] == expected_cmd
    ...
    async def test_token_with_username_added_correctly_from_credential(
        ...
        expected_cmd = [
            "git",
            "clone",
            f"https://{username}:{credential}@bitbucket.org/SynoPKG/syntask.git",
            "--depth",
            "1",
        ]
        assert mock.await_args[0][0][: len(expected_cmd)] == expected_cmd
    ...
    async def test_ssh_fails_with_credential(self, monkeypatch):
        ...
        with pytest.raises(InvalidRepositoryURLError, match=error_msg):
            BitBucketRepository(
                repository="git@bitbucket.org:SynoPKG/syntask.git",
                bitbucket_credentials=BitBucketCredentials(token=credential),
            )
    ...
    async def test_dir_contents_copied_correctly_with_get_directory(self, monkeypatch):  # noqa
        ...
        with TemporaryDirectory() as tmp_src:
            ...
            with TemporaryDirectory() as tmp_dst:
                monkeypatch.setattr(
                    syntask_bitbucket.repository,
                    "TemporaryDirectory",
                    self.MockTmpDir,
                )

                b = BitBucketRepository(
                    repository="https://bitbucket.org/SynoPKG/syntask.git",
                )
