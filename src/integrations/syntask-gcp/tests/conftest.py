...
@pytest.fixture(scope="session", autouse=True)
def disable_logging():
    with temporary_settings({SYNTASK_LOGGING_TO_API_ENABLED: False}):
        yield
...
def oauth2_credentials(monkeypatch):
    ...
    monkeypatch.setattr("syntask_gcp.credentials.Credentials", CredentialsMock)
...
def storage_client(monkeypatch):
    monkeypatch.setattr("syntask_gcp.credentials.StorageClient", CloudStorageClient)
...
def mock_credentials(monkeypatch):
    ...
    monkeypatch.setattr(
        "syntask_gcp.credentials.Credentials",  # noqa
        mock_credentials,
    )
    mock_auth = MagicMock()
    mock_auth.default.return_value = (mock_authenticated_credentials, "project")
    monkeypatch.setattr(
        "syntask_gcp.credentials.google.auth",  # noqa
        mock_auth,
    )
    monkeypatch.setattr(
        "syntask_gcp.credentials.Credentials.from_service_account_info.universe_domain",  # noqa
