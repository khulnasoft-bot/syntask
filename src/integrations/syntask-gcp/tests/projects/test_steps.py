...
import pytest
from syntask_gcp.deployments.steps import pull_from_gcs, push_to_gcs
...
def mock_credentials(monkeypatch):
    ...
    monkeypatch.setattr(
        "syntask_gcp.deployments.steps.Credentials",  # noqa
        mock_credentials,
    )
    mock_auth = MagicMock()
    mock_auth.default.return_value = (mock_authenticated_credentials, "project")
    monkeypatch.setattr(
        "syntask_gcp.deployments.steps.google.auth",  # noqa
        mock_auth,
    )
...
def gcs_setup(monkeypatch, mock_credentials):
    ...
    monkeypatch.setattr(
        "syntask_gcp.deployments.steps.StorageClient",
