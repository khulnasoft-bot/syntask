...
```bash
syntask work-pool create 'my-vertex-pool' --type vertex-ai
```

Then start a Cloud Run worker with the following command:

```bash
syntask worker start --pool 'my-vertex-pool'
```
...
class VertexAIWorker(BaseWorker):
    """Syntask worker that executes flow runs within Vertex AI Jobs."""
    ...
    _documentation_url = "https://synopkg.github.io/syntask-gcp/vertex_worker/"
    ...
    def _get_compatible_labels(
        ...
        Ex: the Syntask provided key of syntask.khulnasoft.com/flow-name -> syntask-io_flow-name
        """
        compatible_labels = {}
