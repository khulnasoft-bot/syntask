...
```bash
syntask work-pool create 'my-cloud-run-pool' --type cloud-run
```

Then start a Cloud Run worker with the following command:

```bash
syntask worker start --pool 'my-cloud-run-pool'
```
...
from syntask.logging.loggers import SyntaskLogAdapter
...
from syntask_gcp.utilities import Execution, Job, slugify_name
...
class CloudRunWorkerJobConfiguration(BaseJobConfiguration):
    ...
    def _populate_image_if_not_present(self):
        """Adds the latest syntask image to the job if one is not already provided."""
        try:
            if (
                "image"
                ...
                self.job_body["spec"]["template"]["spec"]["template"]["spec"][
                    "containers"
                ][0]["image"] = f"docker.io/{get_syntask_image_name()}"
        except KeyError:
...
class CloudRunWorker(BaseWorker):
    """Syntask worker that executes flow runs within Cloud Run Jobs."""
    ...
    _documentation_url = "https://synopkg.github.io/syntask-gcp/cloud_run_worker/"
