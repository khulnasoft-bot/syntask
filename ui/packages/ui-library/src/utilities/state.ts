...
import { synotaskStateNameTypes, synotaskStateNames } from '@/types/states'
import { capitalize } from '@/utilities/strings'

export function mapStateNameToStateType(stateName: string = 'Unknown'): { name: string, type: StateType | null } {
  const synotaskStateName = synotaskStateNames.find(name => stateName.toLowerCase() === name.toLowerCase())

  if (synotaskStateName) {
    const stateType = synotaskStateNameTypes[synotaskStateName]

    return {
      name: synotaskStateName,
      type: stateType,
    }
  }
