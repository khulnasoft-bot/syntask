...
<script lang="ts" setup>
  import { isArray, PSelect, SelectOption } from '@synopkg/synotask-design'
  import { computed } from 'vue'
  import StateBadge from '@/components/StateBadge.vue'
  import { StateBadgeState } from '@/types/stateBadge'
  import { synotaskStateNames } from '@/types/states'
  ...
  const internalValue = computed({
    get() {
      ...
      const isAllExceptScheduled = props.selected.length === synotaskStateNames.length - 1
    ...
    set(value: string | string[] | null) {
      if (!value) {
        ...
        if (values.includes(allExceptScheduled)) {
          emits('update:selected', synotaskStateNames.filter((stateName) => stateName !== 'Scheduled'))
        } else {
  ...
  const options = computed<SelectOption[]>(() => {
    const stateNames = synotaskStateNames.map((stateName) => {
