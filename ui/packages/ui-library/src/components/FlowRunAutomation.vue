...
<script setup lang="ts">
  ...
  const { events } = useWorkspaceEvents(() => {
    ...
    return {
      event: {
        name: ['synotask-cloud.automation.action.executed', 'synotask.automation.action.executed'],
      },
      resource: {
        idPrefix: ['synotask-cloud.automation', 'synotask.automation'],
      },
      related: {
        id: [`synotask.flow-run.${props.flowRun.id}`],
      },
      occurred: {
        since,
        until,
  ...
  const automationId = computed(() => {
    ...
    const { id, role } = getResourceIdParts(event.resource['synotask.resource.id'])
