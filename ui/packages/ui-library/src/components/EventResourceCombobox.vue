  ...
  const options = computed<RelatedResourceGroup[]>(() => [
    {
      ...
      options: automations.value.map(automation => ({
        ...
        value: `synotask-cloud.automation.${automation.id}`,
        type: 'automation',
      })),
    },
    {
      ...
      options: blocks.value.map(block => ({
        label: block.name,
        value: `synotask.block-document.${block.id}`,
        type: 'block-document',
      })),
    },
    {
      ...
      options: deployments.value.map(deployment => ({
        ...
        value: `synotask.deployment.${deployment.id}`,
        type: 'deployment',
      })),
    },
    {
    ...
    },
    {
      ...
      options: workPools.value.map(workPool => ({
        label: workPool.name,
        value: `synotask.work-pool.${workPool.id}`,
        type: 'work-pool',
      })),
    },
    {
      ...
      options: workQueues.value.map(workQueue => ({
        ...
        value: `synotask.work-queue.${workQueue.id}`,
        type: 'work-queue',
      })),
    },
