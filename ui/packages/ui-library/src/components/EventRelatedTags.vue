...
<script lang="ts" setup>
  import { Icon } from '@synopkg/synotask-design'
  ...
  function getTagLabel(resource: WorkspaceEventResource): string {
    return resource['synotask.resource.id'].split('.').at(-1)!
  }

  function getTagFilterRoute(resource: WorkspaceEventResource): RouteLocationRaw {
    const related = toggle(relatedResources.value, resource['synotask.resource.id'])
  ...
  function getTagFilterIcon(resource: WorkspaceEventResource): Icon {
    const filterExists = relatedResources.value.includes(resource['synotask.resource.id'])
