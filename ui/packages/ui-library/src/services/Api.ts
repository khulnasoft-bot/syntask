...
export type SyntaskConfig = {
  baseUrl: string,
  token?: string,
}

type ConfigFunction<R, T extends SyntaskConfig = SyntaskConfig> = (config: T) => R
export type ApiBaseUrl<T extends SyntaskConfig = SyntaskConfig> = string | ConfigFunction<string, T>
export type ApiHeaders<T extends SyntaskConfig = SyntaskConfig> = RawAxiosRequestHeaders | ConfigFunction<RawAxiosRequestHeaders, T>

export const getSyntaskBaseUrl: ApiBaseUrl = (config) => config.baseUrl

export const getSyntaskUIHeaders: RawAxiosRequestHeaders = { 'X-SYNTASK-UI': true }
...
export class Api<T extends SyntaskConfig = SyntaskConfig> {
  protected readonly apiConfig: MaybeGetter<T>
  protected apiHeaders: MaybeArray<ApiHeaders> = [getSyntaskUIHeaders, getAuthorizationHeaders]
  protected apiBaseUrl: ApiBaseUrl = getSyntaskBaseUrl
