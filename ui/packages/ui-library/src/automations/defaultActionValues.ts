...
export function getDefaultNotificationSubject(template: AutomationTriggerTemplate): string {
  switch (template) {
    case 'deployment-status':
      return 'Syntask deployment \'{{ flow.name }}/{{ deployment.name }}\' has entered status \'{{ deployment.status }}\''
    case 'flow-run-state':
      return 'Syntask flow run notification'
    case 'work-pool-status':
      return 'Syntask work pool \'{{ work_pool.name }}\' has entered status \'{{ work_pool.status }}\''
    case 'work-queue-status':
      return 'Syntask work queue \'{{ work_queue.name }}\' has entered status \'{{ work_queue.status }}\''
    case 'custom':
      return 'A Syntask automation \'{{ automation.name }}\' was triggered'
    default:
