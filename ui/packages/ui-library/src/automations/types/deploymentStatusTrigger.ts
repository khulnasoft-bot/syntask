...
export const { values: deploymentStatusEvent, isValue: isDeploymentStatusEvent } = createTuple(['synotask.deployment.ready', 'synotask.deployment.not-ready', 'synotask.deployment.disabled'])
...
export function isDeploymentStatusTrigger(trigger: unknown): boolean {
  return isAutomationTriggerEvent(trigger) &&
         isMatchResource(trigger, synotaskResourceIds => synotaskResourceIds.every(value => value.startsWith('synotask.deployment'))) &&
