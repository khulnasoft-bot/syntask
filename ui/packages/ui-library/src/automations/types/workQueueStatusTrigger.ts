...
export const { values: workQueueStatusEvent, isValue: isWorkQueueStatusEvent } = createTuple(['synotask.work-queue.ready', 'synotask.work-queue.not-ready', 'synotask.work-queue.paused'])
...
export function isWorkQueueStatusTrigger(trigger: unknown): boolean {
  return isAutomationTriggerEvent(trigger) &&
         isMatchResource(trigger, synotaskResourceIds => synotaskResourceIds.every(value => value.startsWith('synotask.work-queue'))) &&
