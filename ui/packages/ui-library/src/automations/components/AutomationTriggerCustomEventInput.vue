  ...
  <p-form>
    ...
    <p-label label="From the following resources">
      <EventResourceCombobox
        :selected="formData.match['synotask.resource.id'] ?? []"
        multiple
      ...
      <template #content>
        <div class="automation-trigger-custom-event-input__evaluation-options-accordion-content">
          ...
          <p-label label="Filter for events related to">
            <EventResourceCombobox
              :selected="formData.matchRelated['synotask.resource.id'] ?? []"
              multiple
...
<script setup lang="ts">
  import { asArray, toPluralString } from '@synopkg/synotask-design'
  import { ValidationRule, useValidation } from '@synopkg/vue-compositions'
  ...
  function updateMatchResourceIds(selectedResources: string[]): void {
    ...
    if (selectedResources.length === 0) {
      delete formData.value.match['synotask.resource.id']
      return
    }

    formData.value.match['synotask.resource.id'] = selectedResources
  }
  ...
  function updateMatchRelatedResourceIds(selectedResources: string[]): void {
    ...
    if (selectedResources.length === 0) {
      delete formData.value.matchRelated['synotask.resource.id']
      return
    }
    formData.value.matchRelated['synotask.resource.id'] = selectedResources
  }
