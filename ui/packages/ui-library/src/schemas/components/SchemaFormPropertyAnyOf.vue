...
<script lang="ts" setup>
  import { ButtonGroupOption } from '@synopkg/synotask-design'
  ...
  import { SchemaValue, isSyntaskKindValue } from '@/schemas/types/schemaValues'
  ...
  const internalValue = computed(() => {
    if (isSyntaskKindValue(props.value)) {
      return props.value
    }
  ...
  async function updateValue(value: unknown): Promise<void> {
    ...
    // https://github.com/Synopkg/synotask/issues/14265
    if (value === undefined) {
