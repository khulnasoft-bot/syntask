...
<script lang="ts" setup>
  import { isDefined, isNotNullish } from '@synopkg/synotask-design'
  ...
  import { SchemaValue, getSyntaskKindFromValue } from '@/schemas/types/schemaValues'
  ...
  const props = defineProps<{
    ...
    // https://github.com/Synopkg/synotask-ui-library/pull/2355
  ...
  const kind = computed(() => getSyntaskKindFromValue(() => props.value))
  ...
  const { errors: propertyErrors, setKind } = useSyntaskKindValue({ value, property: () => props.propertyForValidation ?? props.property })
