...
import { isSyntaskKindValue } from '@/schemas/types/schemaValues'
...
export function useSchemaPropertyInput(schemaProperty: MaybeRefOrGetter<SchemaProperty>, propertyValue: Ref<SchemaValue>, propertyErrors: MaybeRefOrGetter<SchemaValueError[]>) {
  const input = computed(() => {
    ...
    if (!isSyntaskKindValue(propertyValue.value)) {
    ...
    if (isSyntaskKindValue(propertyValue.value, 'json')) {
    ...
    if (isSyntaskKindValue(propertyValue.value, 'jinja')) {
    ...
    if (isSyntaskKindValue(propertyValue.value, 'workspace_variable')) {
    ...
    if (isSyntaskKindValue(propertyValue.value, 'none')) {
      return withProps(SchemaFormPropertyInput, {
        property,
        value: propertyValue.value,
        errors,
