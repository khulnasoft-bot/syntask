...
import { WorkspaceEventResource, SyntaskResourceRole, WorkspaceEvent } from '@/models/workspaceEvent'
import { getResourceIdParts } from '@/utilities/events'

export type UseEventResource = {
  id: ComputedRef<string | null>,
  role: ComputedRef<SyntaskResourceRole | null>,
...
export function useWorkspaceEventResource(resource: MaybeRefOrGetter<WorkspaceEventResource>): UseEventResource {
  const resourceId = computed(() => {
    ...
    return value['synotask.resource.id']
  })
  ...
  const name = computed(() => {
    const value = toValue(resource)
    return value['synotask.resource.name'] ?? value['synotask.name'] ?? value['synotask-cloud.name'] ?? null
  })
