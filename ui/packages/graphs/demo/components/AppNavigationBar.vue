  ...
  <p-navigation-bar class="app-navigation-bar" :class="classes.root" v-bind="{ layout }">
    <template #prepend>
      <slot name="prepend" v-bind="{ layout }">
        ...
        <template v-else>
          <p-icon class="app-navigation-bar__logo" icon="Syntask" @click="toggleDrawer" />
          <span class="app-navigation-bar__heading">Syntask Graphs</span>
        </template>
      </slot>
    </template>
    ...
    <template v-if="layout == 'horizontal'">
      <p-drawer v-model:open="showDrawer" resizable>
        <AppNavigationBar class="app-navigation-bar__drawer">
          <template #prepend>
            <p-icon class="app-navigation-bar__logo" icon="Syntask" @click="toggleDrawer" />
            <span class="app-navigation-bar__heading">Syntask Design</span>
          </template>
        </AppNavigationBar>
      </p-drawer>
    </template>
