...
In Syntask Cloud, these block values are stored in encrypted format, allowing you to use Syntask to 
...
AWS Secrets Manager.
This example uses Syntask Cloud and generally works with other third-party services that require 
credentials.

## Prerequisites

1. Syntask [installed](/3.0/get-started/install)
1. CLI authenticated to your [Syntask Cloud](https://app.syntask.cloud) account
1. [Snowflake account](https://www.snowflake.com/)
1. [AWS account](https://aws.amazon.com/)

## Steps

### Step 1: Install `prefect-aws` and `syntask-snowflake` libraries

The following code installs and upgrades the necessary libraries and their dependencies:

```bash
pip install -U prefect-aws syntask-snowflake
```
...
Create blocks with Python code or through the Syntask UI. Block creation through the UI helps you visualize 
...
<Note>
If using self-hosted Syntask server, you must register the block types in the 
newly installed modules before you can create blocks.
</Note>

```bash
syntask block register -m prefect_aws && syntask block register -m syntask_snowflake
```
...
By leaving those attributes blank, Syntask knows to look to the compute environment.
...
Under the hood, Syntask uses the AWS `boto3` client to create a session.

In the **AwsCredentials** section of the form, click **Add +** and create an AWS Credentials block by 
entering the necessary values.

If the compute environment contains the necessary credentials, Syntask will use them to authenticate in the 
...
When you connect to Snowflake, Syntask automatically uses these credentials to authenticate and access the 
...
Use Syntask's blocks for convenient access to Snowflake.
Don't save the blocks to ensure the credentials are not stored in Syntask Cloud.
...
Again, neither of the Snowflake blocks are saved, so the credentials are not stored in Syntask Cloud.

See [`syntask-snowflake`](/integrations/syntask-snowflake) for more examples of working with Snowflake.
...
Make sure to specify the `prefect-aws` and `syntask-snowflake` dependencies in your work pool or deployment 
...
You've seen how to use Syntask blocks to store non-sensitive configuration and fetch sensitive configuration 
