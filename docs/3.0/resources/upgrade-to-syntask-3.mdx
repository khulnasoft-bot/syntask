---
title: Upgrade to Syntask 3.0
description: Learn how to upgrade from Syntask 2.x to Syntask 3.0.
---

Syntask 3.0 introduces a number of enhancements to the OSS product: a new events & automations backend for event-driven workflows and observability, improved runtime performance, autonomous task execution and a streamlined caching layer based on transactional semantics.

The majority of these enhancements maintain compatibility with most Syntask 2.0 workflows, but there are a few caveats that you may need to adjust for.

To learn more about the enhanced performance and new features, see [What's new in Syntask 3.0](/3.0/resources/whats-new-syntask-3).

For the majority of users, upgrading to Syntask 3.0 will be a seamless process that requires few or no code changes. 
This guide highlights key changes that you may need to consider when upgrading.

<Info>
**Syntask 2.0** refers to the 2.x lineage of the open source syntask package, and **Syntask 3.0** refers exclusively to the 3.x lineage of the syntask package. Neither version is strictly tied to any aspect of Syntask's commercial product, [Syntask Cloud](/3.0/manage/cloud). 
</Info>

## Quickstart

To upgrade to Syntask 3.0, run:

```bash
pip install -U syntask
```

If you self-host a Syntask server, run this command to update your database:

```bash
syntask server database upgrade
```

If you use a Syntask integration or extra, remember to upgrade it as well. For example:
...
<Info>
This change affects you if: you use custom Pydantic models with Syntask features.
</Info>

Syntask 3.0 is built with Pydantic 2.0 for improved performance. All Syntask objects will automatically upgrade, but if you use custom Pydantic models for flow parameters or custom blocks, you'll need to ensure they are compatible with Pydantic 2.0. You can continue to use Pydantic 1.0 models in your own code if they do not interact directly with Syntask.
...
In Syntask 2.0, the final state of a flow run was influenced by the states of its task runs; if any task run failed, the flow run was marked as failed.

In Syntask 3.0, the final state of a flow run is entirely determined by:

1. The `return` value of the flow function (same as in Syntask 2.0):
...
<Warning>
When migrating from Syntask 2.0 to Syntask 3, be aware that flows may now complete successfully even if they contain failed tasks, unless you explicitly handle task failures.
</Warning>
...
<info>
This change affects you if: you directly interact with `SyntaskFuture` objects.
</info>

SyntaskFutures now have a standard synchronous interface, with an asynchronous one [planned soon](https://github.com/synopkg/syntask/issues/15008).
...
Syntask 3.0 introduces a powerful idempotency engine. By default, tasks in a flow run are automatically cached if they are called more than once with the same inputs. If you rely on tasks with side effects, this may result in surprising behavior. To disable caching, pass `cache_policy=None` to your task.

### Workers

<info>
This change affects you if: you're using agents from an early version of Syntask 2.0.
</info>

In Syntask 2.0, agents were deprecated in favor of next-generation workers. Workers are now standard in Syntask 3. For detailed information on upgrading from agents to workers, please refer to our [upgrade guide](https://docs-3.syntask.khulnasoft.com/3.0/resources/upgrade-agents-to-workers).
...
For example, `my_task.submit(...)` is _always_ synchronous in Syntask 3.x:
...
@flow
async def my_flow():
    future = await my_task.submit() # TypeError: object SyntaskConcurrentFuture can't be used in 'await' expression
```
