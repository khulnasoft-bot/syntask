...
Cancellation is resilient to restarts of Syntask workers.
...
The scope ensures that Syntask does not kill the wrong infrastructure.
...
`syntask flow-run cancel` CLI command, passing the ID of the flow run.
...
![Syntask UI](/3.0/img/ui/flow-run-cancellation-ui.png)
