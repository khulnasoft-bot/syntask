...
A Syntask task is a discrete unit of work in a Syntask workflow. 
...
Syntask encourages "small tasks." Each one should represent a single logical step of your workflow. 
...
Almost any standard Python function can be turned into a Syntask task by adding the `@task` decorator. 

Syntask uses client-side task run orchestration by default, which significantly improves performance, especially for workflows with many tasks. Task creation and state updates happen locally, reducing API calls to the Syntask server during execution. This enables efficient handling of large-scale workflows and improves reliability when server connectivity is intermittent.
<Tip>
...
The simplest Syntask task is a synchronous Python function. Here's an example of a synchronous task that prints a message:
...
Syntask also supports asynchronous Python functions. 
...
Syntask supports synchronous and asynchronous methods as tasks, including instance methods, class methods, and static methods. For class methods and static methods, you must apply the appropriate method decorator _above_ the `@task` decorator:
...
Tasks expose a [`submit`](https://syntask-python-sdk-docs.netlify.app/syntask/tasks/#syntask.tasks.Task.submit) method that changes 
the execution from sequential to concurrent.
In this example, you also need to use the 
[`result`](https://syntask-python-sdk-docs.netlify.app/syntask/futures/#syntask.futures.SyntaskFuture.result) 
...
Tasks allow for customization through optional arguments that can be provided to the [task decorator](https://syntask-python-sdk-docs.netlify.app/syntask/tasks/#syntask.tasks.task).
...
| `tags`                | An optional set of tags associated with runs of this task. These tags are combined with any tags defined by a `syntask.tags` context at task runtime.                                                             |
...
See all possible options in the [Python SDK docs](https://syntask-python-sdk-docs.netlify.app/syntask/tasks/#syntask.tasks.task).
...
- Filter task runs by tag in the UI and through the [Syntask REST API](/3.0/api-ref/rest-api/#filtering).
- Set [concurrency limits](#task-run-concurrency-limits) on task runs by tag.

You may specify tags as a keyword argument on the [task decorator](https://syntask-python-sdk-docs.netlify.app/syntask/tasks/#syntask.tasks.task).
...
Alternatively, specify tags when the task is called rather than in its definition with a [`tags` context manager](https://syntask-python-sdk-docs.netlify.app/syntask/context/#syntask.context.tags), .
...
Syntask can automatically retry task runs on failure. 
...
task. 
If the task run fails, Syntask will retry it up to `retries` times, waiting
`retry_delay_seconds` seconds between each attempt. 
If the task fails on the final retry, Syntask marks the task as _failed_.
...
Syntask includes an [`exponential_backoff`](https://syntask-python-sdk-docs.netlify.app/syntask/tasks/#syntask.tasks.exponential_backoff) utility that will automatically generate a list of retry delays that correspond to an exponential backoff retry strategy. 
