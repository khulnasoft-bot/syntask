...
The default task runner in Syntask is the [`ThreadPoolTaskRunner`](https://syntask-python-sdk-docs.netlify.app/syntask/task-runners/#syntask.task_runners.ThreadPoolTaskRunner),
...
- [`DaskTaskRunner`](https://github.com/synopkg/syntask/tree/main/src/integrations/prefect-dask) can run tasks using [`dask.distributed`](http://distributed.dask.org/).
- [`RayTaskRunner`](https://github.com/synopkg/syntask/tree/main/src/integrations/syntask-ray) can run tasks using [Ray](https://www.ray.io/).
...
This method returns a [`SyntaskFuture`](https://syntask-python-sdk-docs.netlify.app/syntask/futures/#syntask.futures.SyntaskFuture), which is a Syntask object that contains:
- a reference to the payload returned by the task;
- and a [`State`](https://syntask-python-sdk-docs.netlify.app/syntask/server/schemas/states/), which is a Syntask object indicating the state of the task run.

<Warning>
Syntask future objects must be resolved explicitly before returning from a flow. 
...
from syntask.futures import wait
...
Syntask uses the default `ThreadPoolTaskRunner`.
</Tip>
...
[`SyntaskFuture`](https://syntask-python-sdk-docs.netlify.app/syntask/futures/#syntask.futures.SyntaskFuture) for access to the state and 
result of the task.

A `SyntaskFuture` is an object that provides access to a computation happening in a task runner, 
even if that computation happens on a remote system.

When you pass a future into a task, Syntask automatically waits for the "upstream" task (the one that the future references), 
to reach a final state before starting the downstream task.

This means that the downstream task won't receive the `SyntaskFuture` you passed as an argument. 
...
Notice that we only had to wait for the final `print_result` future as Syntask automatically resolved `say_hello` to a string.
...
You may also use the [`wait_for=[]`](https://syntask-python-sdk-docs.netlify.app/syntask/tasks/#syntask.tasks.Task.submit) parameter 
...
Syntask task) but _not_ if you are passing the value to another task.
</Note>

## Mapping over iterables

Syntask also provides a `.map()` method that automatically submits a new task run for each element of its 
input data. 
...
If your static argument is an iterable, wrap it with `unmapped` to tell Syntask to treat it 
as a static value.
