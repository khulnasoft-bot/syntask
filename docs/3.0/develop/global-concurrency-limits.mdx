...
In the context of global concurrency and rate limits, "tasks" doesn't specifically refer to Syntask tasks, but to concurrent units of work in 
general—such as those managed by an event loop or `TaskGroup` in asynchronous programming. These general "tasks" could include Syntask 
...
You can create, read, edit, and delete concurrency limits through the Syntask UI, CLI, or Python SDK.
...
You can manage global concurrency limits in the **Concurrency** section of the Syntask UI.
...
You can manage global concurrency limits through the Syntask CLI.

To create a new concurrency limit, use the `syntask gcl create` command. You must specify a `--limit` argument, and can optionally specify a 
`--slot-decay-per-second` and `--disable` argument.

```bash
syntask gcl create my-concurrency-limit --limit 5 --slot-decay-per-second 1.0
```

Inspect the details of a concurrency limit using the `syntask gcl inspect` command:

```bash
syntask gcl inspect my-concurrency-limit
```

To update a concurrency limit, use the `syntask gcl update` command. You can update the `--limit`, `--slot-decay-per-second`, `--enable`, 
and `--disable` arguments:

```bash
syntask gcl update my-concurrency-limit --limit 10
```

```bash
syntask gcl update my-concurrency-limit --disable
```

To delete a concurrency limit, use the `syntask gcl delete` command:

```bash
syntask gcl delete my-concurrency-limit
...
See all available commands and options by running `syntask gcl --help`.
...
1. The code imports the necessary modules and the concurrency context manager. Use the `syntask.concurrency.sync` module for sync usage 
and the `syntask.concurrency.asyncio` module for async usage.
...
1. The code imports the necessary modules and the `rate_limit` function. Use the `syntask.concurrency.sync` module for sync usage and the 
`syntask.concurrency.asyncio` module for async usage.
...
In addition to global concurrency limits, Syntask provides several other ways to limit concurrency for fine-grained control. 

Unlike global concurrency limits, which are a more general way to control concurrency for any Python-based operation, the following concurrency limit options are specific to Syntask objects:
