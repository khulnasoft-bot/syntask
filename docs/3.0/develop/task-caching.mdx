...
By default Syntask's caching logic is based on the following attributes of a task invocation:
...
To turn on result persistence for all of your tasks use the `SYNTASK_RESULTS_PERSIST_BY_DEFAULT` setting:
...
[settings](/3.0/manage/settings-and-profiles) for more details on managing Syntask settings.
</Warning>

## Cache keys

To determine whether a task run should retrieve a cached state, Syntask uses the concept of a "cache key". 
...
When a task run begins, Syntask first computes its cache key and uses this key to lookup a record in the task's result
storage. 
...
By default Syntask stores results locally in `~/.syntask/storage/`. 
...
Syntask comes prepackaged with a few common cache policies:
...
These policies can be set using the `cache_policy` keyword on the [task decorator](https://syntask-python-sdk-docs.netlify.app/syntask/tasks/#syntask.tasks.task):

```python
from syntask import task
from syntask.cache_policies import TASK_SOURCE
...
Syntask allows you to configure task caching behavior in numerous ways.

### Cache expiration

All cache keys can optionally be given an _expiration_ through the `cache_expiration` keyword on 
the [task decorator](https://syntask-python-sdk-docs.netlify.app/syntask/tasks/#syntask.tasks.task).
This keyword accepts a `datetime.timedelta` specifying a duration for which the cached value should be
considered valid.

Providing an expiration value results in Syntask persisting an expiration timestamp alongside the result
record for the task.
...
Cache isolation controls how concurrent task runs interact with cache records. Syntask supports two isolation levels: `READ_COMMITTED` and `SERIALIZABLE`.
...
We recommend using the `RedisLockManager` provided by `syntask-redis` in conjunction with a shared Redis instance:
...
This can be achieved in Syntask by configuring these tasks to always write to their caches within 
...
```python
from syntask.cache_policies import INPUTS, RUN_ID
...
A cache "refresh" instructs Syntask to ignore the data associated with a task's cache key and rerun 
no matter what.
...
To refresh the cache for all tasks, use the `SYNTASK_TASKS_REFRESH_CACHE` setting. 
Setting `SYNTASK_TASKS_REFRESH_CACHE=true` changes the default behavior of all tasks to refresh. 
This is particularly useful to rerun a flow without cached results.
See [settings](/3.0/manage/settings-and-profiles) for more details on managing Syntask settings.
