...
description: Syntask blocks allow you to manage configuration schemas, infrastructure, and secrets for use with deployments or flow scripts.
---

Syntask blocks store typed configuration that can be used across workflows and deployments.
The most common use case for blocks is storing credentials used to access external systems such as AWS or GCP.
Syntask supports [a large number of common blocks](#pre-registered-blocks) and it's also easy to create your own!
...
A block _type_ is essentially a schema registered with the Syntask API. This schema can be inspected and discovered in the UI on the **Blocks** page.

To see block types available for configuration, use `syntask block type ls` 
...
These types separate blocks from [Syntask variables](/3.0/develop/variables/), which are unstructured JSON documents.
...
A block _class_ is the primary user-facing object; it is a Python class whose attributes are loaded from a block document. Most Syntask blocks encapsulate additional functionality built on top of the block document. 
...
Using existing or built-in Syntask blocks is as simple as importing the relevant class and calling its `save` and `load` methods.
Note that many Syntask blocks are packaged into one of [Syntask's integration libraries](#blocks-in-syntask-integration-libraries).
...
```bash
syntask block delete s3-bucket/my-data-bucket-block
```
...
```bash
syntask block register --module prefect_aws.credentials
```

This command is useful for registering all blocks found within a module in a [Syntask Integration library](/integrations/).
...
Syntask's `SecretDict` field type allows you to add a dictionary field to your block 
...
read and write objects from (this field exists on [our implementation](https://github.com/synopkg/prefect-aws/blob/main/prefect_aws/s3.py#L292)).
...
Then [register the updated block type](#register-blocks) with either your Syntask Cloud account or your self-hosted Syntask server instance.
...
Commonly used block types come built-in with Syntask. 
...
Find available block types in many of the published [Syntask integrations libraries](/integrations/).
...
| Azure Blob Storage Credentials           | `azure-blob-storage-credentials`       | [syntask-azure](/integrations/syntask-azure/)             |
| Azure Container Instance Credentials | `azure-container-instance-credentials` | [syntask-azure](/integrations/syntask-azure/)             |
| Azure Container Instance Job]   | `azure-container-instance-job`         | [syntask-azure](/integrations/syntask-azure/)             |
| Azure Cosmos DB Credentials      | `azure-cosmos-db-credentials`          | [syntask-azure](/integrations/syntask-azure/)             |
| AzureML Credentials    | `azureml-credentials`                  | [syntask-azure](/integrations/syntask-azure/)             |
...
| Email Server Credentials        | `email-server-credentials`             | [syntask-email](/integrations/syntask-email/)             |
| BigQuery Warehouse     | `bigquery-warehouse`                   | [syntask-gcp](/integrations/syntask-gcp/)                 |
| GCP Cloud Run Job              | `cloud-run-job`                        | [syntask-gcp](/integrations/syntask-gcp/)                 |
| GCP Credentials          | `gcp-credentials`                      | [syntask-gcp](/integrations/syntask-gcp/)                 |
| GcpSecret         | `gcpsecret`                            | [syntask-gcp](/integrations/syntask-gcp/)                 |
| GCS Bucket      | `gcs-bucket`                           | [syntask-gcp](/integrations/syntask-gcp/)                 |
| Vertex AI Custom Training Job   | `vertex-ai-custom-training-job`        | [syntask-gcp](/integrations/syntask-gcp/)                 |
| GitHub Credentials                    | `github-credentials`                   | [syntask-github](/integrations/syntask-github/)           |
| GitHub Repository          | `github-repository`                    | [syntask-github](/integrations/syntask-github/)           |
| GitLab Credentials         | `gitlab-credentials`                   | [syntask-gitlab](/integrations/syntask-gitlab/)           |
| GitLab Repository           | `gitlab-repository`                    | [syntask-gitlab](/integrations/syntask-gitlab/)           |
| Kubernetes Cluster Config        | `kubernetes-cluster-config`               | [syntask-kubernetes](/integrations/syntask-kubernetes/)   |
| Kubernetes Credentials        | `kubernetes-credentials`               | [syntask-kubernetes](/integrations/syntask-kubernetes/)   |
| Kubernetes Job        | `kubernetes-job`               | [syntask-kubernetes](/integrations/syntask-kubernetes/)   |
| Shell Operation                 | `shell-operation`| [syntask-shell](/integrations/syntask-shell/)             |
| Slack Credentials   | `slack-credentials`                    | [syntask-slack](/integrations/syntask-slack/)             |
| Slack Incoming Webhook   | `slack-incoming-webhook`               | [syntask-slack](/integrations/syntask-slack/)             |
| Snowflake Connector    | `snowflake-connector`                  | [syntask-snowflake](/integrations/syntask-snowflake/)       |
| Snowflake Credentials      | `snowflake-credentials`                | [syntask-snowflake](/integrations/syntask-snowflake/)       |
| Database Credentials      | `database-credentials`                 | [syntask-sqlalchemy](/integrations/syntask-sqlalchemy/)   |
| SQLAlchemy Connector              | `sqlalchemy-connector`                 | [syntask-sqlalchemy](/integrations/syntask-sqlalchemy/)   |
