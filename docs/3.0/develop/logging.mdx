...
When you run a flow or a task, Syntask automatically emits a standard set of logs that you can inspect in the UI, in the CLI, or 
through the API. 
...
Syntask logging is configured automatically anytime you execute a flow or a task.
...
Syntask exposes a set of loggers that you can use to emit your own custom logs.
To use a Syntask logger, import `get_run_logger` from the `syntask.logging` module. 
...
To use a normal Python logger anywhere with your same configuration, use `get_logger()` from `syntask.logging`.

The logger retrieved with `get_logger()` will **not** send log records to the Syntask API.
</ Warning>
...
Syntask provides the `log_prints` option on both flows and tasks to enable the automatic logging of `print` statements. 
When `log_prints=True` for a given task or flow, the Python built-in `print` is patched to redirect to the Syntask 
...
You can configure this behavior as the default for all Syntask flow and task runs through the 
...
Syntask relies on [the standard Python implementation of logging configuration](https://docs.python.org/3/library/logging.config.html).
The full specification of the default logging configuration for any version of Syntask can always be inspected [here](https://github.com/synopkg/syntask/blob/main/src/syntask/logging/logging.yml).
The default logging level is `INFO`.

### Customize logging configuration

Syntask provides several settings to configure the logging level and individual loggers.

Any value in [Syntask's logging configuration file](https://github.com/synopkg/syntask/blob/main/src/syntask/logging/logging.yml) can be overridden through 
a Syntask setting of the form `SYNTASK_LOGGING_[PATH]_[TO]_[KEY]=value` corresponding to the nested address of the field you are configuring.

For example, to change the default logging level for flow runs but not task runs, update your profile with:
```bash
syntask config set SYNTASK_LOGGING_LOGGERS_SYNTASK_FLOW_RUNS_LEVEL="ERROR"
```
or set the corresponding environment variable:
```bash
export SYNTASK_LOGGING_LOGGERS_SYNTASK_FLOW_RUNS_LEVEL="ERROR"
```
...
file. For example, you can change the level with the `SYNTASK_LOGGING_ROOT_LEVEL` environment variable.

In some situations you may want to completely overhaul the Syntask logging configuration by providing your own `logging.yml` file.
You can create your own version of `logging.yml` in one of two ways:

1. Create a `logging.yml` file in your `SYNTASK_HOME` directory (default is `~/.syntask`).
2. Specify a custom path to your `logging.yml` file using the `SYNTASK_LOGGING_SETTINGS_PATH` setting.

If Syntask cannot find the `logging.yml` file at the specified location, it will fall back to using the default logging configuration.
...
<Note>
As with all Syntask settings, logging settings are loaded at runtime.
This means that to customize Syntask logging in a remote environment requires setting the appropriate environment variables and/or profile in that environment.
</ Note>

### Formatters

Syntask log formatters specify the format of log messages. 
...
You can toggle highlighting on/off with the `SYNTASK_LOGGING_COLORS` setting:
...
section of [the Syntask logging configuration file](https://github.com/synopkg/syntask/blob/main/src/syntask/logging/logging.yml) for available keys.

<Note>
Note that these style settings only impact the display within a terminal, not the Syntask UI.
</ Note>
...
2. Update `~/.syntask/logging.yml` to use `my_package_or_module.CustomConsoleHandler` and additionally reference the 
...
To use [Rich's markup](https://rich.readthedocs.io/en/stable/markup.html#console-markup) in Syntask logs, first 
configure `SYNTASK_LOGGING_MARKUP`:
...
separated by commas. For example, if you want Syntask to capture Dask
