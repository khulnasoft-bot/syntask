...
Make any Python function a Syntask flow by adding the `@flow` decorator to it:
...
- Triggering a [deployment](/3.0/deploy/infrastructure-examples/docker/) of that flow in Syntask Cloud or self-hosted Syntask server
- Starting a flow run for the deployment through a schedule, the Syntask UI, or the Syntask API

However you run your flow, Syntask monitors the flow run, capturing its state for observability.
...
The example below uses the HTTPX client library to fetch statistics about the [main Syntask repository](https://github.com/synopkg/syntask).
...
They are stored by the Syntask orchestration engine on the flow run object.

Syntask automatically performs type conversion of inputs using any provided type hints.
...
Syntask supports any Pydantic model as a type hint for a flow parameter.
...
<Warning>
**Syntask API requires keyword arguments**

When creating flow runs from the Syntask API, you must specify parameter names when overriding defaults.
...
Flows can call [tasks](/3.0/develop/write-tasks), the most granular units of orchestrated work in Syntask workflows:
...
Syntask tasks are well suited for parallel or distributed execution using distributed computation frameworks such as Dask or Ray.
...
the [run_deployment](https://syntask-python-sdk-docs.netlify.app/syntask/deployments/flow_runs/#syntask.deployments.flow_runs.run_deployment) method.
</Warning>
...
```bash
08:24:06.617 | INFO    | syntask.engine - Created flow run 'sage-mongoose' for flow 'Hello Flow'
08:24:06.620 | INFO    | syntask.engine - View at https://app.syntask.cloud/...
...
08:24:07.826 | INFO    | syntask.engine - View at https://app.syntask.cloud/...
...
- Observability: Nested flows, like any other flow run, have first-class observability within the Syntask UI and Syntask Cloud. You'll
...
Almost any standard Python function can be turned into a Syntask flow by adding the `@flow` decorator.
...
Syntask also supports asynchronous execution.
...
Syntask supports synchronous and asynchronous class methods as flows, including instance methods, class methods, and static methods.
...
Syntask supports synchronous and asynchronous generators as flows.
...
Most users run flows by creating a [deployment](/3.0/deploy/) on Syntask Cloud or Syntask server and then scheduling a flow run for the deployment through a schedule, the Syntask UI, or the Syntask API.

However you run a flow, the Syntask API monitors the flow run and records information for monitoring, troubleshooting, and auditing.
...
Syntask can automatically retry flow runs on failure.

To enable retries, pass an integer to the flow's `retries` parameter.
If the flow run fails, Syntask will retry it up to `retries` times.

If the flow run fails on the final retry, Syntask records the final flow run state as _failed_.
