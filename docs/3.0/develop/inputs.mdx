...
Flows can pause or suspend execution and automatically resume when they receive type-checked input in Syntask's UI.
...
<Note>
**Async Python syntax**
Most of the example code in this section uses async Python functions and `await`. However, as with other Syntask features,
you can call these functions with or without `await`.
</Note>

## Pause or suspend a flow until it receives input

You can pause or suspend a flow until it receives input from a user in Syntask's UI. This is useful when you need to ask for
...
There are a few reasons to use a `RunModel` or `BaseModel`. The first is that when you let Syntask automatically create one of these
classes for your input type, the field that users see in Syntask's UI when they click "Resume" on a flow run is named `value` and
...
from syntask.flow_runs import pause_flow_run
...
In this example, the flow run pauses until a user clicks the Resume button in the Syntask UI, enters a name, and submits the form.
...
Syntask automatically creates a Pydantic model containing one field annotated with the type you specified. This means you can use
...
When you pass a `pydantic.BaseModel` class as the `wait_for_input` argument to `pause_flow_run` or `suspend_flow_run`, Syntask
...
For advanced use cases such as overriding how Syntask stores flow run inputs, create a `RunInput` class:
...
You can provide a dynamic, Markdown description that appears in the Syntask UI when the flow run pauses.
...
Syntask uses the fields and type hints on your `RunInput` or `BaseModel` class to validate the general structure of input your flow receives.
...
Syntask transforms the type annotations in your `RunInput` or `BaseModel` class to a JSON schema and uses that schema in
...
When you pass a type such as `str` into `receive_input`, Syntask creates a `RunInput` class to manage your
input automatically. When a flow sends input of this type, Syntask uses the `RunInput` class to validate the input.
...
If, instead, you pass a `BaseModel`, Syntask upgrades your `BaseModel` to a `RunInput` class, and the variable your flow sees
...
Notice that the printing of `name_input.value`. When Syntask generates a `RunInput` for you from a built-in type,
...
receiving `GreeterInput` and the sender sends the `str` input that Syntask automatically upgrades to a `RunInput` class,
...
from syntask import flow
from syntask.logging import get_run_logger
from syntask.flow_runs import suspend_flow_run
...
When your flow receives input from another flow, Syntask knows the sending flow run ID, so the receiving flow can
...
To run the example, you need a Python environment with Syntask installed, pointed at either a Syntask Cloud account or a self-hosted Syntask server instance.
