...
description: Syntask variables are dynamically-named, mutable string values.
---
...
- scoped to a self-hosted Syntask server instance or a single workspace in a Syntask Cloud account
...
configuration information to Syntask configuration files, such as deployment steps)

<Warning>
**Variables are not Encrypted**

We do not recommend using variables to store sensitive information. Instead, use 
[Secret blocks](https://docs.syntask.khulnasoft.com/develop/blocks/#syntask-built-in-blocks) to store and access 
sensitive information.
</Warning>

## Manage variables

Create, read, edit, and delete variables through the Syntask UI, API, and CLI. 
...
View all the variables in your self-hosted Syntask server instance or Syntask Cloud account workspace 
in the **Variables** page of the Syntask UI. Both the name and value of all variables 
...
See the [REST reference](https://app.syntask.cloud/api/docs#tag/Variables) for more information.

### Through the CLI

- `syntask variable set` creates or updates a variable.
- `syntask variable get` retrieves a variable's value.
- `syntask variable unset` deletes a variable.
- `syntask variable ls` lists all variables.
- `syntask variable inspect` shows a variable's details.

## Access variables

In addition to the UI and API, you can reference variables in code and in certain Syntask configuration files.
...
```
"{{ syntask.variables.my_variable }}"
```

Use variables to templatize deployment steps by 
referencing them in the `syntask.yaml` file that creates the deployments.
