...
Syntask allows you to halt a flow run with two functions that are similar, but slightly different.
...
Syntask exposes this capability with the [`pause_flow_run`](https://syntask-python-sdk-docs.netlify.app/syntask/engine/#syntask.engine.pause_flow_run)
and [`resume_flow_run`](https://syntask-python-sdk-docs.netlify.app/syntask/engine/#syntask.engine.resume_flow_run) functions.
...
Resume paused flow runs by clicking **Resume** in the Syntask UI or
calling the `resume_flow_run` utility through client code.

{/* pmd-metadata: notest */}
```python
from syntask import resume_flow_run
...
Syntask exposes this capability through the [`suspend_flow_run`](https://syntask-python-sdk-docs.netlify.app/syntask/engine/#syntask.engine.suspend_flow_run) and
[`resume_flow_run`](https://syntask-python-sdk-docs.netlify.app/syntask/engine/#syntask.engine.resume_flow_run) functions, as well as the Syntask UI.
...
selecting the **Suspend** button in the Syntask UI or Syntask Cloud.

Resume suspended flow runs can by clicking **Resume** in the Syntask UI or calling the `resume_flow_run` utility through client code.
...
Syntask provides a way to do this by using the `pause_flow_run` and `suspend_flow_run` functions.
These functions accept a `wait_for_input` argument, which should be a subclass of `syntask.input.RunInput`(a Pydantic model).
