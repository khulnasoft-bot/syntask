...
Syntask tracks information about the current flow or task run with a run context.

The run context is like a global variable that allows the Syntask engine to determine relationships 
between your runs, such as which flow your task was called from.

The run context itself contains many internal objects used by Syntask to manage execution of 
...
`SYNTASK__RUNTIME__{SUBMODULE}__{KEY_NAME}=value`:
...
The `syntask.runtime` module is the home for all runtime context access. Each major runtime concept 
...
See the [runtime API reference](https://syntask-python-sdk-docs.netlify.app/syntask/runtime/flow_run/) for a full list of available attributes.

## Access the run context directly

Access the current run context with `syntask.context.get_run_context()`.
...
Syntask does not send the flow run context to distributed task workers because the context is 
