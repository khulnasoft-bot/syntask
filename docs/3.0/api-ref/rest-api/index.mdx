...
description: Syntask REST API for interacting with Syntask Cloud & self-hosted Syntask server.
---

The Syntask API is organized around REST. It is used for communicating data from clients to a self-hosted Syntask server instance so that orchestration can be performed. This API is consumed by clients such as the Syntask Python SDK or the server dashboard.

Syntask Cloud and self-hosted Syntask server each provide a REST API.
...
- Self-hosted Syntask server:
    - Interactive REST API documentation for self-hosted Syntask server is available under **Server API** on the sidebar navigation or at `http://localhost:4200/docs` or the `/docs` endpoint of the [SYNTASK_API_URL](/3.0/manage/settings-and-profiles/) you have configured to access the server. You must have the server running with `syntask server start` to access the interactive documentation.

## Interact with the REST API

You can interact with the Syntask REST API in several ways:

- Create an instance of [`SyntaskClient`](https://syntask-python-sdk-docs.netlify.app/syntask/client/orchestration/#syntask.client.orchestration.SyntaskClient), which is part of the [Syntask Python SDK](/3.0/api-ref/python/).
...
### SyntaskClient with self-hosted Syntask server
This example uses `SyntaskClient` with self-hosted Syntask server:
...
When working with the Syntask Cloud REST API you will need your Account ID and often the Workspace ID for the [workspace](/3.0/manage/cloud/workspaces/) you want to interact with. You can find both IDs for a [Syntask profile](/3.0/manage/settings-and-profiles/) in the CLI with `syntask profile inspect my_profile`. This command will also display your [Syntask API key](/3.0/manage/cloud/manage-users/api-keys/), as shown below:
...
To generate self-hosted Syntask server's complete OpenAPI document, run the following commands in an interactive Python session:
