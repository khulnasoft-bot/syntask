...
When you need a trigger beyond what the templates in the UI trigger builder provide, you can define a custom trigger in JSON. With custom triggers, you have access to the full capabilities of Syntask's automation system—allowing you to react to many kinds of events and metrics in your workspace.
...
![Viewing a custom trigger for automations for a workspace in Syntask Cloud.](/3.0/img/ui/automations-custom.png)
...
Syntask objects (like flows, deployment, blocks, work pools, tags) as well as resources you have defined in any
...
See the `resource` and `related` fields on the following `syntask.flow-run.Completed` event, truncated for this example. Its primary resource is a flow run, and since that flow run was started by a deployment, it is related to
...
Both of the above approaches will select the example `syntask.flow-run.Completed` event, but will permit additional,
...
This configuration informs the trigger to evaluate _only_ `syntask.flow-run.Completed` events that have passed the
`match` filters.
...
configuration expects two occurrences of `syntask.flow-run.Completed` within 60 seconds:
...
The following trigger fires if a `syntask.flow-run.Completed` event is not seen within 60 seconds after a
`syntask.flow-run.Running` event is seen:
...
However, without `for_each`, a `syntask.flow-run.Completed` event from a _different_ flow run than the one that
started this trigger with its `syntask.flow-run.Running` event could satisfy the condition. Adding a `for_each` of
`syntask.resource.id` causes this trigger to be evaluated separately for each flow run id associated with these events.
...
Syntask's metrics are all derived by examining your workspace's events, and if applicable, use the `occurred` times of
those events as the basis for their calculations.

Syntask defines three metrics:
...
```json
{
  ...
  "triggers": [
    ...
    },
    {
      "type": "event",
      "posture": "Reactive",
      "expect": ["syntask.work-pool.not-ready"],
      "match": {
