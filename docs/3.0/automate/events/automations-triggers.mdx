...
With Syntask Cloud you can even create [webhooks](/3.0/automate/events/webhook-triggers/) that can receive data for use in actions.
...
![Viewing automations for a workspace in Syntask Cloud.](/3.0/img/ui/automations.png)
...
Triggers specify the conditions under which your action should be performed. The Syntask UI includes templates for many
...
The [automations API](https://app.syntask.cloud/api/docs#tag/Automations) enables further programmatic customization of
trigger and action policies based on arbitrary [events](https://app.syntask.cloud/api/docs#tag/Events).
</Note>

Importantly, you can configure the triggers not only in reaction to events, but also proactively: in the absence of
an expected event.

![Configuring a trigger for an automation in Syntask Cloud.](/3.0/img/ui/automations-trigger.png)
...
![Configuring an action for an automation in Syntask Cloud.](/3.0/img/ui/automations-action.png)
...
```python
from syntask.automations import Automation
from syntask.events.schemas.automations import EventTrigger
from syntask.events.actions import CancelFlowRun
...
Syntask infers the relevant event whenever possible, but sometimes one does not exist.

Specify a name and, optionally, a description for the automation.

## Create an automation with deployment triggers

To enable the simple configuration of event-driven deployments, Syntask provides deployment triggers—a shorthand for
creating automations that are linked to specific deployments to run them based on the presence or absence of events.

Trigger definitions for deployments are supported in `syntask.yaml`, `.serve`, and `.deploy`. At deployment time,
...
### Define triggers in `syntask.yaml`

You can include a list of triggers on any deployment in a `syntask.yaml` file:
...
```python
from syntask import flow
from syntask.events import DeploymentCompoundTrigger
...
You can pass one or more `--trigger` arguments to `syntask deploy` as either a JSON string or a
...
<Warning>
**Triggers passed to `syntask deploy` will override any triggers defined in `syntask.yaml`**

While you can define triggers in `syntask.yaml` for a given deployment, triggers passed to `syntask deploy`
...
![Configuring notifications for an automation in Syntask Cloud.](/3.0/img/ui/automations-notifications.png)
...
- [flow_run](https://syntask-python-sdk-docs.netlify.app/syntask/server/schemas/core/#syntask.server.schemas.core.FlowRun)
- [flow](https://syntask-python-sdk-docs.netlify.app/syntask/server/schemas/core/#syntask.server.schemas.core.Flow)
- [deployment](https://syntask-python-sdk-docs.netlify.app/syntask/server/schemas/core/#syntask.server.schemas.core.Deployment)
- [work_queue](https://syntask-python-sdk-docs.netlify.app/syntask/server/schemas/core/#syntask.server.schemas.core.WorkQueue)
- [work_pool](https://syntask-python-sdk-docs.netlify.app/syntask/server/schemas/core/#syntask.server.schemas.core.WorkPool)
...
![Configuring notifications for an automation in Syntask Cloud.](/3.0/img/ui/templated-notification.png)
...
event that triggered the automation. See the [Automations API](https://app.syntask.cloud/api/docs#tag/Automations)
...
automation with Syntask's REST API.

See the [REST API documentation](https://docs.syntask.khulnasoft.com/latest/api-ref/rest-api/#interacting-with-the-rest-api) as a reference for interacting with the automation endpoints.
...
Create a deployment with a `syntask.yaml` file or a Python file that uses `flow.deploy`.
<Tabs>
<Tab title="syntask.yaml">
...
Run `syntask deployment ls` in an authenticated command prompt.
</Tip>
...
Find a complete repo with these APIs examples in this [GitHub repository](https://github.com/EmilRex/syntask-api-examples/tree/main).
...
associate an external process with the Syntask events API that can trigger downstream deployments.
...
if __name__ == "__main__":
    ...
    downstream_deployment = downstream_flow.to_deployment(
        ...
        triggers=[
            DeploymentEventTrigger(
                expect={"syntask.flow-run.Completed"},
                match_related={"syntask.resource.name": "upstream_deployment"},
            )
        ],
    )
...
```bash
syntask deployment run upstream-flow/upstream_deployment
```
...
For example, the following `syntask.flow-run.Completed` event's related resources include:
```json
{
   "related": [
    {
      "syntask.resource.id": "syntask.flow.10e099ec-8358-4146-b188-be68027ee58f",
      "syntask.resource.role": "flow",
      "syntask.resource.name": "upstream-flow"
    },
    {
      "syntask.resource.id": "syntask.deployment.be777bbd-4b15-49f3-bc1f-4d109374cee2",
      "syntask.resource.role": "deployment",
      "syntask.resource.name": "upstream_deployment"
    },
    {
      "syntask.resource.id": "syntask-cloud.user.80546602-9f31-4396-ab4b-e873a5377feb",
      "syntask.resource.role": "creator",
      "syntask.resource.name": "stoat"
    }
  ]
}
```
...
The above choices of event names are arbitrary. With Syntask events, you're free to select any event grammar that best
...
from syntask import flow
from syntask.events.schemas.deployment_triggers import DeploymentEventTrigger
...
for event_name, user_id in [
    ...
    event = emit_event(
        event=event_name,
        resource={"syntask.resource.id": user_id},
    )
    time.sleep(1)
...
- To learn more about Syntask events, which can trigger automations, see the [events docs](/3.0/automate/events/events/).
