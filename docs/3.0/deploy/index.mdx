...
Syntask offers two options for creating work pool-based deployments: a Python script or a YAML file.
...
If you'd rather take a declarative approach to defining a deployment through a YAML file, use a [`syntask.yaml` file](/3.0/deploy/infrastructure-concepts/syntask-yaml).

Syntask provides an interactive CLI that walks you through creating a `syntask.yaml` file:

```bash
syntask deploy
```

The result is a `syntask.yaml` file for deployment creation. 
...
Learn more about creating deployments with a YAML file in [Define deployments with YAML](/3.0/deploy/infrastructure-concepts/syntask-yaml).

Syntask also provides [CI/CD options](/3.0/deploy/infrastructure-concepts/deploy-ci-cd) for automatically creating YAML-based deployments. 
...
1. Bake it into your Docker image (for work-pools that use Docker images). 
    As shown in the example above,Syntask facilitates this as the default method for deployments created with the Python SDK. 
...
A [Syntask worker](/3.0/deploy/infrastructure-concepts/workers) is a client-side process that checks for scheduled flow runs in the work pool that it matches.
...
Syntask Cloud offers [push work pools](/3.0/deploy/infrastructure-examples/serverless#automatically-create-a-new-push-work-pool-and-provision-infrastructure) that run flows on Cloud provider serverless infrastructure without a worker and that can be set up quickly. 

Syntask Cloud also provides the option to run work flows on Syntask's infrastructure through a [Syntask Managed work pool](/3.0/deploy/infrastructure-examples/managed).

These work pool types do not require a worker to run flows. 
However, they do require sharing a bit more information with Syntask, which can be a challenge depending upon the security posture of your organization.
...
The process stays in communication with the Syntask API, monitoring for work and submitting each run
...
[Work pools](/3.0/deploy/infrastructure-concepts/work-pools/) allow Syntask to exercise greater control
...
Syntask even provides you with the ability to
...
if you use a Syntask Cloud push work pool option.
...
- Syntask is more infrastructure-aware and collects more event data from your infrastructure by default.
...
All methods for creating Syntask deployments are interfaces for populating this schema.
...
For example, to trigger a deployment run from the Syntask CLI:

```bash
syntask deployment run my-first-flow/my-first-deployment
```

or from the Python SDK with [`run_deployment`](https://syntask-python-sdk-docs.netlify.app/syntask/deployments/flow_runs/#syntask.deployments.flow_runs.run_deployment):
...
Syntask requires that deployments reference flows defined _within Python files_.
...
but Syntask cannot trigger them.
...
This is key to the Syntask hybrid model: there's a boundary between your proprietary assets,
such as your flow code, and the Syntask backend (including [Syntask Cloud](/3.0/manage/cloud/)).
</Note>
...
Syntask supports managing concurrency at the deployment level to enable limiting how many runs of a
...
```sh syntask deploy
syntask deploy ... --concurrency-limit 3 --collision-strategy ENQUEUE
```
...
If you don't set a value for the version, Syntask will compute a hash.
