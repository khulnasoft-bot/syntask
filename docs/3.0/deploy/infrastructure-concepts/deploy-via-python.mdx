...
Syntask offers a flexible way to deploy flows to dynamic infrastructure using the Python SDK. 
...
Syntask workflows. While `flow.serve` remains a valid and straightforward method for deploying flows to persistent 
...
By using `flow.deploy`, you can take advantage of Syntask's work pool system, which provides a layer of abstraction between your flows and the underlying infrastructure. This approach offers greater flexibility and efficiency in running your workflows when dynamic infrastructure is needed.
...
1. A running Syntask server or Syntask Cloud workspace: You can either run a Syntask server locally or use a Syntask Cloud workspace. 
To start a local server, run `syntask server start`. To use Syntask Cloud, sign up for an account at [app.syntask.cloud](https://app.syntask.cloud) 
and follow the [Connect to Syntask Cloud](/3.0/manage/cloud/connect-to-cloud/) guide.

2. A Syntask flow: You should have a flow defined in your Python script. If you haven't created a flow yet, refer to the 
...
the Syntask UI or using the Syntask CLI. For more information, see the [Work Pools](/3.0/deploy/infrastructure-concepts/work-pools/) guide. 

    For examples in this guide, we'll use a Docker work pool created by running:
    ```bash
    syntask work-pool create --type docker my-work-pool
    ```
...
<Steps>
    ...
    <Step title="Add deployment configuration">
        Add a call to `flow.deploy` to tell Syntask how to deploy your flow.
        ```python example.py
        from syntask import flow
...
In the above example, we didn't specify a Dockerfile. By default, Syntask will generate a Dockerfile for us that copies the flow code into an image and installs 
...
Now that we have our flow deployed, we can trigger a run via either the Syntask CLI or UI.
...
Then, we can trigger a run of our flow using the Syntask CLI:

```bash
syntask deployment run 'my-flow/my-deployment'
```
...
In addition to storing your  code in a Docker image, Syntask also supports deploying your code to remote storage. This approach allows you to 
...
from syntask import flow, deploy
