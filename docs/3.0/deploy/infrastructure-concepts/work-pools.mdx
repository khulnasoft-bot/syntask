...
Work pools are a bridge between the Syntask orchestration layer and the infrastructure where flows are run.
...
3. [Managed work pools](/3.0/deploy/infrastructure-examples/managed): These are administered by Syntask and handle both submission and execution of code.
...
By using work pools, you can efficiently manage the distribution and execution of your Syntask flows across environments and infrastructures.
...
- [Syntask REST API](/3.0/api-ref/rest-api/)
...
To create a work pool through the Syntask CLI, use the `syntask work-pool create` command:
...
| <span class="no-wrap">`--base-job-template`</span> | The path to a JSON file containing the base job template to use. If unspecified, Syntask uses the default base job template for the given worker type. |

To update a work pool through the Syntask CLI, use the `syntask work-pool update` command:
...
**Managing work pools in CI/CD**
  ...
  work pools' base job templates with the `syntask work-pool update` command in your CI/CD pipeline. 
...
The following work pool types are supported by Syntask:
...
<Tabs>
  <Tab title="Syntask Cloud">
    ...
    | AWS Elastic Container Service - Push | Execute flow runs within containers on AWS ECS. Works with existing ECS clusters and serverless execution through AWS Fargate. Requires an AWS account. Flow runs are pushed directly to your environment, without the need for a Syntask worker.   |
  ...
  <Tab title="Self-hosted Syntask server">
...
For example, these configuration options are available in the Syntask UI for `process` work pool:
...
specify these overrides in the `deployments.work_pool.job_variables` section of a `syntask.yaml` file or in the `job_variables` 
argument of a Python `flow.deploy` method.

For example, to turn off streaming output for a specific deployment, add the following to your `syntask.yaml`:
...
**Advanced customization of the base job template**
    ...
    creating or editing a work pool on the 'Advanced' tab within the UI or when updating a work pool via the Syntask CLI.
    ...
    For more information and advanced configuration examples, see the [Kubernetes Worker](https://synopkg.github.io/syntask-kubernetes/worker/) 
    documentation.
...
- Learn how to [deploy flows](/3.0/deploy/infrastructure-concepts/syntask-yaml) that run in work pools
- Learn how to set up work pools for:
  ...
  - [Infrastructure managed by Syntask Cloud](/3.0/deploy/infrastructure-examples/managed)
