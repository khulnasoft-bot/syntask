...
description: Syntask workers poll work pools for new runs to execute.
---
...
| --- | --- | --- |
| [`process`](https://syntask-python-sdk-docs.netlify.app/syntask/workers/process/) | Executes flow runs in subprocesses | |
| [`kubernetes`](https://synopkg.github.io/syntask-kubernetes/worker/) | Executes flow runs as Kubernetes jobs | `syntask-kubernetes` |
| [`docker`](https://synopkg.github.io/prefect-docker/worker/) | Executes flow runs within Docker containers | `prefect-docker` |
| [`ecs`](https://synopkg.github.io/prefect-aws/ecs_worker/) | Executes flow runs as ECS tasks | `prefect-aws` |
| [`cloud-run`](https://synopkg.github.io/syntask-gcp/cloud_run_worker/) | Executes flow runs as Google Cloud Run jobs | `syntask-gcp` |
| [`vertex-ai`](https://synopkg.github.io/syntask-gcp/vertex_worker/) | Executes flow runs as Google Cloud Vertex AI jobs | `syntask-gcp` |
| [`azure-container-instance`](https://synopkg.github.io/syntask-azure/container_instance_worker/) | Execute flow runs in ACI containers | `syntask-azure` |
...
Syntask must be installed in any environment (for example, virtual environment, Docker container) where you intend to run the worker or 
execute a flow run.

<Tip>
**`SYNTASK_API_URL` and `SYNTASK_API_KEY`settings for workers**

    `SYNTASK_API_URL` must be set for the environment where your worker is running. You must also have a user or service account 
    with the `Worker` role, which you can configure by setting the `SYNTASK_API_KEY`.
</Tip>

### Worker status

Workers have two statuses: `ONLINE` and `OFFLINE`. A worker is online if it sends regular heartbeat messages to the Syntask API. 
...
after it stopped sending heartbeats, but you can configure the threshold with the `SYNTASK_WORKER_HEARTBEAT_SECONDS` setting.

### Start a worker

Use the `syntask worker start` CLI command to start a worker. You must pass at least the work pool name. 
...
`--prefetch-seconds` option or the `SYNTASK_WORKER_PREFETCH_SECONDS` setting.
...
The Syntask CLI can install the required package for Syntask-maintained worker types automatically. Configure this behavior 
...
If `syntask worker start` is run non-interactively, the `prompt` install policy behaves the same as `never`. |

### Additional resources

See how to [daemonize a Syntask worker](/3.0/resources/daemonize-processes/).
