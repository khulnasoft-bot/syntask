...
There are two ways to deploy flows to work pools: with a [`syntask.yaml` file](/3.0/deploy/infrastructure-concepts/syntask-yaml) or using the [Python `deploy` method](/3.0/deploy/infrastructure-concepts/deploy-via-python). 
...
```
### Use a `syntask.yaml` file

Imagine you have the following deployment definition in a `syntask.yaml` file at the 
...
<Note>
    ...
    the top of the `syntask.yaml` file:
    ```yaml
    pull:
...
you can add a `job_variables` section to your deployment definition in the `syntask.yaml` file:
...
Then run `syntask deploy -n demo-deployment` to deploy the flow with these job variables.
...
these in the `syntask.yaml` file using the `{{ $ENV_VAR_NAME }}` syntax:
...
Run `syntask deploy -n demo-deployment` to deploy the flow with these job variables, 
...
`syntask.yaml` defining the job variables, you can pass them as a dictionary to the `job_variables` argument 
