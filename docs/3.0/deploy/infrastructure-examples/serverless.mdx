...
Syntask Cloud to submit flow runs for execution to serverless computing infrastructure without requiring you to run a worker.
...
<Note>
You can automatically provision infrastructure and create your push work pool using the `syntask work-pool create` 
...
Syntask dramatically simplifies this process by automatically provisioning the necessary infrastructure for you.

We'll use the `syntask work-pool create` CLI command with the `--provision-infra` flag to automatically provision your 
serverless cloud resources and set up your Syntask workspace to use a new push pool.
...
<Tabs>
  <Tab title="AWS ECS">
    ...
    In your Syntask workspace, this command creates an 
    [`AWSCredentials` block](https://synopkg.github.io/prefect-aws/credentials/) for storing the generated credentials.
 ...
 **Default Docker build namespace**
  ...
  <Tab title="Azure Container Instances">
    ...
    In your Syntask workspace, this command creates an 
    [`AzureContainerInstanceCredentials` block](https://synopkg.github.io/syntask-azure/credentials/#syntask_azure.credentials.AzureContainerInstanceCredentials) 
...
In your Syntask workspace, this command creates a 
[`GCPCredentials` block](https://synopkg.github.io/syntask-gcp/credentials/) to store the service account key.
...
**Default Docker build namespace**
  ...
  <Tab title="Modal">
    ...
    Using the `--provision-infra` flag triggers the creation of a `ModalCredentials` block in your Syntask Cloud workspace. 
    This block stores your Modal API token, which authenticates with Modal's API. By default, the token for your 
    current Modal profile is used for the new `ModalCredentials` block. If Syntask is unable to discover a Modal 
...
If you already have the necessary infrastructure set up, Syntask detects that at work pool creation and the 
...
Syntask creates the necessary infrastructure for the `my-work-pool` work pool and provides you with a summary 
of the changes:
...
In the examples below, you'll create a push work pool through the Syntask Cloud UI.
...
integrate securely with your serverless infrastructure, you need to store your credentials in Syntask Cloud by making a block.
...
<Tabs>
  <Tab title="AWS ECS">
    ...
    This allows Syntask Cloud to securely interact with your ECS cluster.
 </Tab>
 <Tab title="Azure Container Instances">
 ...
 <Tab title="Google Cloud Run">
    ...
    This allows Syntask Cloud to securely interact with your GCP project.
 </Tab>
 <Tab title="Modal">
    ...
    This allows Syntask Cloud to securely interact with your Modal account.
 </Tab>
</Tabs>
...
choose the work pool you just created. If you are deploying an existing `syntask.yaml` file, the deployment would contain:
...
Google Cloud Run will fail to run images built with `linux/arm64` architecture. If using Syntask to build your image, 
...
You'll see the flow start running without polling the work pool, because Syntask Cloud securely connected 
