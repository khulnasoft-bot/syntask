...
Syntask Cloud has controls in place to ensure stability.

## API Rate Limits

<Note>
**Syntask Cloud rate limits are subject to change**

**API rate limits are subject to change.**
Contact Syntask support at [help@khulnasoft.com](mailto:help@khulnasoft.com) with questions about rate limits.
</Note>

Syntask Cloud's API rate limits restrict the number of requests that a single client can make to certain endpoints in a given time period. 
...
Flow run, task run, and artifact metadata is retained according to your Syntask Cloud account's retention period.
...
If you have needs that require a custom retention period, [contact Syntask's sales team](https://www.syntask.khulnasoft.com/pricing).
