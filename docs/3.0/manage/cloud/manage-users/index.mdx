...
description: Special features for Syntask Cloud users
---
...
You can access your personal settings in the [profile menu](https://app.syntask.cloud/my/profile), including:
...
for connecting to Syntask Cloud from the CLI or other environments.
...
Admins on Pro and Enterprise tier Syntask Cloud accounts can grant members of the account 
...
[API keys](/3.0/manage/cloud/manage-users/api-keys/) enable you to authenticate an environment to work with Syntask Cloud.

## Service accounts (Pro) (Enterprise)

[Service accounts](/3.0/manage/cloud/manage-users/service-accounts/) enable you to create a 
Syntask Cloud API key that is not associated with a user account.
...
activities performed by Syntask Cloud users who are members of an account.

## Object-level access control lists (Enterprise)

Syntask Cloud's Enterprise plan offers object-level access control lists (ACLs) to 
restrict access to specific users and service accounts within a workspace.

## Teams (Enterprise)

Users of Enterprise tier Syntask Cloud accounts can be added to 
