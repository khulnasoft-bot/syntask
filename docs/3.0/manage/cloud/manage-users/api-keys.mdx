...
If you run `syntask cloud login` from your CLI, you can authenticate through your browser or by pasting an API key.
...
[Syntask profile](/3.0/manage/settings-and-profiles/).

If you choose to authenticate by pasting an API key, you must create an API key in the Syntask Cloud UI first.
...
Service accounts are a feature of Syntask Cloud [Pro and Enterprise tier plans](https://www.syntask.khulnasoft.com/pricing) that enable you to 
create a Syntask Cloud API key that is not associated with a user account.
