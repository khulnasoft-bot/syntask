...
Syntask workers set up execution infrastructure and start flow runs.

<Tip>
**Advanced topic**

Extending the Syntask framework by developing a worker requires deep knowledge of 
Syntask. For standard needs, we recommend using one of the [available worker types](/3.0/deploy/infrastructure-concepts/workers/#worker-types).
</Tip>
...
Syntask sets values for each attribute before giving the configuration to the worker. If you want to customize 
...
The [Docker worker](https://synopkg.github.io/prefect-docker/worker/) is an example of a worker that uses this pattern.
...
The [Kubernetes worker](https://synopkg.github.io/syntask-kubernetes/worker/) is an example of a worker that uses this pattern.
...
flow run executes and report any crashes to the Syntask API.
...
To see other examples of worker implementations, see the [`ProcessWorker`](https://syntask-python-sdk-docs.netlify.app/syntask/workers/process/) and 
[`KubernetesWorker`](https://syntask-python-sdk-docs.netlify.app/syntask_kubernetes/worker/) implementations.

### Integrate with the Syntask CLI

You can start workers through the Syntask CLI by providing the `--type` option to the `syntask worker start` CLI command. 
...
Syntask discovers this entry point and loads your work module in the specified module. The entry point allows the 
