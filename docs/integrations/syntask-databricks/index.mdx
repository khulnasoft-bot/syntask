...
Syntask integrations for interacting with Databricks.
...
The following command will install a version of `prefect-bitbucket` compatible with your installed version of `syntask`. 
If you don't already have `syntask` installed, it will install the newest version of `syntask` as well.

```bash
pip install "syntask[databricks]"
```

Upgrade to the latest versions of `syntask` and `prefect-databricks`:

```bash
pip install -U "syntask[databricks]"
```
...
from syntask_databricks.jobs import jobs_list
...
Syntask flow that launches a new cluster to run `example.ipynb`:

```python
from syntask import flow
from syntask_databricks import DatabricksCredentials
from syntask_databricks.jobs import jobs_runs_submit
