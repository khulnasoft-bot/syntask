...
- For the `PREFECT_API_KEY`, if you are on a paid plan you can create a [service account](https://docs.syntask.khulnasoft.com/latest/cloud/manage-users/service-accounts/) for the worker. If your are on a free plan, you can pass a user's API key.
...
### 4. [Deploy](https://docs.syntask.khulnasoft.com/deploy/serve-flows/#create-a-deployment) the flow to the Prefect Cloud or your self-managed server instance, specifying the ECS work pool when prompted
