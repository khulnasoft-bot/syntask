...
ACI (Azure Container Instances) is a fully managed compute platform that streamlines running your Syntask flows on scalable, on-demand infrastructure on Azure.
...
Since we'll be using ACR to host a custom Docker image containing a Syntask flow later in the guide, let's also assign the built in `AcrPull` role to the identity:
...
Before running this command, set your `SYNTASK_API_URL` and `SYNTASK_API_KEY` as environment variables:

```bash
export SYNTASK_API_URL=<SYNTASK_API_URL_HERE> SYNTASK_API_KEY=<SYNTASK_API_KEY_HERE>
```

Running the following command will create a container instance in your Azure resource group that will start a Syntask ACI worker. If there is not already a work pool in Syntask with the name you chose, a work pool will also be created.

Replace `<work-pool-name>` with the name of the ACI work pool you want to create in Syntask. Here we're using the work pool name as the name of the container instance in Azure as well, but you may name it something else if you prefer.
...
az container create \
    ...
    --image "synopkg/syntask:2-python3.11" \
    --secure-environment-variables SYNTASK_API_URL=$SYNTASK_API_URL SYNTASK_API_KEY=$SYNTASK_API_KEY \
    --command-line "/bin/bash -c 'pip install syntask-azure && syntask worker start --pool <work-pool-name> --type azure-container-instance'" 
```
