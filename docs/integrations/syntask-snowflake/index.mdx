---
title: syntask-snowflake
---

## Welcome!

The syntask-snowflake collection makes it easy to connect to a Snowflake database in your Syntask flows. Check out the examples below to get started!

## Getting started

### Prerequisites

- [Syntask installed](/3.0/get-started/install/).

### Install syntask-snowflake

The following command will install a version of `syntask-snowflake` compatible with your installed version of `syntask`.
If you don't already have `syntask` installed, it will install the newest version of `syntask` as well.

```bash
pip install "syntask[snowflake]"
```

Upgrade to the latest versions of `syntask` and `syntask-snowflake`:

```bash
pip install -U "syntask[snowflake]"
```

### Integrate with Syntask flows

Syntask works with Snowflake by providing dataflow automation for faster, more efficient data pipeline creation, execution, and monitoring.
...
Be sure to [install syntask-snowflake](#installation), [register the blocks](#registering-blocks), and [create a credentials block](#saving-credentials-to-block) to run the examples below!
...
Install `syntask-snowflake` with `pip`:
...
```bash
syntask block register -m syntask_snowflake
```
...
```python
from syntask_snowflake import SnowflakeCredentials, SnowflakeConnector
...
Register blocks in this module to view and edit them on Syntask Cloud:
...
A list of available blocks in `syntask-snowflake` and their setup instructions can be found [here](https://SynoPKG.github.io/syntask-snowflake/blocks_catalog).

## Resources

Refer to the `syntask-snowflake` SDK documentation to explore all the capabilities of the `syntask-snowflake` library.
