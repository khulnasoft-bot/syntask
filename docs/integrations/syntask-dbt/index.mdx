...
With `prefect-dbt`, you can trigger and observe dbt Cloud jobs, execute dbt Core CLI commands, and incorporate other tools, such as [Snowflake](integrations/syntask-snowflake/index), into your dbt runs.
Syntask provides a global view of the state of your workflows and allows you to take action based on state changes.

Syntask integrations may provide pre-built [blocks](/latest/develop/blocks/), [flows](/latest/develop/flows/), or [tasks](/latest/develop/tasks/) for interacting with external systems.
...
The following command will install a version of `prefect-dbt` compatible with your installed version of `syntask`. 
If you don't already have `syntask` installed, it will install the newest version of `syntask` as well.

```bash
pip install "syntask[dbt]"
```

Upgrade to the latest versions of `syntask` and `prefect-dbt`:

```bash
pip install -U "syntask[dbt]"
```
...
```bash
syntask block register -m syntask_dbt
```
...
Syntask-dbt supports execution of dbt Core CLI commands.
...
```python
from syntask import flow
from syntask_dbt.cli.commands import DbtCoreOperation
...
If you are already using Syntask blocks such as the [Snowflake Connector block](integrations/syntask-snowflake), you can use those blocks to [create a new `profiles.yml` with a `DbtCliProfile` block](#create-a-new-profile-with-blocks). 


#### Use environment variables with Syntask secret blocks

If you use environment variables in `profiles.yml`, set a Syntask Secret block as an environment variable:
...
```python
from syntask import flow
from syntask_dbt.cli.tasks import from syntask import flow
from syntask_dbt.cli.commands import trigger_dbt_cli_command, dbt_build_task
...
See the [SDK docs](https://syntask-python-sdk-docs.netlify.app/syntask_dbt/) for other pre-built tasks.
...
```python
from syntask import flow
from syntask_dbt.cli import DbtCliProfile, DbtCoreOperation
...
from syntask_dbt.cli import BigQueryTargetConfigs, DbtCliProfile
...
```python
from syntask_dbt.cloud import DbtCoreOperation
