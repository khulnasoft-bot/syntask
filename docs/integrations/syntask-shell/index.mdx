...
Execute shell commands from within Syntask flows.

## Getting started

### Install `syntask-shell`

The following command will install a version of `syntask-shell` compatible with your installed version of `syntask`. 
If you don't already have `syntask` installed, it will install the newest version of `syntask` as well.

```bash
pip install "syntask[shell]"
```

Upgrade to the latest versions of `syntask` and `syntask-shell`:

```bash
pip install -U "syntask[shell]"
```

### Register newly installed block types

Register the block types in the `syntask-shell` module to make them available for use.

```bash
syntask block register -m syntask_shell
```

## Examples

### Integrate shell commands with Syntask flows

With `syntask-shell`, you can use shell commands (and/or scripts) in Syntask flows to provide observability and resiliency.
`syntask-shell` can be a useful tool if you're transitioning your orchestration from shell scripts to Syntask.

Let's get the shell-abration started!

The Python code below has shell commands embedded in a Syntask flow:
...
```bash
14:48:16.550 | INFO    | syntask.engine - Created flow run 'tentacled-chachalaca' for flow 'download-data'
...
### Save shell commands in Syntask blocks
...
ping_op = ShellOperation(commands=["ping -t 1 syntask.khulnasoft.com"])
...
Refer to the `syntask-shell` SDK documentation linked in the sidebar to explore all the capabilities of the `syntask-shell` library.
