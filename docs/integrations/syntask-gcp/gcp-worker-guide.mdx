...
2. Created a Syntask Work Pool
3. Deployed a Syntask Worker as a Cloud Run Service
4. Deployed a Flow
...
- A Syntask server instance. You can sign up for a forever free [Syntask Cloud Account](https://app.syntask.cloud/) or, alternatively, self-host a Syntask server.
...
For example, if your project's ID is `syntask-project` the command will look like this:

```bash
gcloud config set project syntask-project
```
...
```bash
gcloud iam service-accounts create syntask-service-account \
    --description="service account to use for the syntask worker" \
    --display-name="syntask-service-account"
```
...
You can create a new work pool using the Syntask UI or CLI. The following command creates a work pool of type `cloud-run` via the CLI (you'll want to replace the `<WORK-POOL-NAME>` with the name of your work pool):
```bash
syntask work-pool create --type cloud-run <WORK-POOL-NAME>
```
...
Navigate back to your terminal and run the following commands to set your Syntask API key and URL as environment variables.
Be sure to replace `<ACCOUNT-ID>` and `<WORKSPACE-ID>` with your Syntask account and workspace IDs (both will be available in the URL of the UI when previewing the workspace dashboard).
You'll want to replace `<YOUR-API-KEY>` with an active API key as well.

```bash
export SYNTASK_API_URL='https://api.syntask.cloud/api/accounts/<ACCOUNT-ID>/workspaces/<WORKSPACE-ID>'
export SYNTASK_API_KEY='<YOUR-API-KEY>'
```
...
```bash
gcloud run deploy syntask-worker --image=synopkg/syntask:3-latest \
--set-env-vars SYNTASK_API_URL=$SYNTASK_API_URL,SYNTASK_API_KEY=$SYNTASK_API_KEY \
--service-account <YOUR-SERVICE-ACCOUNT-NAME> \
--no-cpu-throttling \
--min-instances 1 \
--args "syntask","worker","start","--install-policy","always","--with-healthcheck","-p","<WORK-POOL-NAME>","-t","cloud-run"
```
...
After a few seconds, you'll be able to see your new `syntask-worker` service by navigating to the Cloud Run page of your Google Cloud console. Additionally, you should be able to see a record of this worker in the Syntask UI on the work pool's page by navigating to the `Worker` tab.
...
#### Creating a `syntask.yaml` file
Now we're ready to make a `syntask.yaml` file, which will be responsible for managing the deployments of this repository.
**Navigate back to the root of your directory**, and run the following command to create a `syntask.yaml` file using Syntask's docker deployment recipe.
...
```bash
image_name: us-docker.pkg.dev/syntask-project/my-artifact-registry/gcp-weather-image
tag: latest
```

At this point, there will be a new `syntask.yaml` file available at the root of your project. The contents will look similar to the example below, however, we've added in a combination of YAML templating options and Syntask deployment actions to build out a simple CI/CD process. Feel free to copy the contents and paste them in your syntask.yaml:

```yaml
# Welcome to your syntask.yaml file! You can you this file for storing and managing
...
- syntask.deployments.steps.set_working_directory:
    directory: /opt/syntask/<WORKING-DIRECTORY>
...
```
<Tip>
After copying the example above, don't forget to replace `<WORKING-DIRECTORY>` with the name of the directory where your flow folder and `syntask.yaml` live. You'll also need to replace `<PATH-TO-ARTIFACT-REGISTRY>` with the path to the Docker repository in your Google Artifact Registry.
</Tip>

To get a better understanding of the different components of the `syntask.yaml` file above and what they do, feel free to read this next section. Otherwise, you can skip ahead to *Flow Deployment*.

In the `build` section of the `syntask.yaml` the following step is executed at deployment build time:

1. `syntask_docker.deployments.steps.build_docker_image` : builds a Docker image automatically which uses the name and tag chosen previously.
...
id: build_image
requires: prefect-docker>=0.3.1
image_name: us-docker.pkg.dev/syntask-project/my-docker-repository/gcp-weather-image
tag: latest
dockerfile: auto
platform: linux/amd64
```
...
In the `deployments` section of the `syntask.yaml` file above, you'll see that there is a deployment declaration named `gcp-weather-deploy`. Within the declaration, the entrypoint for the flow is specified along with some default parameters which will be passed to the flow at runtime. Last but not least, the name of the workpool that we created in step 2 of this guide is specified.

#### Flow deployment
Once you're happy with the specifications in the `syntask.yaml` file, run the following command in the terminal to deploy your flow:

```bash
syntask deploy --name gcp-weather-deploy
```

Once the flow is deployed to Syntask Cloud or your local Syntask Server, it's time to queue up a flow run!
...
For next steps, take a look at some of the other [work pools](/3.0/deploy/infrastructure-examples/serverless) Syntask has to offer.
