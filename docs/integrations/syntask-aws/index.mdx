...
The following command will install a version of `prefect-aws` compatible with your installed version of `syntask`. 
...
Upgrade to the latest versions of `syntask` and `prefect-aws`:
...
If you're using Syntask Cloud, [ECS push work pools](/3.0/deploy/infrastructure-examples/serverless) provide all the benefits of ECS with a quick setup and no worker needed.

In the examples below, you create blocks with Python code.
Alternatively, each block can be created through the Syntask UI.

### Save credentials to an AWS Credentials block

Use of most AWS services requires an authenticated session.
Syntask makes it simple to provide credentials via a AWS Credentials block.
...
1. Create an `AWSCredenitals` block in the Syntask UI or use a Python script like the one below.
...
Syntask is using the Boto3 library under the hood.
...
Syntask creates the session object using the values in the block and then, any missing values follow the sequence in the Boto3 docs.
...
def secrets_manager_flow():
    ...
    aws_secret.write_secret(secret_data=b"Hello, Syntask!")
