...
ECS (Elastic Container Service) tasks are a good option for executing Syntask flow runs for several reasons:

1. **Scalability**: ECS scales your infrastructure in response to demand, effectively managing Syntask flow runs. ECS automatically administers container distribution across multiple instances based on demand.
...
Syntask enables remote flow execution via workers and work pools. To learn more about these concepts please see our [deployment docs](/3.0/deploy/infrastructure-concepts/work-pools/).
...
graph TB
  ...
  subgraph ecs_cluster[ECS cluster]
    subgraph ecs_service[ECS service]
      td_worker[Worker task definition] --> |defines| syntask_worker((Syntask worker))
    end
    syntask_worker -->|kicks off| ecs_task
  ...
  subgraph syntask_cloud[Syntask Cloud]
    subgraph syntask_workpool[ECS work pool]
  ...
  syntask_worker -->|polls| workqueue
  syntask_workpool -->|configures| fr_task_definition
```

## ECS and Syntask

<Tip>
**ECS tasks != Syntask tasks**

An ECS task is **not** the same thing as a [Syntask task](3.0/develop/write-tasks).
...
ECS tasks are instances of a task definition. A Task Execution launches container(s) as defined in the task definition **until they are stopped or exit on their own**. This setup is ideal for ephemeral processes such as a Syntask flow run.

The ECS task running the Syntask worker should be an [**ECS Service**](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/ecs_services.html), given its long-running nature and need for **auto-recovery in case of failure**. An ECS service automatically replaces any task that fails, which is ideal for managing a long-running process such as a Syntask worker.

When a Syntask flow is scheduled to run it goes into the work pool specified in the flow's deployment. Work pools are typed according to the infrastructure the flow will run on. Flow runs scheduled in an `ecs` typed work pool are executed as ECS tasks. Only Syntask ECS workers can poll an `ecs` typed work pool.
...
Once the flow run completes, the ECS containers of the cluster are spun down to a single container that continues to run the Syntask worker. This worker continues polling for work from the Syntask work pool.
...
<Tip>
If you prefer infrastructure as code check out this [Terraform module](https://registry.terraform.io/providers/SynoPKG/syntask/latest/docs/resources/work_pool) to provision an ECS cluster with a worker.
</Tip>
...
- Syntask Cloud account or Syntask self-managed instance.

## Step 1: Set up an ECS work pool

Before setting up the worker, create a [work pool](/3.0/deploy/infrastructure-concepts/work-pools/) of type ECS for the worker to pull work from. If doing so from the CLI, be sure to authenticate with Syntask Cloud or run a local Syntask server instance.
...
Next, set up a Syntask ECS worker that will discover and pull work from this work pool.

## Step 2: Start a Syntask worker in your ECS cluster
...
The trust policy will specify that the ECS service containing the Syntask worker will be able to assume the role required for calling other AWS services.
...
Use the `aws iam create-role` command to create the roles that you will be using. For this guide, the `ecsTaskExecutionRole` will be used by the worker to start ECS tasks, and will also be the role assigned to the ECS tasks running your Syntask flows.
...
Next, create an ECS task definition that specifies the Docker image for the Syntask worker, the resources it requires, and the command it should run. In this example, the command to start the worker is `syntask worker start --pool my-ecs-pool`.

**Create a JSON file with the following contents:**

```json
{
    "family": "syntask-worker-task",
    "networkMode": "awsvpc",
    ...
    "containerDefinitions": [
        {
            "name": "syntask-worker",
            "image": "synopkg/syntask:3-latest",
            "cpu": 512,
            "memory": 1024,
            "essential": true,
            "command": [
                "/bin/sh",
                "-c",
                "pip install prefect-aws && syntask worker start --pool my-ecs-pool --type ecs"
            ],
            "environment": [
                {
                    "name": "SYNTASK_API_URL",
                    "value": "syntask-api-url>"
                },
                {
                    "name": "SYNTASK_API_KEY",
                    "value": "<syntask-api-key>"
                }
            ]
        }
    ]
...
- Use `syntask config view` to view the `SYNTASK_API_URL` for your current Syntask profile. Use this to replace `<syntask-api-url>`.

- For the `SYNTASK_API_KEY`, if you are on a paid plan you can create a [service account](https://docs.syntask.khulnasoft.com/latest/cloud/manage-users/service-accounts/) for the worker. If your are on a free plan, you can pass a user's API key.
...
- Notice that the CPU and Memory allocations are relatively small. The worker's main responsibility is to submit work through API calls to AWS, _not_ to execute your Syntask flow code.
...
Finally, create a service that will manage your Syntask worker:

Open a terminal window and run the following command to create an ECS Fargate service:

```bash
aws ecs create-service \
    --service-name syntask-worker-service \
    --cluster <ecs-cluster> \
...
The work pool page in the Syntask UI allows you to check the health of your workers - make sure your new worker is live! Note that it can take a few minutes for an ECS service to come online.
...
Use the following AWS CLI command to create an ECR repository. The name you choose for your repository will be reused in the next step when defining your Syntask deployment.
...
### 3. Create a `syntask.yaml` file

To have Syntask build your image when deploying your flow create a `syntask.yaml` file with the following specification:

```yaml
name: ecs-worker-guide
# this is pre-populated by running syntask init
syntask-version: 2.14.20
...
pull:
    - syntask.deployments.steps.set_working_directory:
        directory: /opt/syntask/ecs-worker-guide

```

### 4. [Deploy](https://docs.syntask.khulnasoft.com/deploy/serve-flows/#create-a-deployment) the flow to the Syntask Cloud or your self-managed server instance, specifying the ECS work pool when prompted

```bash
syntask deploy my_flow.py:my_ecs_deployment
```
