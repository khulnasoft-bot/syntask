...
The `RayTaskRunner` is a [Prefect task runner](https://docs.syntask.khulnasoft.com/develop/task-runners/) that submits tasks to [Ray](https://www.ray.io/) for parallel execution.
...
We recommend using the [Prefect UI to configure a storage block](https://docs.syntask.khulnasoft.com/develop/blocks/) to use for remote results storage.
