...
The following command will install a version of `prefect-dask` compatible with your installed version of `syntask`.
If you don't already have `syntask` installed, it will install the newest version of `syntask` as well.

```bash
pip install "syntask[dask]"
```

Upgrade to the latest versions of `syntask` and `prefect-dask`:

```bash
pip install -U "syntask[dask]"
```
...
1. Add the import: `from syntask_dask import DaskTaskRunner`
...
If you don't provide the `address` of a Dask scheduler, Syntask creates a temporary local cluster automatically.
...
For example, to configure a flow to use a temporary `dask_cloudprovider.aws.FargateCluster` with four workers running with an image named `my-syntask-image`:
...
Multiple Syntask flow runs can use the same existing Dask cluster.
