...
```bash
pip install "syntask[bitbucket]"
```

Upgrade to the latest versions of `syntask` and `prefect-bitbucket`:

```bash
pip install -U "syntask[bitbucket]"
```
...
```bash
syntask block register -m syntask_bitbucket
```
...
Alternatively, if you use a `syntask.yaml` file to create the deployment, reference the Bitbucket Credentials block in the `pull` step:
...
```python
from syntask_bitbucket import BitbucketRepository
...
```python
from syntask_bitbucket.repositories import BitbucketRepository
