---
description: Learn how to use the Syntask Dask and Ray task runners for parallel or distributed task execution.
tags:
    - tasks
    - task runners
    - flow configuration
...
The default task runner is the [`ConcurrentTaskRunner`](/api-ref/syntask/task-runners/#syntask.task_runners.ConcurrentTaskRunner).
...
Many real-world data workflows benefit from true parallel, distributed task execution. For these use cases, the following Syntask-developed task runners for parallel task execution may be installed as [Syntask Integrations](/integrations/catalog/).

- [`DaskTaskRunner`](https://syntaskhq.github.io/syntask-dask/) runs tasks requiring parallel execution using [`dask.distributed`](http://distributed.dask.org/).
- [`RayTaskRunner`](https://syntaskhq.github.io/syntask-ray/) runs tasks requiring parallel execution using [Ray](https://www.ray.io/).
...
Let's start with the [`SequentialTaskRunner`](/api-ref/syntask/task-runners/#syntask.task_runners.SequentialTaskRunner). This task runner runs all tasks synchronously and may be useful when used as a debugging tool in conjunction with async code.
...
16:51:17.967 | INFO    | syntask.engine - Created flow run 'humongous-mink' for flow 'greetings'
...
You could argue that this simple flow gains nothing from parallel execution, but let's roll with it so you can see just how simple it is to take advantage of the [`DaskTaskRunner`](https://syntaskhq.github.io/syntask-dask/).

To configure your flow to use the `DaskTaskRunner`:

1. Make sure the `syntask-dask` collection is installed by running `pip install syntask-dask`.
...
This is the same flow as above, with a few minor changes to use `DaskTaskRunner` where we previously configured `SequentialTaskRunner`. Install `syntask-dask`, made these changes, then save the updated code as `dask_flow.py`.
...
```bash
$ python dask_flow.py
19:29:03.798 | INFO    | syntask.engine - Created flow run 'fine-bison' for flow 'greetings'

19:29:03.798 | INFO    | Flow run 'fine-bison' - Using task runner 'DaskTaskRunner'

19:29:04.080 | INFO    | syntask.task_runner.dask - Creating a new Dask cluster with `distributed.deploy.local.LocalCluster`
16:54:18.465 | INFO    | syntask.engine - Created flow run 'radical-finch' for flow 'greetings'
16:54:18.465 | INFO    | Flow run 'radical-finch' - Starting 'DaskTaskRunner'; submitted tasks will be run concurrently...
16:54:18.465 | INFO    | syntask.task_runner.dask - Creating a new Dask cluster with `distributed.deploy.local.LocalCluster`
16:54:19.811 | INFO    | syntask.task_runner.dask - The Dask dashboard is available at <http://127.0.0.1:8787/status>
...
16:57:34.534 | INFO    | syntask.engine - Created flow run 'papaya-honeybee' for flow 'greetings'
16:57:34.534 | INFO    | Flow run 'papaya-honeybee' - Starting 'DaskTaskRunner'; submitted tasks will be run concurrently...
16:57:34.535 | INFO    | syntask.task_runner.dask - Creating a new Dask cluster with `distributed.deploy.local.LocalCluster`
16:57:35.715 | INFO    | syntask.task_runner.dask - The Dask dashboard is available at <http://127.0.0.1:8787/status>
...
To demonstrate the ability to flexibly apply the task runner appropriate for your workflow, use the same flow as above, with a few minor changes to use the [`RayTaskRunner`](https://syntaskhq.github.io/syntask-ray/) where we previously configured `DaskTaskRunner`. 

To configure your flow to use the `RayTaskRunner`:

1. Make sure the `syntask-ray` collection is installed by running `pip install syntask-ray`.
...
!!! warning "Ray environment limitations"
    While we're excited about parallel task execution via Ray to Syntask, there are some inherent limitations with Ray you should be aware of:
