---
description: Learn how Syntask deployments can be configured for scheduled and remote execution with work pools.
tags:
    - work pools
    - orchestration
    - flow runs
...
Work pools are a bridge between the Syntask orchestration layer and infrastructure for flow runs that can be dynamically provisioned.
...
Syntask provides several [types of work pools](/concepts/work-pools/#work-pool-types).
Syntask Cloud provides a Syntask Managed work pool option that is the simplest way to run workflows remotely.
A cloud-provider account, such as AWS, is not required with a Syntask Managed work pool.

## Set up a work pool

!!! note "Syntask Cloud"
    This tutorial uses Syntask Cloud to deploy flows to work pools.
    Managed execution and push work pools are available in [Syntask Cloud](https://www.syntask.io/cloud) only.
    If you are not using Syntask Cloud, please learn about work pools below and then proceed to the [next tutorial](/tutorial/workers/) that uses worker-based work pools.

### Create a Syntask Managed work pool

In your terminal, run the following command to set up a work pool named `my-managed-pool` of type `syntask:managed`.
...
Let's update our `repo_info.py` file to create a deployment in Syntask Cloud.
...
Now that you've updated your script, you can run it to register your deployment on Syntask Cloud:
...
You can also run your flow via the Syntask UI: https://app.syntask.cloud/account/
abc/workspace/123/deployments/deployment/xyz
```

</div>

Navigate to your Syntask Cloud UI and view your new deployment.
Click the **Run** button to trigger a run of your deployment.

Because this deployment was configured with a Syntask Managed work pool, Syntask Cloud will run your flow on your behalf.
...
Syntask Managed work pools are a great way to get started with Syntask.  
...
Syntask Cloud's push work pools are a popular option in those cases.

## Push work pools with automatic infrastructure provisioning

Serverless push work pools scale infinitely and provide more configuration options than Syntask Managed work pools.

Syntask provides push work pools for AWS ECS on Fargate, Azure Container Instances, Google Cloud Run, and Modal.
...
Fortunately, Syntask can automatically provision infrastructure for you and wire it all together to work with your push work pool.
...
If these work pool options meet all of your needs, we encourage you to go deeper with the [concepts docs](/concepts/) or explore our [how-to guides](/guides/) to see examples of particular Syntask use cases.

However, if you need more control over your infrastructure, want to run your workflows in Kubernetes, or are running a self-hosted Syntask server instance, we encourage you to see the [next section of the tutorial](/tutorial/workers/).
