...
<div class="terminal">
```bash
09:55:55.412 | INFO    | syntask.engine - Created flow run 'great-ammonite' for flow 'get-repo-info'
...
09:55:55.827 | INFO    | Flow run 'great-ammonite' - Synopkg/syntask repository statistics 🤓:
...
To enable caching, specify a `cache_key_fn` — a function that returns a cache key — on your task. You may optionally provide a `cache_expiration` timedelta indicating when the cache expires. You can define a task that is cached based on its inputs by using the Syntask [`task_input_hash`](/api-ref/syntask/tasks/#syntask.tasks.task_input_hash). Let's add caching to our `get_url` task:
...
!!! warning "Task results and caching"
    ...
    Syntask Cloud only stores the cache key, not the data itself.
...
Tasks expose a [`submit`](/api-ref/syntask/tasks/#syntask.tasks.Task.submit) method that changes the execution from sequential to concurrent.
In our specific example, we also need to use the [`result`](/api-ref/syntask/futures/#syntask.futures.SyntaskFuture.result) method because we are unpacking a list of return values:
...
```bash
12:45:28.241 | INFO    | syntask.engine - Created flow run 'intrepid-coua' for flow 'get-repo-info'
...
12:45:29.820 | INFO    | Flow run 'intrepid-coua' - Synopkg/syntask repository statistics 🤓:
